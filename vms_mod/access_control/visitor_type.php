...
$title   = 'Visitor Type Setup';
$table   = 'vms_visitor_type';
$unique  = 'id';
$shown   = 'visitor_type_name';
$page    = 'visitor_type.php';
$tr_from = 'VMS';
$tr_type = 'Show';
$tr_no   = 0;
$tr_id   = 0;
...
if(isset($_POST[$shown])){
    ...
    if(isset($_POST['insert'])){
        ...
        header("Location: ".$page."?msg=Visitor+Type+Added+Successfully"); exit;
    }
...
<!-- ============ LEFT — LIST ============ -->
  <div class="col-sm-6 col-md-6 col-lg-6 col-xl-6 col-6 setup-left">
    ...
    <form class="n-form1 pt-0" method="post" action="">
        <h4 align="center" class="n-form-titel1">Search Visitor Type</h4>

        <input type="hidden" name="<?=$shown?>" value="1">

        <div class="form-group row m-0 pl-3 pr-3">
            <label class="col-sm-3 pl-0 pr-0 col-form-label">Type Name</label>
    ...
    <table id="table_head" class="table1 table-striped table-bordered table-hover table-sm" cellspacing="0">
        ...
        if(isset($_POST['search'])){
            if(!empty($_POST['f_name']))   $con = " AND visitor_type_name LIKE '%".addslashes($_POST['f_name'])."%'";
        ...
        <tr onclick="DoNav('<?=$row->id?>')" style="cursor:pointer;">
            <td><?=$sl++?></td>
            <td style="text-align:left;font-weight:600;"><?=htmlspecialchars($row->visitor_type_name)?></td>
            <td style="text-align:left;font-size:12px;color:#6c757d;">
                <?php
                $desc = !empty($row->description) ? $row->description : '';
                echo $desc
                    ? htmlspecialchars(strlen($desc)>60 ? substr($desc,0,60).'...' : $desc)
                    : '-';
                ?>
            </td>
            <td>
...
<form class="n-form setup-fixed" action="<?=$page?>" method="post" name="form1" id="form1">
    ...
    <h4 align="center" class="n-form-titel1">
        <?= $$unique > 0 ? 'Update Visitor Type' : 'Create Visitor Type' ?>
    </h4>

    <input type="hidden" name="<?=$unique?>"  value="<?=$$unique?>">
    <input type="hidden" name="<?=$shown?>"   id="vt_trigger"
           value="<?=htmlspecialchars($visitor_type_name)?>">
    ...
    <div class="form-group row m-0 pl-3 pr-3">
        <label class="col-sm-4 pl-0 pr-0 col-form-label ">Type Name</label>
        <div class="col-sm-8 p-0">
            <input type="text" name="visitor_type_name" required
                   value="<?=htmlspecialchars($visitor_type_name)?>"
                   placeholder="e.g. Guest, Vendor, Contractor"
                   oninput="document.getElementById('vt_trigger').value=this.value">
        </div>
    </div>
    ...
    <div class="form-group row m-0 pl-3 pr-3">
        ...
        <div class="col-sm-8 p-0">
            <textarea name="description" rows="3"
                      placeholder="Notes for this visitor type..."><?=htmlspecialchars($description)?></textarea>
        </div>
    </div>
    ...
    <div class="n-form-btn-class">
        ...
        <input name="delete" type="submit" value="DELETE" class="btn1 btn1-bg-delete"
               onclick="return confirm('Delete this visitor type?')">
        <?php } ?>
