...
$title   = 'Visitor Pre-Registration List';
$table   = 'vms_pre_registration';
$unique  = 'id';
$page    = 'visitor_unaproved_view.php';
...
if(isset($_POST['quick_id']) && isset($_POST['quick_action'])){
    ...
    db_query("UPDATE vms_pre_registration
              SET status='".$qstat."',
                  approved_by=".$uid.",
                  approved_at='".date('Y-m-d H:i:s')."'
              WHERE id=".$qid." AND group_for='".$grp."'");
            ...
            <div class="form-group row m-0">
                ...
                <div class="col-sm-8 col-md-8 col-lg-8 col-xl-8 p-0 pr-2">
                    <select name="f_status" id="f_status" class="form-control">
                        ...
                        <?
                        foreach(['Pending','Approved','Rejected','CheckedIn','Expired'] as $st){
...
<table  id="table_head"  class="table1 table-striped table-bordered table-hover table-sm">
    ...
    <tbody class="tbody1">
    <?
    $con = " AND p.group_for='".$grp."'";

    if(isset($_POST['submitit'])){
        if(!empty($_POST['f_date']))   $con .= " AND p.visit_date='".addslashes($_POST['f_date'])."'";
        if(!empty($_POST['f_status'])) $con .= " AND p.status='".addslashes($_POST['f_status'])."'";
    ...
    while($row = mysqli_fetch_object($query)){
        $bg  = isset($bc[$row->status]) ? $bc[$row->status][0] : '#6c757d';
        $txt = isset($bc[$row->status]) ? $bc[$row->status][1] : '#fff';
    ?>
    <tr>
        <td><?=$sl++?></td>
        ...
        <td style="white-space:nowrap;">
            <button type="button" class="btn2 btn1-bg-submit" onclick="parent.location='visitor_pre_reg_view.php?id=<?=$row->id?>'">
                <i class="fa-solid fa-eye"></i>
            </button>
            <? if($row->status == 'Pending'){ ?>
            <button type="button" class="btn2 btn1-bg-submit" onclick="parent.location='visitor_pre_registration.php?id=<?=$row->id?>'">
                <i class="fa-solid fa-pen-to-square"></i>
            </button>
<!--            <button type="button" class="btn2 btn1-bg-submit" style="background:#28a745;color:#fff;border:none;" onclick="quick_action(<?=$row->id?>,'approve')">
