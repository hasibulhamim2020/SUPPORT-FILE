...
$del = "DELETE FROM sale_target_area WHERE 1 and target_year='".$_POST['target_year']."' and target_month='".$_POST['target_month']."' ";
db_query($del); 
// end delete
...
for($b=1;$b<=$s;$b++){
	...
	for($g=3;$g<=$x;$g++)
	{
			if($data[$b][$g]>0 && $i_dp[$data[$b][1]]>0){
			

		$sql = "INSERT INTO sale_target_area  (target_year,target_month,target_period,area_id,area_name,item_id,target_qty,        price, target_amt, entry_by, entry_at 	
			)VALUES(
			'".$target_year."','".$target_month."','".$target_period."','".$data[0][$g]."',   ''   ,   '".$i_id[$data[$b][1]]."','".$data[$b][$g]."'
