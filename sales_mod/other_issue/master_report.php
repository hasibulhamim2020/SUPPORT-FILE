...
{$to_date=$t_date; $fr_date=$f_date; $date_con=' and a.po_date between \''.$fr_date.'\' and \''.$to_date.'\'';}
	...
	break;
	case 11010:
	$report='Inventory Other Issue Report';
	break;
    case 2:
...
if($_POST['f_date']!='')
{
$date_con=' and m.oi_date between "'.$_POST['f_date'].'" and "'.$_POST['t_date'].'"';

}


$sql="select m.oi_no,i.item_name,m.oi_date,
w.warehouse_name as depot,s.rate,s.qty,s.amount,m.issued_to,m.approved_by,m.status

from warehouse_other_issue m,warehouse_other_issue_detail s, warehouse w, item_info i

where m.status in ('CHECKED','COMPLETED') and m.oi_no=s.oi_no and i.item_id=s.item_id
".$depot_con.$date_con.$item_con.$vondor_con.$dtype_con." order by m.oi_date,m.oi_no";
...
<td><?=$data->rate?></td>
<td><?=$data->qty?></td>
<td style="text-align:right"><?=$data->amount; $total=$total+$data->amount?></td>
