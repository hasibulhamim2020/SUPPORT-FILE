...
$title='Pending WO Unhold Request';
do_calander('#fdate');
do_calander('#tdate');
...
$target_url = '../wo/wo_unhold_request_approval.php';
  ...
  <form action="" method="post" name="codz" id="codz">
    <table width="80%" border="0" align="center">
      ...
      <tr>
        ...
        <? foreign_relation('sale_do_unhold_request','do_no','job_no',$_POST['do_no'],'status="UNCHECKED"');?>
    </select>
...
$con .= ' and m.do_no in ('.$_POST['do_no'].') ';
 ...
 //$sql = "select d.do_no, count(d.revise_request) as revise_request  from sale_do_master m, sale_do_details d where m.do_no=d.do_no and d.revise_request=1 group by d.do_no ";
//		 $query = db_query($sql);
//		 while($info=mysqli_fetch_object($query)){
//  		 $revise_request[$info->do_no]=$info->revise_request;
//		}



   $res="select m.do_no, m.do_no, m.job_no, m.dealer_code, m.do_date,   m.buyer_code, m.merchandizer_code, m.status from sale_do_master m, sale_do_unhold_request d where m.do_no=d.do_no and d.status='UNCHECKED' and  m.status='CHECKED' ".$con." group by m.do_no order by m.do_date, m.do_no ";
