...
$destination_count= find_a_field('sale_do_chalan_cancel','COUNT(destination)','chalan_no="'.$chalan_no.'" and destination!=""');
$referance_count= find_a_field('sale_do_chalan_cancel','COUNT(referance)','chalan_no="'.$chalan_no.'" and referance!=""');
$sku_no_count= find_a_field('sale_do_chalan_cancel','COUNT(sku_no)','chalan_no="'.$chalan_no.'" and sku_no!=""');
$pack_type_count= find_a_field('sale_do_chalan_cancel','COUNT(pack_type)','chalan_no="'.$chalan_no.'" and pack_type!=""');
$color_count= find_a_field('sale_do_chalan_cancel','COUNT(color)','chalan_no="'.$chalan_no.'" and color!=""');
$size_count= find_a_field('sale_do_chalan_cancel','COUNT(size)','chalan_no="'.$chalan_no.'" and size!=""');

$do_no= find_a_field('sale_do_chalan_cancel','do_no','chalan_no='.$chalan_no);

$master= find_all_field('sale_do_master','','do_no='.$do_no);


$ch_data= find_all_field('sale_do_chalan_cancel','','chalan_no='.$chalan_no);
...
$to_ctn = find_a_field('sale_do_chalan_cancel','sum(pkt_unit)','chalan_no='.$chalan_no);

$to_pcs = find_a_field('sale_do_chalan_cancel','sum(dist_unit)','chalan_no='.$chalan_no); 
		  ...
		  <tr>
		    ...
		    <td width="100%" valign="top">
		      ...
		      <table width="100%" border="0" cellspacing="0" cellpadding="3"  style="font-size:12px">
		        ...
		        <tr>
		          <td width="10%" align="left" valign="middle">Product</td>
		          <td width="31%">:	&nbsp;Carton</td>
	              <td width="6%">PO No </td>
	              <td width="10%">: &nbsp;<?=$master->customer_po_no;?></td>
		        ...
		        <tr>
		          <td align="left" valign="middle">Customer Name</td>
		          <td>:	&nbsp;
    ...
    <td><div id="pr">
      ...
      <table width="100%" class="tabledesign" border="1" bordercolor="#000000" cellspacing="0" cellpadding="5"  style="font-size:10px">
        ...
        <?    $sqlc = 'select c.order_no, c.item_name as ch_item_name, c.delivery_date, c.delivery_place,c.printing_info,c.additional_info, c.measurement_unit, sum(c.total_unit) as total_unit,
		 ...
		 from sale_do_chalan_cancel c, item_info i,  item_sub_group s, item_group g where i.item_id=c.item_id and i.sub_group_id=s.sub_group_id and s.group_id=g.group_id  and c.chalan_no='.$chalan_no.' group by c.id, c.total_unit order by c.id asc';
  ...
  <tr>
  	<td colspan="2">
		...
		sum(c.bundle_1+c.bundle_2+c.bundle_3) as tot_bundle
		 from sale_do_chalan_cancel c, item_info i,  item_sub_group s, item_group g where i.item_id=c.item_id and i.sub_group_id=s.sub_group_id and s.group_id=g.group_id  and c.chalan_no='.$chalan_no.' group by c.item_id order by s.sub_group_id ';
			$queryc=db_query($sqlc);
			while($datac = mysqli_fetch_object($queryc)){
	...
	<table width="100%" cellspacing="0" cellpadding="0"  >
		...
		<tr>
		  ...
		  <td align="left"><strong>Customer' Acknowledgement </strong></td>
		  </tr>
		<tr>
		  <td align="center" >&nbsp;</td>
		  <td align="center">&nbsp;</td>
		  <td align="left">Certified that the particular given goods are received in good condition and quantity as per challan.</td>
		  </tr>
		<tr>
		  <td align="center" >&nbsp;</td>
		  <td align="center">&nbsp;</td>
		...
		<tr style="font-size:12px">
		    ...
		    <td  align="center" width="25%"><strong><?= find_a_field('authorized_by','authorized_by','id='.$ch_data->authorized_by);?></strong></td>
		    <td  align="center" width="25%"><strong>Receiver Name &amp; Signature With Seal &amp; Date </strong></td>
		    </tr>
		<tr style="font-size:12px">
    ...
    <td><div class="header">
        ...
        <!--<td bgcolor="#666666" style="text-align:center; color:#FFF; font-size:18px; font-weight:bold;">WORK ORDER </td>-->
					...
					<tr>
					  ...
					  <td align="center"><h4 style="font-size:16px;">Acknowledgement Copy</h4></td>
    ...
    <td>
      ...
      <table width="100%" class="tabledesign" border="1" bordercolor="#000000" cellspacing="0" cellpadding="5"  style="font-size:10px">
        ...
        <tr style="font-size:10px;">
          ...
          <td align="center" valign="top"><?=$datac->unit_name;?></td>
          <td align="center" valign="top"><?= find_a_field('sale_do_details','total_unit','id="'.$datac->order_no.'"');?></td>
          <td align="center" valign="top"><?=number_format($datac->total_unit,2); $grand_tot_unit3 +=$datac->total_unit; ?></td>
          <td align="center" valign="top">
		  (<? if($datac->qty_1>0) {?><?=$datac->pcs_1?>Pcs<strong>X</strong><?=$datac->bundle_1?>Bndl<? }?><? if($datac->qty_2>0) {?><strong>+</strong><?=$datac->pcs_2?>Pcs<strong>X</strong><?=$datac->bundle_2?>Bndl<? }?><? if($datac->qty_3>0) {?><strong>+</strong><?=$datac->pcs_3?>Pcs<strong>X</strong><?=$datac->bundle_3?>Bndl<? }?>) </td>
        </tr>
        ...
        <td align="center" valign="middle">&nbsp;</td>
        <td align="center" valign="middle">&nbsp;</td>
        <td align="center" valign="middle"><strong><?=number_format($grand_tot_unit3,2) ;?></strong></td>
        <td align="center" valign="middle">&nbsp;</td>
        </tr>
      </table>
  	...
  	<td colspan="2">
		...
		sum(c.bundle_1+c.bundle_2+c.bundle_3) as tot_bundle
		 from sale_do_chalan_cancel c, item_info i,  item_sub_group s, item_group g where i.item_id=c.item_id and i.sub_group_id=s.sub_group_id and s.group_id=g.group_id  and c.chalan_no='.$chalan_no.' group by c.item_id order by s.sub_group_id ';
			...
			?>
        <tr style="font-size:12px;">
          <td align="center" valign="top"><?=++$kksm3;?></td>
          <td align="center" valign="top"><?
		  if ($datac->ch_item_name=="") {
		   echo $datac->item_name;
		  } else {
		  ...
		  ?></td>
          <td align="center" valign="top"><?=number_format($datac->tot_bundle,2);  $tot_bundle_sum3 +=$datac->tot_bundle;?></td>
          <td align="center" valign="top"><?=number_format($datac->total_unit,2); $tot_unit_sum3 +=$datac->total_unit; ?></td>
          </tr>
        ...
        <tr style="font-size:12px;">
        <td colspan="2" align="right" valign="middle"><strong> Total:</strong></td>
        <td align="center" valign="middle"><strong><?=number_format($tot_bundle_sum3,2);?></strong></td>
        <td align="center" valign="middle"><strong><?=number_format($tot_unit_sum3,2) ;?></strong></td>
        </tr>
