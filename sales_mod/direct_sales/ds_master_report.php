...
switch ($_POST['report']) {
    case 1:
		...
		if(isset($dealer)) 				        {$dealer_con=' and m.vendor_id='.end(explode("#",$dealer));} 
		elseif(isset($item_id)) 				{$item_con=' and s.item_id='.$item_id;} 
		if(isset($t_date)) 
		{$to_date=$t_date; $fr_date=$f_date; $date_con=' and m.oi_date between \''.$fr_date.'\' and \''.$to_date.'\'';}

		
		 $sql='select m.oi_no as DS_no,m.oi_date as DS_date,m.status,m.vendor_name as Customer,m.warehouse_id,i.item_name,s.qty,s.rate,s.amount, m.entry_by, w.warehouse_id, wd.warehouse_id, wd.user_id
		 from warehouse_other_issue m, warehouse_other_issue_detail s, item_info i, warehouse w, warehouse_define wd
		 where i.item_id=s.item_id and m.status="CHECKED" and m.oi_no=s.oi_no and m.warehouse_id=w.warehouse_id and w.warehouse_id=wd.warehouse_id and m.entry_by=wd.user_id and wd.status="Active" and m.issue_type="DirectSales" '.$warehouse_con.$item_sub_con.$dealer_con.$item_con.$date_con.'';
...
<table width="100%" cellspacing="0" cellpadding="2" border="0" id="ExportTable"><thead><tr><td style="border:0px;" colspan="9"><div class="header"><h1>ZISAN FOOD & BEVERAGE LTD</h1><h2><?=$report?></h2>
...
if(isset($garden_id)) 	{$garden_id_con=' and b.garden_id='.$garden_id;}
 ...
 $res='
select a.req_no as req_no, a.req_date, a.entry_by, a.entry_at, b.qty as qty, i.item_name as item_name from requisition_master a, requisition_order b, item_info i,item_sub_group g where a.req_no=b.req_no and i.item_id=b.item_id and i.sub_group_id=g.sub_group_id '.$con.$item_con.$sub_group_con.' order by a.req_no';
...
?>
    ...
    <tr>
	  ...
	  <td valign="top" style="border:none;">&nbsp;</td>
	  <td valign="top" style="border:none;">&nbsp;</td>
	  <td valign="top" style="border:none; font-weight:bold; text-align:right;">Total Quantity</td>
