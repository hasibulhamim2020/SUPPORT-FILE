...
$do_sql = "select a.*, m.do_no,m.depot_id,m.sales_type, m.do_date,s.sales_return_type from sale_return_master m, sale_return_details a, item_info b, sales_return_type s where m.sales_type=s.id and  m.do_no=a.do_no and  b.item_id=a.item_id  and a.do_no='".$or_no."' order by a.id ";
...
$do_query = db_query($do_sql);	
	...
	journal_item_control($do_data->item_id, $do_data->depot_id, $do_data->do_date, $do_data->total_unit, 0,  $do_data->sales_return_type, $do_data->id, $do_data->unit_price, $do_data->depot_id, $do_data->do_no, '', '',$do_master->group_for, $do_data->unit_price, '' );
