...
getData2('edit_accept_ajax.php', 'ppp'+id,id,info);
}
...
<td align="center">
<span id="ppp<?=$data->id?>"><? if ($data->edit_request==1) {?><input name="<?='edit#'.$data->id?>" type="button" id="Edit" value="Pending" style="width:100px; background:#FF0000; height:30px; color:#000; font-weight:700" onclick="update_edit(<?=$data->id?>);start(this);"  /> <? }?>
...
<form action="confirm_all_edit_accept.php" method="post" name="cz" id="cz" onSubmit="if(!confirm('Are You Sure Execute this?')){return false;}">
...
<input name="confirm" type="submit" class="btn1" value="ACCEPT ALL" style="width:270px; font-weight:bold; font-size:12px; height:30px; color:#090; float:right" />
