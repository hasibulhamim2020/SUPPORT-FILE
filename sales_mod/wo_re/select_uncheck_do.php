...
$title='Unapprove Work Order List';
do_calander('#fdate');
do_calander('#tdate');
...
if(isset($_POST['confirmm']))
{
		...
		$_POST['checked_at'] = date('Y-m-d H:i:s');
		$_POST['checked_by'] = $_SESSION['user']['id'];
		$_POST['sa_approved_by'] = $_SESSION['user']['id'];
		$_POST['di_approved_by'] = $_SESSION['user']['id'];
		$_POST['sa_approved_at'] = date('Y-m-d H:i:s');
		$_POST['di_approved_at'] = date('Y-m-d H:i:s');
		$_POST[$unique_master]=$$unique_master;
		$_POST['sa_approval']='APPROVED';
		...
		unset($_SESSION[$unique_master]);
		$type=1;
		echo $msg='<span style="color:green;">Successfully Instructed To Delivery Challan</span>';
		$tr_type="Complete";
}
...
where m.status in ('PROCESSING') and m.received_amt>=0 and received_status='received' and m.sa_approval='PENDING' and m.dealer_type='Distributor' and m.dealer_code=d.dealer_code ".$con."
and w.warehouse_id=m.depot_id and m.entry_by=u.user_id order by m.do_date desc,d.dealer_name_e";
                    $query = db_query($res);
    ...
    <table width="80%" border="0" align="center">
      ...
      <tr>
        ...
        <td width="1" bgcolor="#00bfff"><strong>
          <input type="text" name="fdate" id="fdate" class="from-control" style=" width:200px;"  value="<?=($_POST['fdate']!='')?$_POST['fdate']:date('Y-m-01')?>" autocomplete="off"/>
        </strong></td>
        <td align="center" bgcolor="#00bfff"><strong> -to- </strong></td>
        <td width="1" bgcolor="#00bfff"><strong>
          <input type="text" name="tdate" id="tdate"  style=" width:200px;" class="from-control" value="<?=($_POST['tdate']!='')?$_POST['tdate']:date('Y-m-d')?>" autocomplete="off"/>
        </strong></td>
...
$res="select m.do_no,m.do_date,concat(d.dealer_code,'-',d.dealer_name_e) as dealer_name,m.entry_at,u.fname,w.warehouse_name
from sale_do_master m,dealer_info d, warehouse w, user_activity_management u
where m.status in ('PROCESSING')  and m.dealer_code=d.dealer_code ".$con." 
