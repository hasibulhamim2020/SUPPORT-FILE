...
<div class="form-container_large">
    ...
    <form action="" method="post" name="codz" id="codz">
        ...
        <div class="container-fluid bg-form-titel">
            <div class="row">
			<div class="col-sm-10 col-md-10 col-lg-10 col-xl-10 pt-1 pb-1 row">
                ...
                <div class="col-sm-6 col-md-6 col-lg-6 col-xl-6 m-0 pt-1 pb-1">
                    <div class="form-group row m-0">
	...
	if($_SESSION['user']['id']==10171){
							$res="select m.do_no, m.do_no, m.do_date, m.dealer_code, m.job_no, m.sales_type, m.depot_id,  d.dealer_name_e, m.entry_by, m.status,m.checked_by,m.sa_approved_by,m.di_approved_by from 
					sale_do_master m, sale_do_details c,  dealer_info d, user_group u 
					where 
					  m.group_for=u.id  and m.dealer_code=d.dealer_code and m.do_no=c.do_no and m.dealer_type='Distributor' ".$group_for_con.$con.$dealer_con."  group by c.do_no order by m.do_no desc";
					...
					sale_do_master m, sale_do_details c,  dealer_info d, user_group u 
					where 
					  m.group_for=u.id  and m.dealer_code=d.dealer_code and m.do_no=c.do_no and m.entry_by='".$_SESSION['user']['id']."' and m.dealer_type='Distributor' ".$group_for_con.$con.$dealer_con."  group by c.do_no order by m.do_no desc";
					...
					?>
                        <tr> 
							...
							<td><?= find_a_field('warehouse','warehouse_name','warehouse_id='.$data->depot_id);?></td>
							<td><?= find_a_field('user_activity_management','fname','user_id='.$data->entry_by);?></td>
							<td><?= $data->status?></td>
							<td><?php
							if($data->checked_by<1){
							echo "<span style='color:red;font-weight:bold;'>Accounts Approval Pending</span>";
							}
							elseif($data->sa_approved_by<1 && $data->di_approved_by<1){
							echo "<span style='color:red;font-weight:bold;'>SA Approval Pending</span>";
							}
							elseif($data->di_approved_by<1){
							echo "<span style='color:red;font-weight:bold;'>CO-Ordinator Approval Pending</span>";
							}
							else{
							echo "<span style='color:green;font-weight:bold;'>Approved</span>";
							}
							?></td>
							<td><button class="btn2 btn1-bg-submit" onClick="custom(<?=$data->do_no;?>);"><i class="fa-solid fa-eye"></i></button></td>
