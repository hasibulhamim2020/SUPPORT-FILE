...
var strURL="challan_edit_accept_ajax.php?id="+id+"&edit_request="+edit_request+"&flag="+flag;
...
<table width="100%" border="0" align="center" id="grp" cellpadding="0" cellspacing="0" style="font-size:10px">
 ...
 // $op = find_all_field('journal_item','','warehouse_id="'.$_POST['warehouse_id'].'" and group_for="'.$_POST['group_for'].'" and item_id = "'.$data->item_id.'" and tr_from = "Opening" order by id desc');
  ...
  <tr bgcolor="<?=($i%2)?'#E8F3FF':'#fff';?>">
    ...
    <td><?=$data->pcs_3?></td>
    <td><?=$data->bundle_3?>
      <input name="edit_request_<?=$data->id?>" id="edit_request_<?=$data->id?>" type="hidden" size="10" maxlength="10" value="2" style="width:20px; height:20px;" /></td>
    <td><span id="divi_<?=$data->id?>">

            <?  if($data->edit_request==0)  { ?>

			   <input type="button" name="Button" value="Request"  style="width:80px; height:30px;  font-size:12px; background-color: #99CC99;"/><? }?>
			...
			<?  if($data->edit_request==1)  { ?>
			  ...
			  <input type="button" name="Button" value="Pending"  onclick="update_value(<?=$data->id?>)" style="width:80px; font-size:12px; height:30px;background-color:#FF0000"/>
