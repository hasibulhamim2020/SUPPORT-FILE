...
if(isset($_POST['confirm'])){
 ...
 $sql = 'update sale_do_master set status="UNHOLD", unhold_req_by="'.$entry_by.'", unhold_req_at="'.$entry_at.'" where do_no = '.$do_no;
	db_query($sql);
...
VALUES
( "'.$entry_date.'", "'.$do_no.'", "'.$ms_data->do_date.'", "'.$ms_data->job_no.'", "'.$ms_data->group_for.'", "'.$ms_data->dealer_code.'", "'.$ms_data->buyer_code.'",
 "'.$ms_data->merchandizer_code.'", "'.$ms_data->marketing_team.'", "'.$ms_data->marketing_person.'", "'.$remarks.'", "'.$ms_data->customer_po_no.'", 
 "'.$ms_data->customer_po_date.'", "UNCHECKED", "'.$entry_at.'", "'.$entry_by.'", "'.$ms_data->max_delivery_date.'"  )';
...
<? 
 ...
 $uh_status = find_a_field('sale_do_unhold_request','status','do_no="'.$$unique.'" and status="UNCHECKED"');
...
<input name="confirm" type="submit" class="btn1" value="WO UNHOLD REQUEST" style="width:280px; font-weight:bold; float:right; font-size:12px; height:30px; color:#090" /></td>
...
<td colspan="2" align="center" bgcolor="#FF3333"><strong>WORK ORDER UNHOLD REQUEST
 </strong></td>
