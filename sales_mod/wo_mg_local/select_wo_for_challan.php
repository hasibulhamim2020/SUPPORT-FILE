  ...
  <form action="" method="post" name="codz" id="codz">
    <table width="80%" border="0" align="center">
      ...
      <tr>
        ...
        <? foreign_relation('sale_do_master','do_no','job_no',$_POST['do_no'],'status not in ("CANCEL REQUEST","CANCELED","MANUAL","UNCHECKED","HOLD REQUEST","HOLD","UNHOLD")');?>
    </select>
...
$con .= ' and m.do_no in ('.$_POST['do_no'].') ';
  ...
  // $res="select m.do_no, m.do_no, m.job_no, m.dealer_code, m.do_date,   m.buyer_code, m.merchandizer_code, m.status, sum(d.total_unit) as wo_qty from sale_do_master m, sale_do_details d where m.do_no=d.do_no  and  m.status not in ('CANCEL REQUEST','CANCELED','MANUAL','UNCHECKED','HOLD REQUEST','HOLD','UNHOLD')  ".$con." group by m.do_no order by m.do_date, m.do_no ";

 $res="select  m.do_no, m.job_no, m.dealer_code, m.do_date,   m.buyer_code, m.merchandizer_code, m.status, sum(d.total_unit) as wo_qty
 
 
  from sale_do_master m, sale_do_details d 
  
 
 where m.do_no=d.do_no  and  m.status not in ('CANCEL REQUEST','CANCELED','MANUAL','UNCHECKED','HOLD REQUEST','HOLD','UNHOLD','COMPLETED') ".$con." group by m.do_no order by m.do_date, m.do_no ";
