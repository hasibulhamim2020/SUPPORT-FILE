...
{
$sql = 'UPDATE `sales_corporate_price` SET  `set_price` = "'.$set_price.'",`discount` = "'.$discount.'", `edit_by` = "'.$_SESSION['user']['id'].'", `edit_at` = "'.date('Y-m-d H:i:s').'" WHERE `dealer_code` = "'.$dealer_code.'" and `item_id` = "'.$item_id.'"';
}
db_query($sql);
echo 'Success!';
?>