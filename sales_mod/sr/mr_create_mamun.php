...
if(document.getElementById("warehouse_id").value>0)
  ...
  document.getElementById("req_date").focus();
	...
	<div class="form-container_large">
		...
		<form action="mr_create_mamun.php" method="post" name="codz" id="codz" onSubmit="if(!confirm('Are You Sure Execute this?')){return false;}">
						...
						<div class="container n-form2">
							...
							<div class="form-group row m-0 pb-1">
								...
								<div class="col-sm-8 col-md-8 col-lg-8 col-xl-8 p-0 pr-2">
									...
									<input name="<?=$field?>" type="text" id="req_date" id="<?=$field?>" value="<?=$$field?>"/>
<?php /*?><input  name="<?=$field?>" type="date" id="<?=$field?>" value="<?=$$field?>" /><?php */?>
								</div>
					...
					<div class="col-sm-6 col-md-6 col-lg-6 col-xl-6 ">
						...
						<div class="container n-form2">
							...
							<div class="form-group row m-0 pb-1">
								...
								<label for="<?=$field?>" class="col-sm-4 col-md-4 col-lg-4 col-xl-4 m-0 p-0 d-flex justify-content-end align-items-center pr-1 bg-form-titel-text">Requisition By :</label>
							...
							<div class="form-group row m-0 pb-1">
								...
								<div class="col-sm-8 col-md-8 col-lg-8 col-xl-8 p-0 pr-2">
									...
									<input  name="<?=$field?>" type="text" id="need_by" value="<?=$$field?>" autocomplete="off"/>
									
<?php /*?><input  name="<?=$field?>" type="date" id="<?=$field?>" value="<?=$$field?>" autocomplete="off"/><?php */?>
		...
		<form action="?<?=$unique?>=<?=$$unique?>" method="post" name="cloud" id="cloud">
				...
				<table class="table1  table-striped table-bordered table-hover table-sm">
					...
					<tr>
						<td>
							...
							<input  name="item_id" list="item_ids" type="text" id="item_id" value="<?=$item_id?>" required="required" onblur="getData2('mr_ajax_mamun.php', 'mr', this.value, document.getElementById('warehouse_id').value+'##'+document.getElementById('req_date').value);"/>
							
							<datalist id="item_ids">
								
								
								<? foreign_relation('item_info','concat(item_name,"#>",item_description,"#>",item_id)','concat(item_name,"#>",item_description,"#>",item_id)',$tt,'1 order by item_id asc');?>
							</datalist>
							</td>
						...
						<table style="width:100%;" border="1">
							...
							<tr>
								...
								<td width="20%"><input name="last_pur_rate" type="text" id=" 	last_pur_rate" class="form-control" onfocus="focuson('qty')" /></td>

								<td width="10%"><input name="ratio3m" type="text" id="ratio3m" readonly maxlength="100" class="form-control" onfocus="focuson('qty')" /></td>
			...
			<div class="container-fluid pt-5 p-0 ">
				...
				$res='select a.id,b.item_name as item_name,a.qoh as stock_qty,a.last_p_qty as last_pur_qty,a.last_p_date as last_pur_date,a.qty, a.ratio3m,"x" from requisition_order a,item_info b where b.item_id=a.item_id and a.req_no='.$req_no;
				...
				<table class="table1  table-striped table-bordered table-hover table-sm">
					...
					<tbody class="tbody1">
						...
						$res='select a.id,b.item_name as item_name,a.qoh as stock_qty,a.last_p_qty as last_pur_qty,a.last_p_date as last_pur_date,a.qty,a.unit_name,ratio3m,"x" from requisition_order a,item_info b where b.item_id=a.item_id and a.req_no='.$req_no;
