...
echo $sql_i='select finish_goods_code from item_info where  product_nature="Salable"';
	  ...
	  value="<? if($$unique_master>0){ echo $$unique_master;} else{ echo (find_a_field($table_master,'max('.$unique_master.')','1')+1); } ?>" readonly="readonly"/>
						...
						<div class="form-group row m-0 pb-1">
                            ...
                            <div class="col-sm-8 col-md-8 col-lg-8 col-xl-8 p-0 pr-2">
                                ...
                                <textarea name="delivery_address" id="delivery_address" ><?php if($delivery_address!=''){ echo $delivery_address;} else{ echo $dealer->address_e ;} ?></textarea>
                    ...
                    <div class="container n-form2">
						...
						<div class="form-group row m-0 pb-1">
                            ...
                            <div class="col-sm-8 col-md-8 col-lg-8 col-xl-8 p-0 pr-2">
                                ...
                                <input  name="do_date" type="text"  id="do_date" value="<?=$do_date?>" readonly="readonly"/>
                        ...
                        <div class="form-group row m-0 pb-1">
                            ...
                            <label class="col-sm-4 col-md-4 col-lg-4 col-xl-4 m-0 p-0 d-flex justify-content-end align-items-center pr-1 bg-form-titel-text">Attachement</label>

                            <div class="col-sm-8 col-md-8 col-lg-8 col-xl-8 p-0 pr-2">

							<?php if ($file_upload!=''){?>
		
		<a href="<?=SERVER_CORE?>core/upload_view.php?name=<?=$file_upload?>&folder=do&proj_id=<?=$_SESSION['proj_id']?>" target="_blank" >View Attachment</a>
			<?php } ?>
            ...
            <table style="padding: 5px; border-collapse:collapse; border:1px; font-size: 9px !important;">
                <tr>
                <td style="padding: 2px; border-right: none; text-align:right;" ><strong style="font-weight: bold;">Entry By : </strong> &nbsp;&nbsp;<?= find_a_field('user_activity_management', 'fname', 'user_id=' . $entry_by); ?></td>
                <!-- <td  style="padding: 7px;" align="left">
                    &nbsp;&nbsp;<?= find_a_field('user_activity_management', 'fname', 'user_id=' . $entry_by); ?>
                </td> -->
                <td style="padding: 2px; text-align:right;"<strong style="font-weight: bold;">Entry At : </strong>&nbsp;&nbsp;<?= date('d M Y, h:i:s A', strtotime($entry_at)); ?></td>
                <!-- <td style="padding: 7px;" align="left">
                    &nbsp;&nbsp;<?= date('H:i:s, d M Y', strtotime($entry_at)); ?>
                </td> -->
            <?php /*?>    <td style="padding: 2px; text-align:center;" >
                    <a href="sales_order_print_view.php?do_no=<?=rawurlencode(url_encode($do_no));?>" target="_blank">
				 ...
				 <td style="padding: 2px; text-align:center;" >
                    <a href="sales_order_print_view.php?c=<?=rawurlencode(url_encode($c_id));?>&v=<?=rawurlencode(url_encode($do_no));?>" target="_blank">
                    <img src="../../../../public/assets/images/print.png" style="width: 20px;" />
                    </a>
                </td>
                </tr>
        ...
        <div class="container-fluid pt-2 p-0 ">
            ...
            <table class="table1  table-striped table-bordered table-hover table-sm">
                ...
                <tbody class="tbody1">
					...
					$res='select a.id,b.finish_goods_code as code,b.item_name,a.unit_price,a.ctn_unit,a.pkt_unit as crt_qty,a.dist_unit as pcs ,a.total_amt,"X" from sale_do_details a,item_info b where b.item_id=a.item_id and a.do_no='.$$unique_master.' order by a.id';
                ...
                <tr>
					...
					<td><?=$data->pcs; $total_pcs+=$data->pcs; ?></td>
					...
					<td  style="text-align:center;"><?=number_format($total_crt,2)?></td>
					<td  style="text-align:center;"><?=number_format($total_pcs,2)?></td>
...
<?php 
$credit_bal=find_a_field('journal','sum(dr_amt-cr_amt)','sub_ledger="'.$dealer->sub_ledger_id.'"');
?>
    ...
    <form action="select_uncheck_do.php" method="post" name="cz" id="cz" onSubmit="if(!confirm('Are You Sure Execute this?')){return false;}">
        ...
        <div class="container-fluid p-0 ">
            ...
            <div class="n-form-btn-class">
				   ...
				   <input name="return"  type="button" class="btn1 btn1-bg-update" value="RETURN TO USER" onclick="return_function()" />

				   <input  name="do_no" type="hidden" id="do_no" value="<?=$do_no?>"/><input type="hidden" name="return_remarks" id="return_remarks">
				   
				   <input  name="entry_time" type="hidden" id="entry_time" value="<?=date('Y-m-d H:i:s')?>"/>
			...
			<?php 
			$tot_credit_bal=$credit_bal+$total_amount;
			$set_credit_limit=$dealer->credit_limit;
			if($dealer->credit_limit_appli=="YES" && $tot_credit_bal>$set_credit_limit){
			
			?>
			<h2 style="text-align:center;font-weight:bold;color:red;">Credit Limit Is over.You Can not approved this Sales Order</h2>
			<?php } 
			else{
			?>
