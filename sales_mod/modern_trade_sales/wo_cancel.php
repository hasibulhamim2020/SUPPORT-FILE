...
$title='Work Order Cancel Request';
...
if(isset($_POST['confirm'])){
 ...
 $sql = 'update sale_do_master set status="CANCEL REQUEST", cancel_req_date="'.$entry_date.'", cancel_note="'.$remarks.'", cancel_req_by="'.$entry_by.'",
  cancel_req_at="'.$entry_at.'" where do_no = '.$do_no;
	db_query($sql);
...
$massage  = "Dear Sir,\r\nRequest for WO Calcel Approval. \r\n";
...
<? 
 ...
 $pr_update = find_a_field('sale_do_production_issue','sum(total_unit)','do_no='.$$unique);
 $ch_update = find_a_field('sale_do_chalan','sum(total_unit)','do_no='.$$unique);

if($pr_update==0 || $ch_update==0){
...
<input name="confirm" type="submit" class="btn1" value="WO CANCEL REQUEST" style="width:270px; font-weight:bold; float:right; font-size:12px; height:30px; color:#090" /></td>
...
<td colspan="2" align="center" bgcolor="#FF3333"><strong>You can't cancel this work order</strong></td>
