...
$title='Work Order Hold Approval';
do_calander('#fdate');
do_calander('#tdate');
...
$target_url = '../wo/wo_hold_approval.php';
...
if(isset($_POST['approved'])){
	...
	$sql = "update sale_do_master set status='HOLD', hold_app_by='".$_POST['checked_by']."', hold_app_at='".$_POST['checked_at']."' where do_no=".$_POST['do_no']."";
	db_query($sql);
  ...
  <form action="" method="post" name="codz" id="codz">
    <table width="80%" border="0" align="center">
      ...
      <tr>
		...
		foreign_relation('sale_do_master','do_no','job_no',$_POST['old_do_no'],'status="HOLD REQUEST"');
...
$con .= ' and m.do_no in ('.$_POST['old_do_no'].') ';
  ...
  $res="select m.do_no as old_do_no, m.do_no, m.job_no, m.dealer_code, m.do_date,   m.buyer_code, m.merchandizer_code, m.status from sale_do_master m, sale_do_details d where m.do_no=d.do_no and m.status='HOLD REQUEST'  ".$con." group by m.do_no order by m.do_date, m.do_no ";
