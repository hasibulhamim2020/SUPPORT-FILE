...

{
		...
		$driver_mobile=$_POST['driver_mobile'];
		 ...
		 $upch_sql = 'update sale_do_chalan set status="'.$status.'", delivery_man="'.$delivery_man.'", delivery_man_mobile="'.$delivery_man_mobile.'", vehicle_no="'.$vehicle_no.'", 
		driver_name="'.$driver_name.'", driver_mobile="'.$driver_mobile.'", checked_by="'.$checked_by.'", checked_at="'.$checked_at.'" where chalan_no = '.$chalan_no;
		 db_query($upch_sql);
		...
		$msg='Successfully Completed Delivery Challan.';
		
		echo '<script>window.location.replace("wo_challan_unapproved.php")</script>';
...
if($chalan_no>0)
$_POST['chalan_no'] = $chalan_no;
else
$chalan_no = $_POST['chalan_no'] = next_transection_no($group_for,$_POST['chalan_date'],'sale_do_chalan','chalan_no');
  ...
  <tr>
	...
	<? $field='group_for'; $table='user_group';$get_field='id';$show_field='group_name';?>
      ...
      <div>
        ...
        <input  name="sales_type2" type="text" id="sales_type2" value="<?=find_a_field('sales_type','sales_type','id='.$sales_type);?>" required="required"/>

		<input  name="sales_type" type="hidden" id="sales_type" value="<?=$sales_type?>" required="required"/>
    ...
    <tr>
	  ...
	  <table  width="100%" border="1" align="left"  cellpadding="2" cellspacing="2">
		<tr>
			...
			<td width="11%"><strong>Pnding</strong></td>
  ...
  <tr>
    ...
    <input  name="warehouse_to" type="hidden" id="warehouse_to" value="<?=$warehouse_to?>"/>
     ...
     <?php /*?> <input  name="item_id" type="text" id="item_id" value="<?=$item_id?>" style="width:500px; height:40px; font-size:18px;" required="required" onblur="getData2('production_receive_ajax.php', 'pr', this.value, document.getElementById('pr_no').value+'=>'+document.getElementById('bag_type').value);"/><?php */?>
	  ...
	  <select id="order_no" name="order_no"   style="width:200px;" onchange="getData2('wo_data_ajax.php', 'wo_data_filter', this.value, document.getElementById('sales_type').value);" >
	  <option></option>
	  
	 <? 
	 
	 
	  $sql = 'select order_no, sum(total_unit) as total_unit  from sale_do_chalan
    ...
    $itm_sql='select s.id,  concat(i.item_name," - [Order ID: ",s.id, "]") as order_item, s.total_unit as order_qty
   ...
   where i.item_id=s.item_id and s.do_no="'.$$unique.'"';
	...
	<table  width="100%" border="1" align="left"  cellpadding="2" cellspacing="2">
		<tr>
		    ...
		    <td>
			<select id="barcode_id" name="barcode_id"   style="width:150px;"  >
	  		<option></option>

        	<? foreign_relation('production_receive_detail','id','barcode',$barcode_id,'do_no="'.$$unique.'" and status="RECEIVED"'); ?>
        	</select>			</td>
		</tr>
	</table>
	</span></td>
    <td align="center" bgcolor="#CCCCCC">
	  ...
	  <table  width="100%" border="1" align="left"  cellpadding="2" cellspacing="2">
		<tr>
		<td width="50%"><input name="total_unit" type="text" class="input3" id="total_unit"   maxlength="100" style="width:80px;height:30px; font-size:18px;"  onKeyUp="count()" onBlur="count()"/></td>
		<td width="50%"><input name="bag_size" type="text" class="input3" id="bag_size"   maxlength="100" style="width:70px;height:30px; font-size:18px;"  onKeyUp="count()" onBlur="count()"/></td>
		</tr>
	</table>
	 </span>	 	</td>
    </tr>
...
<? 
    ...
    $res='select a.*, s.sub_group_name,  b.item_name
...
<td align="center"><?php /*?><input name="delete" type="submit" class="btn1" value="CANCEL WO" style="width:270px; font-weight:bold; font-size:12px;color:#F00; height:30px" /><?php */?>

<input  name="chalan_no" type="hidden" id="chalan_no" value="<?=$chalan_no;?>"/></td>
