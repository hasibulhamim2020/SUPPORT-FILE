...
switch ($_REQUEST['report']) {
    case 1:
	$report="Customer Credit Limit Information";
	break;
	case 1100:
...
if($_POST['dealer_code']>0) $dcon = ' and d.dealer_type="'.find_a_field('dealer_info','dealer_type','dealer_code="'.$_POST['dealer_code'].'"').'"';
if($_POST['dealer_type']>0) $dcon .= ' and d.dealer_type="'.$_POST['dealer_type'].'"';
//if($_POST['f_date']!='' && $_POST['t_date']!='') $dcon .= ' and c.start_date<="'.$_POST['f_date'].'" and c.end_date>="'.$_POST['f_date'].'"';

function credit_cal($dealer_code,$dealer_type,$s_date,$e_date){

$dealer = find_all_field('dealer_info','','dealer_code="'.$dealer_code.'"');

$sql = 'select c.* from credit_limit_config c where c.effective_start_date>="'.$s_date.'" and c.effective_end_date<="'.$e_date.'" and if(c.dealer_code>0,c.dealer_code="'.$dealer_code.'",c.dealer_type="'.$dealer_type.'") order by c.id desc limit 1';
$qry = db_query($sql);
...
$date2 = $config->effective_end_date;  
}else{
$date2 = $config->effective_start_date;  
}
...
if($dealerAgeCalanderMonth<1){
$dealerAgeCalanderMonth = 1;
}
//echo $dealerAgeCalanderMonth;
...
$thisMonth = date('m',strtotime($current_date));
$thisYear = date('Y',strtotime($current_date));
if($thisMonth==1 || $thisMonth==01){
	$preMonth = 12;
	$preYear = $thisYear-1;
	}else{
...
//$previous_month_date = date('Y-m-d',strtotime($current_date.'-30 days'));		
$transaction_start_date = $preYear.'-'.$preMonth.'-01'; //date($preYear.'-'.$preMonth.'-01',strtotime($previous_month_date));
$mdays = date('t',strtotime($transaction_start_date));
$transaction_end_date = date('Y-m-'.$mdays,strtotime($transaction_start_date));
...
if($config->conditionalEffectiveMon=='Above 12 Month'){
$requiredDealerAgeCalander = $dealerAgeCalanderMonth;
}
...
/*if($requiredDealerAgeCalander>12){
 $requiredDealerAgeCalander = $dealerAgeCalanderMonth;
 echo 'bimol';
 }*/
...
echo $payment_sql = 'select sum(cr_amt) as payment from journal where tr_from="Receipt" and ledger_id="'.$dealer_ledger.'" and jv_date between "'.$transaction_start_date.'" and "'.  $transaction_end_date.'"';
...
$pay_data = mysqli_fetch_object($qry);
 ...
 //Minimum Limit Check
 if($total_amt>$config->minimumLimit){
...
$payment_sql = 'select sum(dr_amt) as salesAmount from journal where tr_from="Sales" and ledger_id="'.$dealer_ledger.'" and jv_date between "'.$transaction_start_date.'" and "'. $transaction_end_date.'"';
 $qry = db_query($payment_sql);
 $pay_data = mysqli_fetch_object($qry);
...
return $creditAmt.'|'.$config->effective_start_date.'|'.$config->effective_end_date.'|'.$dealerAgeDays.'|'.$dealerAgeCalanderMonth;
}
...
<th>Dealer Name</th>
<th>GL</th>
<th>Dealer Created At</th>
<th>Effective Start Date</th>
<th>Effective End Date</th>
<th>Age(Days)</th>
<th>Age(Calander Month)</th>
<th>Credit Amount</th>
...
$end_date = '2025-12-31';
$sql = 'select d.dealer_name_e,d.account_code,d.entry_at, d.dealer_code,d.dealer_type, dt.dealer_type as dealer_type_name from dealer_info d, dealer_type dt where d.dealer_type=dt.id '.$dcon.'';
$qqry = db_query($sql);
while($data=mysqli_fetch_object($qqry)){

$credit_info = credit_cal($data->dealer_code,$data->dealer_type,$start_date,$end_date);
$credit = explode("|",$credit_info);
$s++;

?>
<tr>
<td><?=$s?></td>
<td><?=$data->dealer_type_name?></td>
<td><?=$data->dealer_name_e.'-'.$data->dealer_code?></td>
...
<td align="right"><?=number_format($credit[0],2)?></td>
...
$tp_t = $tp_t+$info[1];
?>
<tr><td><?=$s?></td><td><?=$data->do_no?></td><td><?=$data->do_date?></td><td><?=$data->entry_at?></td><td><?=$data->entry_time?></td><td><?=$data->dealer_name?></td><td><?=$data->area?></td><td><?=$data->depot?></td><td><?=$data->grp?></td><td style="text-align:right"><?=$data->rcv_amt?></td><td><?=$data->Payment_Details?></td><td><?=$info[0]?></td><td><?=$info[1]?></td></tr>
<?
}
...
where dl.do_no=m.do_no and m.status in ('CHECKED','COMPLETED')  and m.dealer_code=d.dealer_code and w.warehouse_id=d.depot".$depot_con.$date_con.$pg_con.$dealer_con.$dtype_con." group by d.dealer_code order by d.dealer_name_e";
$query = db_query($sql);
echo '<table width="100%" cellspacing="0" cellpadding="2" border="0" id="ExportTable">
