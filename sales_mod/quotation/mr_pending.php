...
$con .= 'and a.req_date between '.$_POST['fdate'].' and '.$_POST['tdate'];
 ...
 $res='select  	a.req_no,a.req_no, a.req_date, b.group_name as Concern_name,  a.need_by, c.fname as entry_by, a.entry_at,a.status from requisition_master a, user_group b, user_activity_management c where  a.group_for=b.id and a.entry_by=c.user_id '.$con.' and a.status in ("PENDING") order by a.req_no';
