...
// $res='select  	a.do_no,concat("Q-000",a.do_no) as Quotation_no, DATE_FORMAT(a.do_date, "%d-%m-%Y") as Qutation_date, a.status,concat(d.dealer_code,"-",d.dealer_name_e) as dealer_name,  a.entry_at,u.fname from sale_requisition_master a,dealer_info d, user_activity_management u where  a.dealer_code=d.dealer_code and u.user_id=a.entry_by and a.status in ("MANUAL") group by a.do_no order by a.do_no desc';
                   ...
                   <td><?=$data->Quotation_no;?></td>
                    ...
                    <td>
						...
						<form action="" method="post" name="cz" id="cz" onSubmit="if(!confirm('Are You Sure Execute this?')){return false;}">
								 ...
								 <input class="btn1 btn1-bg-submit" name="send" type="submit" value="Complete Quo" />
								
								<? if(($_SESSION['user']['id']==1) || ($_SESSION['user']['id']==10087) ) { ?>
								  <input name="delete" type="submit" value="Delete Quo."  />
							<? } ?>
					</form>
					</td>
