...
case 217:
$report='Target Vs Achivement Report (Dealer Wise)';
//report_log('Sales', 'master_report_target', 1, $report);
...
if(isset($dealer_code)) { $dealer_con=' and t.dealer_code="'.$dealer_code.'"';}
  ...
  $sql="select t.target_year,t.target_month,a.AREA_CODE,a.AREA_NAME,t.dealer_code,t.dealer_name,sum(t.target_con) as Target_Ratio 
    from  ss_target_ratio t,dealer_info d, area a
...
//report_log('Sales', 'master_report_target', 4, $report);
  ...
  $sql="select t.target_year,t.target_month,d.dealer_code,d.dealer_name_e,t.emp_code as so_code,t.emp_name as so_name,t.target_con  
    from ss_target_ratio t,dealer_info d
    where 1 and t.dealer_code=d.dealer_code
    ".$date_con."
...
/////////zone ////
$sql='select * from zon where 1 group by ZONE_CODE';
$query=db_query($sql);
while($row=mysqli_fetch_object($query)){
	$zone_name_get[$row->ZONE_CODE]=$row->ZONE_NAME;
}

/////////branch ////
$sql='select * from branch where 1 group by BRANCH_ID';
$query=db_query($sql);
while($row=mysqli_fetch_object($query)){
	$branch_name_get[$row->BRANCH_ID]=$row->BRANCH_NAME;
}
/////////Area ////
$sql='select * from area where 1 group by AREA_CODE';
$query=db_query($sql);
while($row=mysqli_fetch_object($query)){
	$area_name_get[$row->AREA_CODE]=$row->AREA_NAME;
}
...
<tr>
 ...
 <th>Dealer Code</th><th>Dealer Name</th>
...
<tr>
    ...
    <td><?=$data->target_year;?></td>
    <td><?=$data->target_month;?></td>
    <td><?=$branch_name_get[$data->region_id];?> </td>
    <td><?=$zone_name_get[$data->zone_id];?></td> 
    <td><?=$area_name_get[$data->area_id];?> </td>
...
$end_date=$_POST['year']."-".$_POST['month']."-31";

//////////sale qty get////////
  $sql2='select sum(total_amt) as sale_amt,dealer_code,entry_by from ss_do_details where do_date between "'.$start_date.'"  and "'.$end_date.'" group by entry_by';
$query2=db_query($sql2);
while($drow=mysqli_fetch_object($query2)){
	$total_sales_get[$drow->entry_by]=$drow->sale_amt;
}

 

    $sql='select s.target_year,s.target_month,s.dealer_code,s.target_amount,s.dealer_code,d.dealer_code,d.dealer_name_e,s.so_code,ss.username,ss.fname from ss_target_upload s,dealer_info d,ss_user ss where s.target_month="'.$_POST['month'].'" and s.so_code=ss.username and s.target_year="'.$_POST['year'].'" and s.dealer_code=d.dealer_code group by s.so_code';
$query=db_query($sql);
while($row=mysqli_fetch_object($query)){
?>
<tr>
	...
	<td><?php echo $row->dealer_name_e;?></td>
	<td><?php echo $row->fname;?></td>
	<td><?php echo $row->target_amount;?></td>
		<td><?php echo $total_sales_get[$row->so_code];?></td>
			<td><?php echo number_format($achivement=(100*$total_sales_get[$row->so_code])/$row->target_amount,2)." %";?></td>
...
//////////sale qty get////////
  $sql2='select sum(total_unit) as sale_qty,dealer_code from sale_do_details where do_date between "'.$start_date.'"  and "'.$end_date.'" group by dealer_code';
$query2=db_query($sql2);
while($drow=mysqli_fetch_object($query2)){
	$total_sales_get[$drow->dealer_code]=$drow->sale_qty;
}

 

  $sql='select s.target_year,s.target_month,s.dealer_code,sum(s.target_ctn) as tot_target_qty,s.dealer_code,d.dealer_code,d.dealer_name_e from sale_target_upload s,dealer_info d where s.target_month="'.$_POST['month'].'" and s.target_year="'.$_POST['year'].'" and s.dealer_code=d.dealer_code group by s.dealer_code';
$query=db_query($sql);
while($row=mysqli_fetch_object($query)){
?>
<tr>
	...
	<td><?php echo $row->dealer_code;?></td>
	<td><?php echo $row->dealer_name_e;?></td>
	<td><?php echo $row->tot_target_qty;?></td>
		<td><?php echo $total_sales_get[$row->dealer_code];?></td>
			<td><?php echo number_format($achivement=(100*$total_sales_get[$row->dealer_code])/$row->tot_target_qty,2)." %";?></td>
