...
switch ($_REQUEST['report']) {
	...
	break;
		...
		case 136:
		$report="Customer Region Report";	
	break;
	
	
	case 135135:
		$report="Territory Details Report";	
	break;
...
break;	
	...
	case 191199:
$report="Customer Collection Statement";
break;

case 301:
$report="DO Pending Summary Report";
break;
...
$report="Delivery Details Report with Chalan Amount";
	break;
case 191:
$report="Delivery Summary  Report with Amount";
break;
...
<th style="text-align:center">Rcv Amt</th>
...
<th style="text-align:center">AIT Amount</th>
...
<?php /*?><td style="text-align:center"><a href="../wo/sales_order_print_view.php?v_no='<?=rawurlencode(url_encode($data->do_no));?>'" target="_blank"><?=$data->do_no?></a></td><?php */?>
...
<td><?=$data->depot?></td>
<td style="text-align:right"><?=number_format($data->acc_amt,2)?></td>
<td><?=$data->Payment_Details?></td>
<td style="text-align:right"><?=number_format($sub=$data->total_amt,2); $tot_sub+=$sub;?></td>
<td style="text-align:right"><?=number_format($disc=$data->total_amt*($data->discount/100),2); $tot_disc+=$disc;?></td>
<td style="text-align:right"><?=number_format($vat=$data->total_amt*($data->vat/100),2); $tot_vat+=$vat;?></td>
<td style="text-align:right"><?=number_format($ait=$data->total_amt*($data->ait/100),2); $tot_ait+=$ait;?></td>
<td style="text-align:right"><?=number_format($total_invoice_amt=($sub+$vat+$ait)-$disc,2); $tot_total+=$total_invoice_amt; ?></td>
</tr>
<? } ?>
<tr >
...
<td>&nbsp;</td>
<td style="text-align:right"><?=number_format($tot_sub,2)?></td>
<td style="text-align:right"><?=number_format($tot_disc,2)?></td>
<td style="text-align:right"><?=number_format($tot_vat,2)?></td>
<td style="text-align:right"><?=number_format($tot_ait,2)?></td>
<td style="text-align:right"><?=number_format($tot_total,2)?></td>
</tr>
</tbody>
</table>
<? }
...
if($depot_id>0) {$dpt_con=' and d.depot="'.$depot_id.'"';} 
 ...
 $sql = "select m.do_no,m.do_date,m.dealer_code,m.depot_id,m.entry_by,m.status as do_status, i.item_id,i.unit_name,i.pack_size, o.unit_price, o.total_unit,d.product_group as grp,concat(i.finish_goods_code,'- ',item_name) as item_name, o.id, w.warehouse_id, wd.warehouse_id, wd.user_id,wd.status from 
sale_do_master m,sale_do_details o, item_info i,dealer_info d, warehouse w, warehouse_define wd
where m.do_no=o.do_no and i.item_id=o.item_id and m.group_for='".$_SESSION['user']['group']."' and m.depot_id=w.warehouse_id and w.warehouse_id=wd.warehouse_id and m.entry_by=wd.user_id and wd.status='Active' and m.dealer_code=d.dealer_code and m.status in ('CHECKED','COMPLETED','UNCHECKED','PROCESSING') ".$date_con.$item_con.$dpt_con.$dtype_con.$dealer_con.$item_brand_con;
...
<div class="date" style="text-align:right">Reporting Time: <?=date("h:i A d-m-Y")?></div>
	...
	<th style="text-align:center">Unit</th>
      ...
      <th style="text-align:center">DO Rate </th>
      ...
      <th style="text-align:center">Undelivery Qty</th>
...
$ch_qty = find_a_field('sale_do_chalan','sum(total_unit)','do_no="'.$info->do_no.'" and item_id="'.$info->item_id.'" and order_no = "'.$info->id.'" ');

$due_qty = $info->total_unit - $ch_qty;
?>

    <tr>
      <td style="text-align:center"><?=++$i;?></td>
     
	<?php /*?>  <td style="text-align:center"><a href="../wo/sales_order_print_view.php?v_no='<?=url_encode($info->do_no);?>'" target="_blank"><?=$info->do_no?></td><?php */?>
	  
	  <td style="text-align:center"><a href="../wo/sales_order_print_view.php?c='<?=url_encode($c_id);?>&v=<?=url_encode($info->do_no);?>'" target="_blank"><?=$info->do_no?></td>
	  
	  <td><?=find_a_field('warehouse','warehouse_name','warehouse_id='.$info->depot_id);?></td>
	  <td style="text-align:center"><?=$info->do_date;?></td>
	  <td><?=$info->do_status;?></td>
	  <td><?=find_a_field('dealer_info','dealer_name_e','dealer_code="'.$info->dealer_code.'"');?></td>
	  <td style="text-align:center"><?=$info->item_id?></td>
      <td><?=$info->item_name;?></td>
	  <td style="text-align:center"><?=$info->unit_name;?></td>
      <!--<td style="text-align:center"><?=$info->pack_size?></td>-->
      <td style="text-align:right"><?=number_format($info->unit_price,2);?></td>
      <td style="text-align:right"><?=number_format($info->total_unit,2); $tot_total_unit+=$info->total_unit;?></td>
      <td style="text-align:right"><?=number_format($ch_qty,2); $total_ch_qty+=$ch_qty;?></td>
      <td style="text-align:right"><?=number_format($due_qty,2); $total_due_qty+=$due_qty;?></td>
...
<? } ?>
    ...
    <tr>
      ...
      <td style="text-align:right"><?=number_format($tot_total_unit,2);?></td>
      <td style="text-align:right"><?=number_format($total_ch_qty,2);?></td>
      <td style="text-align:right"><?=number_format($total_due_qty,2);?></td>
...
$do_ch_amt[$info->do_no] = $info->ch_amt;
 ...
 $sql="select m.do_no,m.do_date,concat(d.dealer_code,'- ',d.dealer_name_e) as dealer_name, m.depot_id, m.entry_by, m.status as do_status,w.warehouse_id, wd.warehouse_id, wd.user_id,wd.status, w.warehouse_name as depot, sum(ds.total_amt) as do_amt,m.rcv_amt,concat(m.payment_by,' ',m.bank,' ',m.remarks) as Payment_Details,d.area_code 
from sale_do_master m,dealer_info d  , warehouse w, sale_do_details ds, warehouse_define wd
where m.do_no=ds.do_no and m.status in ('CHECKED','COMPLETED')  and m.dealer_code=d.dealer_code and w.warehouse_id=m.depot_id and w.warehouse_id=wd.warehouse_id and m.entry_by=wd.user_id and wd.status='Active' and ds.total_amt>0 ".$depot_con.$date_con.$pg_con.$dealer_con.$dtype_con." group by m.do_no order by m.do_date,m.do_no";
$query = db_query($sql);
?>
<style>
...
<tr>
<td style="text-align:center"><?=$s?></td>
<?php /*?><td style="text-align:center"><a href="../wo/sales_order_print_view.php?v_no='<?=url_encode($data->do_no);?>'" target="_blank"><?=$data->do_no?></td><?php */?>

<td style="text-align:center"><a href="../wo/sales_order_print_view.php?c='<?=url_encode($c_id);?>&v=<?=url_encode($data->do_no);?>" target="_blank"><?=$data->do_no?></td>
...
<td style="text-align:right"><?=number_format($data->rcv_amt,2)?></td>
<td><?=$data->Payment_Details?></td>
<td style="text-align:right"><?=number_format($data->do_amt,2);?></td>
<td style="text-align:right"><?=number_format($do_ch_amt[$data->do_no],2)?></td>
<td style="text-align:right"><?=number_format($due_amt,2);?></td></tr>
<?
}
echo '<tr>
<td>&nbsp;</td>
...
<td style="text-align:right">'.number_format($due_t,2).'</td>
</tr>
</tbody>
</table>';
}
...
{
 $sql="select m.do_no,m.do_date, m.depot_id, m.entry_by, concat(d.dealer_code,'- ',d.dealer_name_e) as dealer_name,(select AREA_NAME from area where AREA_CODE=d.area_code) as area,w.warehouse_id, w.warehouse_name as depot,d.dealer_code,sum(d1.total_amt) as dp_t, wd.warehouse_id, wd.user_id

from sale_do_master m,dealer_info d  , warehouse w, sale_do_details d1, warehouse_define wd

where d1.do_no=m.do_no and m.status in ('CHECKED','COMPLETED')  and m.dealer_code=d.dealer_code and w.warehouse_id=m.depot_id and w.warehouse_id=wd.warehouse_id and m.entry_by=wd.user_id and wd.status='Active' ".$depot_con.$date_con.$pg_con.$dealer_con.$dtype_con." group by d.dealer_code order by d.dealer_name_e";
...
<th style="text-align:center">Total DO Amount</th>
<th style="text-align:center">Total Chalan Amount</th>
<th style="text-align:center">Due Amount</th>
</tr>
</thead>
<tbody>';
while($data=mysqli_fetch_object($query)){$s++;
$tp_t= find_a_field('sale_do_chalan','sum(total_amt)','dealer_code='.$data->dealer_code);
$tp_total +=$tp_t;
...
<td><?=$data->area?></td>
<td><?=$data->depot?></td>
<td style="text-align:right"><?=number_format($data->dp_t,2)?></td>
<td style="text-align:right"><?=number_format($tp_t,2)?></td>
...
<td><strong>Total</strong></td>
<td style="text-align:right">'.number_format($dp_t,2).'</td>
<td style="text-align:right">'.number_format($tp_total,2).'</td>
<td style="text-align:right">'.number_format($due_tot,2).'</td>
</tr>
</tbody>
</table>';
}
...
elseif($_REQUEST['report']==191199) 
{


/////////////////dealer info///////////////

$dql="select  * from dealer_info where 1 group by account_code ";
$dquery = db_query($dql);
while($ddata=mysqli_fetch_object($dquery)){
$dealer_name_get[$ddata->account_code]=$ddata->dealer_name_e;
$dealer_address_get[$ddata->account_code]=$ddata->address_e;
$dealer_ledger_id_get[$ddata->account_code]=$ddata->address_e;
}


if($_POST['dealer_acc_code']>0){

$deal_con='and a2.ledger_id="'.$_POST['dealer_acc_code'].'"';
$deal_con2='and a.ledger_id="'.$_POST['dealer_acc_code'].'"';
}
?>
<table width="100%" cellspacing="0" cellpadding="2" border="0" id="ExportTable">
<thead>
<tr><td style="border:0px;" colspan="9"><?php echo $str;?></td></tr>
	<tr>
		<th>S/L</th>
...
//echo $sql="select j.*,a.ledger_id,a.ledger_group_id,a.ledger_name from journal j,accounts_ledger a where j.ledger_id=a.ledger_id and a.ledger_group_id=126002 and  j.jv_date between '".$_POST['f_date']."' and '".$_POST['t_date']."'  group by j.ledger_id ";
...
FROM 
    journal j
JOIN 
    accounts_ledger a ON j.ledger_id = a.ledger_id
JOIN 
    journal j2 ON j.jv_no = j2.jv_no
JOIN 
    accounts_ledger a2 ON j2.ledger_id = a2.ledger_id
WHERE 
    a.ledger_group_id = 127002
    AND j.jv_date BETWEEN '".$_POST['f_date']."' and '".$_POST['t_date']."'
    AND a2.ledger_group_id = 124001
	".$deal_con."
GROUP BY 
    j.ledger_id";
...
?>
<tr>
	<td bgcolor="#00FF66"><?php echo ++$i;?></td>
	<td bgcolor="#00FF66"> </td>
	<td bgcolor="#00FF66"> </td>
	<td bgcolor="#00FF66"> </td>
	<td bgcolor="#00FF66"></td>
	<td bgcolor="#00FF66"><?php echo $data->ledger_name;?></td>
	<td bgcolor="#00FF66"></td>
</tr>
<?php 
    $bsql="select j.*,a.ledger_id,a.ledger_group_id,a.ledger_name from journal j,accounts_ledger a where j.ledger_id=a.ledger_id and a.ledger_group_id=124001 and  j.jv_date between '".$_POST['f_date']."' and '".$_POST['t_date']."' and j.jv_no in(select jv_no from journal where ledger_id='".$data->ledger_id."' and jv_date between '".$_POST['f_date']."' and '".$_POST['t_date']."' ) ".$deal_con2." group by j.ledger_id ";
$bquery = db_query($bsql);

while($bdata=mysqli_fetch_object($bquery)){
?>
<tr>
	<td><?php echo ++$s;?></td>
	<td> <?php echo $bdata->jv_no;?></td>
	<td> <?php echo $bdata->ledger_name;?></td>
	<td> <?php echo $dealer_address_get[$bdata->account_code];?></td>
	<td> <?php echo $bdata->cr_amt;?></td>
	<td></td>
	<td></td>
</tr>
<?

$tot_sub_amt+=$bdata->cr_amt;
}
?>
<tr class="footer">
<td colspan="4">Sub Total</td>
<td><?php echo $tot_sub_amt;?></td>
</tr>

<?php 
$tot_gr_amt+=$tot_sub_amt;
}
 
 ?>
<tr class="footer">
<td colspan="4">Total</td>
<td><?php echo $tot_gr_amt;?></td>
</tr>
</tbody>
</table>
 <?php 
...
$query =db_query($sql);
?>
<table width="100%" cellspacing="0" cellpadding="2" border="0" id="ExportTable"><thead><tr><td style="border:0px;" colspan="9"><div class="header">
...
$query =db_query($sql);
?>
<table width="100%" cellspacing="0" cellpadding="2" border="0" id="ExportTable"><thead><tr><td style="border:0px;" colspan="9"><div class="header"><h1>Natura Agro Processing Ltd</h1>
<h2><?=$report?></h2>
<h3>Date Interval : <?=$fr_date?> to <?=$to_date?> </h3>
</div>
<div class="left"></div><div class="right"></div>
...
$report="Customer Details Report";	
		...
		$acc_con .= ' and d.region_code = "'.$_POST['branch'].'" ';

		

		if($_POST['zon']!='')

		$acc_con .= ' and d.zone_code = "'.$_POST['zon'].'" ';

		

		if($_POST['area']!='')

		$acc_con .= ' and d.area_code = "'.$_POST['area'].'" ';
		
		
		if($_POST['dealer_code']!='')

		$acc_con .= ' and d.dealer_code = "'.$_POST['dealer_code'].'" ';



$sqll="SELECT d.dealer_code,d.dealer_name_e,t.dealer_type,d.account_code,d.sub_ledger_id,d.contact_no,d.email,d.area_code,d.zone_code,d.region_code,d.propritor_name_e,d.address_e,d.credit_limit,d.contact_person_name,d.contact_person_designation,d.contact_person_mobile FROM dealer_info d LEFT JOIN dealer_type t ON t.id=d.dealer_type WHERE 1 ".$acc_con."";
...
$report="Territory Details Report";	
		...
		$acc_con .= ' and a.BRANCH_ID = "'.$_POST['branch'].'" ';

		

		if($_POST['zon']!='')

		$acc_con .= ' and b.ZONE_CODE = "'.$_POST['zon'].'" ';

		

		if($_POST['area']!='')

		$acc_con .= ' and c.AREA_CODE = "'.$_POST['area'].'" ';
 ...
 $sqll="select a.*,b.*,c.* from branch a,zon b,area c  where c.ZONE_ID=b.ZONE_CODE and b.REGION_ID=a.BRANCH_ID ".$acc_con." ";
...
?>
	<tr>
		...
		<td style="text-align:center"><?=$data->BRANCH_ID;?></td>
		<td><?=$data->BRANCH_NAME;?></td>
		<td style="text-align:center"><?=$data->ZONE_CODE;?></td>
		<td><?=$data->ZONE_NAME;?></td>
		<td style="text-align:center"><?=$data->AREA_CODE;?></td>
...
$sqlmon0 = mysqli_fetch_row(db_query("select sum(c.pkt_unit) from sale_do_chalan c, dealer_info d where d.dealer_code=c.dealer_code and c.chalan_date between '".$f_mons0."' and '".$t_date."' and c.item_id=".$item->item_id.$pg_con));

$sqlmon1 = mysqli_fetch_row(db_query("select sum(c.pkt_unit) from sale_do_chalan c, dealer_info d where d.dealer_code=c.dealer_code and c.chalan_date between '".$f_mons1."' and '".$f_mone1."' and c.item_id=".$item->item_id.$pg_con));

$sqlmon2 = mysqli_fetch_row(db_query("select sum(c.pkt_unit) from sale_do_chalan c, dealer_info d where d.dealer_code=c.dealer_code and c.chalan_date between '".$f_mons2."' and '".$f_mone2."' and c.item_id=".$item->item_id.$pg_con));

$sqlmon3 = mysqli_fetch_row(db_query("select sum(c.pkt_unit) from sale_do_chalan c, dealer_info d where d.dealer_code=c.dealer_code and c.chalan_date between '".$f_mons3."' and '".$f_mone3."' and c.item_id=".$item->item_id.$pg_con));
...
<tr><th rowspan="2">S/L</th><th rowspan="2">Fg</th><th rowspan="2">Item Name</th><th rowspan="2">Unit</th><th rowspan="2">Brand</th><th rowspan="2">Pack Size</th><th rowspan="2">GRP</th>
	  ...
	  $q1=db_query($sql_zone);
	  while($zdata=mysqli_fetch_object($q1)){
	  ?>
...
// and c.total_unit>0 
$query2 = db_query($sql2);
while($data2 = mysqli_fetch_object($query2)){$group_a[$data2->zone_code] = $data2->group_amount;}
...
$query2 = db_query($sql2);
while($data2 = mysqli_fetch_object($query2)){$group_b[$data2->zone_code] = $data2->group_amount;}
...
$query2 = db_query($sql2);
while($data2 = mysqli_fetch_object($query2)){$group_c[$data2->zone_code] = $data2->group_amount;}
...
$query2 = db_query($sql2);
while($data2 = mysqli_fetch_object($query2)){$group_d[$data2->zone_code] = $data2->group_amount;}
...
$query2 = db_query($sql2);
while($data2 = mysqli_fetch_object($query2)){$group_e[$data2->zone_code] = $data2->group_amount;}
...
?>
<table width="100%" cellspacing="0" cellpadding="2" border="0" id="ExportTable"><thead><tr><td style="border:0px;" colspan="8"><?=$str?><div class="left"></div><div class="right"></div><div class="date">Reporting Time: <?=date("h:i A d-m-Y")?>222</div></td></tr>
