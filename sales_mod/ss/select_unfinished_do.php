...
<table width="70%" border="0" align="center">
  ...
  <tr>
	...
	<?
	$query ="select a.do_no,a.dealer_code,d.PBI_NAME from sale_other_master a,personnel_basic_info d where d.PBI_ID=a.dealer_code and  a.status='MANUAL' and do_date='".date('Y-m-d')."' and a.entry_by=".$_SESSION['user']['id'];
	?>
	 ...
	 <option></option>
	  ...
	  {
	  ?>
	   <option value="<?=$data->do_no?>"><?=$data->do_no.'-'.$data->PBI_NAME.'('.$data->dealer_name_e.')';?></option>
	  <?
	  }
	  ?></select>
    </strong></td>
