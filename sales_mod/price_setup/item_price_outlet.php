...
?>
        <option <? if($datas->brand_name==$brand_name) echo 'Selected ';?> value="<?=$datas->brand_name?>">
          <?=$datas->brand_name?>
          </option>
        <? } ?>
    </select></td>
...
where 
sub_group_id=1096000100010000 
and finish_goods_code not between 5000 and 5999
and finish_goods_code not between 9000 and 9999
'.$brand_name_con.'
order by finish_goods_code';

  $query = db_query($sql);
  while($data=mysqli_fetch_object($query)){$i++;
 $sql_price = 'select d.discount,d.set_price from sales_corporate_price d, dealer_info i where i.dealer_code=d.dealer_code and dealer_outlet_name ="'.$_POST['dealer'].'" and item_id= "'.$data->item_id.'"';
  ...
  <tr bgcolor="<?=($i%2)?'#E8F3FF':'#fff';?>">
    ...
    value="<?=$set_distount;?>" style="width:30px;" onchange="set_price(<?=$data->item_id?>)" /></td>
    <td><input name="set_<?=$data->item_id?>" id="set_<?=$data->item_id?>" type="text" size="10" maxlength="10" 
    value="<?=$set_price;?>" style="width:50px;" onblur="auto_blank(<?=$data->item_id?>)" />      </td>
