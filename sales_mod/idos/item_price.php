...
auto_complete_from_db('dealer_info d,dealer_type t','""','concat(d.dealer_name_e,"#",d.dealer_code)','d.dealer_type=t.id','dealer'); //changed by alamin on 11-20-2021
?>
...
<div class="form-container_large">
  ...
  ?>
    ...
    <td style="text-align:left"><?=$data->item_brand?></td>

    <td style="text-align:left"><?=$data->sales_item_type?></td>
	...
	<input name="dealer_code" id="dealer_code" type="hidden" value="<?=$dealer_code?>" />

	<input name="dis_<?=$data->item_id?>" id="dis_<?=$data->item_id?>" type="text" size="5" maxlength="5" value="<? echo $set_price = find_a_field('sales_price_dealer','discount','dealer_code="'.$_POST['dealer'].'" and item_id= "'.$data->item_id.'"');?>"  onchange="set_price(<?=$data->item_id?>)" /></td>

    <td><input name="set_<?=$data->item_id?>" id="set_<?=$data->item_id?>" type="text" size="10" maxlength="10" value="<? echo $set_price = find_a_field('sales_price_dealer','set_price','dealer_code="'.$dealer_code.'" and item_id= "'.$data->item_id.'"');?>"  onblur="auto_blank(<?=$data->item_id?>)" />      </td>
