...

{
		...
		if($_POST['item_name'])
		{
		$item_name_con=' and i.item_id="'.$_POST['item_name'].'"'; 
		
		}
		if($_POST['item_sub_group'])
		{
		$item_sub_group_con=' and s.sub_group_id="'.$_POST['item_sub_group'].'"'; 
		
		}
		

  $sql="select g.group_name, s.sub_group_name, i.item_id, i.item_name,  i.finish_goods_code, i.unit_name,  i.pack_size, 
    i.m_price, i.d_price, i.t_price, i.vat_price, i.status
...
<tr>
  ...
  <th bgcolor="#CCFFFF">Vat Price </th>
  </tr>
</thead>
...
<tr><td><?=$s?></td>
  ...
  <td bgcolor="#CCFFFF"><input name="vat_price#<?=$datas[2]?>" type="text" id="vat_price#<?=$datas[2]?>" value="<?=$datas[10]?>"  style="width:70px;" /></td>
  <td>
...
document.getElementById('vat_price#<?=$datas[2]?>').value+'#'+
