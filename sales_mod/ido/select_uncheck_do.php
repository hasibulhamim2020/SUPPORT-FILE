...
$target_url = '../report/work_order_bill_corporate.php';
...
<tr><th>Do No</th><th>Do Date</th><th>Dealer Name</th><th>Area</th><th>Depot</th><th>Payment Details</th><th>SEND AMT</th><th>RCV AMT</th></tr>
...
$res="select m.do_no,m.do_date,concat(d.dealer_code,'- ',d.dealer_name_e,'(',product_group,')') as dealer_name,(select AREA_NAME from area where AREA_CODE=d.area_code) as area,w.warehouse_name as depot,concat(m.payment_by,':',m.bank,':',m.remarks) as Payment_Details,m.rcv_amt as SEND_AMT,m.received_amt as RCV_AMT from 
sale_do_master m,dealer_info d  , warehouse w
where m.status in ('PROCESSING')  and m.dealer_code=d.dealer_code and w.warehouse_id=d.depot".$con." and d.dealer_type!='Distributor' order by m.do_date,d.dealer_name_e";
$query = db_query($res);
