...
$find = find_a_field('sales_corporate_price','set_price','dealer_code = "'.$dealer->dealer_code.'" and `item_id` = "'.$item_id.'"');
	...
	{
	$sql = 'UPDATE `sales_corporate_price` SET  `set_price` = "'.$set_price.'",`discount` = "'.$discount.'", `edit_by` = "'.$_SESSION['user']['id'].'", `edit_at` = "'.date('Y-m-d H:s:i').'" WHERE `dealer_code` = "'.$dealer->dealer_code.'" and `item_id` = "'.$item_id.'"';
	}
	db_query($sql);
}
echo 'Success!';
