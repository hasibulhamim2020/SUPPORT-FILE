...
$table_chalan='sale_other_details';
$unique_chalan='id';
  ...
  <form action="" method="post" name="codz" id="codz">
    <table width="90%" border="0" align="center">
      ...
      <tr>
        ...
        <td colspan="3" bgcolor="#FF9966">&nbsp;</td>
...
if($_POST['fdate']!=''&&$_POST['tdate']!='') $con .= ' and m.do_date between "'.$_POST['fdate'].'" and "'.$_POST['tdate'].'"';
    ...
    $res="select m.do_no, m.do_no, m.do_date,  m.sales_type, m.depot_id,  d.warehouse_name, m.entry_by, m.status from 
sale_other_master m, sale_other_details c,  warehouse d, user_group u 
where 
  m.group_for=u.id  and m.depot_id=d.warehouse_id and m.do_no=c.do_no ".$group_for_con.$con.$dealer_con."  group by c.do_no order by  m.do_date, m.do_no ";
$query = db_query($res);
...
<td onClick="custom(<?=$data->do_no;?>);" <?=(++$z%2)?'':'class="alt"';?>><?= find_a_field('issue_type','issue_type','id='.$data->sales_type);?></td>
