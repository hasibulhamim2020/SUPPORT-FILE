...
if(isset($_POST['approved'])){
	...
	$sql = "update sale_do_chalan set status='CHECKED' where chalan_no=".$_POST['chalan_no']."";
	db_query($sql);
	
	
	$jv_found = find_a_field('secondary_journal','count(id)','tr_no="'.$_POST['chalan_no'].'" and tr_from="Sales"');
	
	if ($jv_found>0) {
	
		$del1 = "delete from secondary_journal where tr_from = 'Sales' and tr_no = '".$_POST['chalan_no']."'";
		db_query($del1);
		
		$del2 = "delete from journal where tr_from = 'Sales' and tr_no = '".$_POST['chalan_no']."'";
		db_query($del2);

	
		auto_insert_sales_chalan_secoundary($_POST['chalan_no']);
...
$con .= ' and m.do_no in ('.$_POST['old_do_no'].') ';
		 ...
		 $sql = "select tr_no, count(tr_no) as ch_no  from secondary_journal where tr_from='Sales' group by tr_no ";
		 $query = db_query($sql);
		 while($info=mysqli_fetch_object($query)){
  		 $ch_no[$info->tr_no]=$info->ch_no;
  ...
  $res="select m.do_no as old_do_no, m.do_no,  d.chalan_no, d.chalan_date,  m.job_no, m.dealer_code, m.do_date,   m.buyer_code, m.merchandizer_code, m.status from sale_do_master m, sale_do_chalan d where m.do_no=d.do_no and d.status='CHECKED'  ".$con." group by d.chalan_no order by  m.do_no ";
