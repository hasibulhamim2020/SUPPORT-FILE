...
<td>
	...
	<?
	$sql = 'select a.*,u.fname from approver_notes a, user_activity_management u where a.entry_by=u.user_id and master_id="'.$$unique_master.'" and type in ("SalesReturn")';
...
<input name="item_id" type="text" class="input3"  value="" id="item_id" style="width:90%; height:30px;" onblur="getData2('return_ajax.php', 'so_data_found', this.value, document.getElementById('do_no').value);"/>
...
<input name="delete"  type="submit" class="btn1" value="DELETE SR" style="width:60%; font-weight:bold; background:#0073AA; font-size:12px;color:#F00; height:30px" />
...
<input name="confirm" type="submit" class="btn1" value="CONFIRM SR" style="width:60%; background:#0073AA; font-weight:bold; font-size:12px; height:30px; color: #FFFFFF; float:right" />
