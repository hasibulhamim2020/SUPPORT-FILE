...
$res='select  	a.req_no,a.req_no, DATE_FORMAT(a.req_date, "%d-%m-%Y") as req_date,  b.warehouse_name as despatch_depot,   DATE_FORMAT(a.need_by, "%d-%m-%Y") as need_by,  a.req_note as Transport , c.fname as entry_by, a.status from requisition_fg_master a,warehouse b,user_activity_management c where a.warehouse_id='.$_SESSION['user']['depot'].' and a.sub_depot=b.warehouse_id and a.entry_by=c.user_id '.$con.' and a.status in ("UNCHECKED","CHECKED") order by a.req_no desc';
