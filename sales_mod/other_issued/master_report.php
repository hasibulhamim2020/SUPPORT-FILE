...
{
$vondor_con=' and d.vendor_id="'.$_POST['vendor_id'].'"';

}
if($_POST['f_date']!='')
{
$date_con=' and m.pr_date between "'.$_POST['f_date'].'" and "'.$_POST['t_date'].'"';

}


   $sql="select m.pr_no,i.item_name,m.pr_date,d.vendor_name,
w.warehouse_name as depot,s.unit_price,s.total_unit,s.total_amt,m.status


from purchase_return_master m,purchase_return_details s,vendor d  , warehouse w, item_info i

where m.status in ('CHECKED','COMPLETED') and m.pr_no=s.pr_no and m.vendor_id=d.vendor_id and i.item_id=s.item_id
".$depot_con.$date_con.$item_con.$vondor_con.$dtype_con." order by m.pr_date,m.pr_no";
