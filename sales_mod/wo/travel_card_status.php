...
$target_url = 'travel_card_print_view.php';
...
if($_POST['fdate']!=''&&$_POST['tdate']!='') $con .= ' and c.tc_date between "'.$_POST['fdate'].'" and "'.$_POST['tdate'].'"';
    ...
    $res="select c.tc_no, c.tc_date,  m.do_no, m.job_no,  m.do_date, m.dealer_code, m.buyer_code, m.merchandizer_code,  d.dealer_name_e, c.entry_by from 
sale_do_master m, sale_do_travel_card c, dealer_info d, user_group u 
where 

 m.group_for=u.id  and m.dealer_code=d.dealer_code and m.do_no=c.do_no ".$group_for_con.$con.$dealer_con."  group by c.tc_no order by  c.tc_date, m.do_no,c.tc_no ";
$query = db_query($res);
...
<tr <?=($data->RCV_AMT>0)?'style="background-color:#FFCCFF"':'';?>>
  ...
  <td onClick="custom(<?=$data->tc_no;?>);" <?=(++$z%2)?'':'class="alt"';?>>&nbsp;<?=$data->do_no;?></td>
  <td onClick="custom(<?=$data->tc_no;?>);" <?=(++$z%2)?'':'class="alt"';?>><?=$data->job_no;?></td>
  <td onClick="custom(<?=$data->tc_no;?>);" <?=(++$z%2)?'':'class="alt"';?>>&nbsp;<?=$data->dealer_name_e;?></td>
