...
//if ($prevent_multi<1) {
		...
		//$master = find_all_field_sql("select * from sale_do_master m, dealer_info d where d.dealer_code = m.dealer_code and m.do_no = '".$or_no."'");
		  ...
		  $sql3 = 'update sale_do_details set edit_request=2, `accept_by` = "'.$accept_by.'", `accept_at` = "'.$accept_at.'" where do_no = '.$or_no;
		db_query($sql3);
...
?>
<script language="javascript">
window.location.href = "select_do_for_accept_request.php";
</script>
<?
