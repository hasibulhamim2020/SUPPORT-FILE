...
<td width="17%" align="right" class="qr-code">
              ...
              $print_url = "https://saaserp.ezzy-erp.com/app/views/sales_mod/wo/sales_order_print_view.php?c=" . rawurlencode($company_id) . "&v=" . rawurlencode($req_no_qr_data);
	...
	<div class="row">
		...
		<div class="col-md-6 col-sm-6 col-lg-6 left">
			<p class="left-text mt-1 mb-1"> SO No: <span> <span> <?php 
      if (!empty($master->manual_do_no)) {
        echo $master->manual_do_no;
      } else {
        echo $master->do_no;
      } ?> </span></p>
			...
			<p class="left-text mt-1 mb-1">VAT No: <span> <?php echo $master->vat?></span></p>
			<p class="left-text mt-1 mb-1">C.R No: <span> <?php if (!empty($dealer->cr_no)) {echo $dealer->cr_no;} else {echo "N/A";}?></span></p>

			<p class="left-text mt-1 mb-1">Remarks: <span> <?php if(!empty($master->remarks)) {echo $master->remarks;} else{echo "N/A";}?></span></p>
		</div>
		...
		<div class="col-md-6 col-sm-6 col-lg-6 right">
			...
			<p class="right-text mt-1 mb-1">JOB  No: <span><?php if(!empty($master->job_no)) {echo $master->job_no;} else{echo "N/A";}?></span></p>
			<p class="right-text mt-1 mb-1">Contact No: <span><?php if(!empty($master->contact_num)) {echo $master->contact_num;} else{echo "N/A";}?> </span></p>
			<p class="right-text mt-1 mb-1">Address: <span><?php if(!empty($master->delivery_address)) {echo $master->delivery_address;} else{echo "N/A";}?> </span></p>
...
<td align="right"><?= number_format($data->total_unit * $data->m_price, 2) ?></td>

<td align="right"><?=(int)find_a_field('sale_do_details','discount_per','id='.$data->id);?> %</td>
<td align="right"><?= number_format(find_a_field('sale_do_details','discount_amt','id='.$data->id), 2) ?></td>
<td align="right"><?= number_format($data->total_amt, 2); $tot_total_amt += $data->total_amt; ?>
</td>
