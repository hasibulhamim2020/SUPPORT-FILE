...
$page_name="Select Dealer for Demand Order";
...

{
		...
		db_query("INSERT INTO sale_do_details_del select * from sale_do_details  where do_no=".$$unique_master."");
...
if($_POST['category_discount']){
  ...
  $insQ = "INSERT INTO `sale_do_discount`( `do_no`, `sub_group_id`, `mrp_value`, `inv_value`,
   `discount_per`, `discount_amt`, `discount_id`, tr_type, `entry_at`, `entry_by`, dealer_code) 
     ...
     '".$discount_per."', '".$discount_amt."', '".$discount_id."', 'slab', '".$now."', '".$_SESSION['user']['id']."', ".$do_master->dealer_code.")"; 
		db_query($insQ);	

		$upql = db_query("update sale_do_master set discount = '".$discount_per."' where do_no = '".$or_no."' ");	
...
if($dealer->bank_reconsila=="YES" && $do_master->received_amt>0 && $do_master->sa_approval=="PENDING"){
