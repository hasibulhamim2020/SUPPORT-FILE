...
auto_complete_from_db('item_info','item_name','concat(item_name,"#>",finish_goods_code)','status="Active" and group_for="'.$_POST['group_for'].'"','item_id');
...
function count() {
    // Ensure unit_price is provided
    if (document.getElementById('unit_price').value !== '') {
        var unit_price = parseFloat(document.getElementById('unit_price').value) || 0;
        var pkt_size = parseFloat(document.getElementById('pkt_size').value) || 1; // Default pkt_size to 1 to avoid division by zero
        var dist_unit = parseFloat(document.getElementById('dist_unit').value) || 0;
        var pkt_unit = parseFloat(document.getElementById('pkt_unit').value) || 0;

        // Perform conversion only if dist_unit >= pkt_size
        if (dist_unit >= pkt_size) {
            var additional_pkt = Math.floor(dist_unit / pkt_size); // Calculate how many packets can be created
            pkt_unit += additional_pkt; // Add new packets to existing packets
            dist_unit %= pkt_size; // Keep the remainder in dist_unit

            // Update the input fields with calculated values
            document.getElementById('pkt_unit').value = pkt_unit;
            document.getElementById('dist_unit').value = dist_unit;
        }
		else{
...
function data_update(id,do_no){
	...
	var unit_price = document.getElementById('unit_price_new_' + id).value;
	var pkt_unit = document.getElementById('pkt_unit_new_' + id).value;
	var pkt_size = document.getElementById('pkt_size_new_' + id).value;
	var total_unit = document.getElementById('total_unit_new_' + id).value;
	var total_amt = document.getElementById('total_amt_new_' + id).value;
	
	
    var strURL = "update_do_details_ajax.php?do_no=" + do_no + "&details_id=" + details_id + "&dist_unit=" + dist_unit + "&unit_price=" + unit_price+ "&pkt_unit=" + pkt_unit+ "&pkt_size=" + pkt_size+ "&total_unit=" + total_unit+ "&total_amt=" + total_amt;
					...
					<div class="col-sm-6 col-md-6 col-lg-6 col-xl-6">
						<div class="container n-form2">
							...
							<div class="form-group row m-0 pb-1">
								...
								<div class="col-sm-8 col-md-8 col-lg-8 col-xl-8 p-0 pr-2 ">
									<?php	if($dealer_code){ ?>
											<input type="text" name="dealer_code_combo" id="dealer_code_combo" value="<?=find_a_field('dealer_info','concat(dealer_name_e,"#",dealer_code)','dealer_code="'.$dealer_code.'" and group_for in ('.$user_group_define.')')?>" readonly />
									<?	}else{ ?>
				...
				<table class="table1  table-striped table-bordered table-hover table-sm">
					...
					<tr>
						<td id="sub">
							...
							<?
auto_complete_from_db('item_info','concat(item_id,"-> ",item_name)','item_id',' product_nature in ("Salable","Both") and status="Active" and group_for="'.$group_for.'"','item_id');
						...
						<td>
							<input name="pkt_unit" type="text" id="pkt_unit"  required="required"  value=""  />						</td>
						<td>
		...
		<? if($$unique_master>0){?>
			...
			<div class="container-fluid pt-5 p-0 ">
			<?
			  $res='select a.id,b.item_name,a.item_id,a.unit_name, a.unit_price,a.pkt_unit,a.pkt_size,a.dist_unit, a.total_unit, 
		...
		?>
						...
						<input  name="pkt_unit_new_<?=$data->id;?>" type="hidden" class="input3" id="pkt_unit_new_<?=$data->id;?>" value="<?=$data->pkt_unit;?>"/>
						<input  name="pkt_size_new_<?=$data->id;?>" type="hidden" class="input3" id="pkt_size_new_<?=$data->id;?>" value="<?=$data->pkt_size;?>"/>
					<input  name="unit_price_new_<?=$data->id;?>" type="hidden" class="input3" id="unit_price_new_<?=$data->id;?>" value="<?=$data->unit_price;?>"/>
						...
						<input  name="total_unit_new<?=$data->id;?>" type="hidden" class="input3" id="total_unit_new<?=$data->id;?>" value="<?=$data->total_unit;?>"/>
						<input  name="total_amt_new<?=$data->id;?>" type="hidden" class="input3" id="total_amt_new<?=$data->id;?>" value="<?=$data->total_amt;?>"/>						
