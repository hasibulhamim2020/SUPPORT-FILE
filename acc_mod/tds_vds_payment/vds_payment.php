...
$vds_ledger = $config_ledger->vds_payable;
...
$res='select sum(j.cr_amt-j.dr_amt) as vds_amt,j.tr_no as system_invoice_no,j.sub_ledger,s.sub_ledger_name,v.invoice_no,invoice_date,j.group_for from journal j left join general_sub_ledger s on s.sub_ledger_id=j.sub_ledger left join vendor_invoice_master v on v.system_invoice_no=j.tr_no where j.ledger_id="'.$vds_ledger.'" and j.jv_date between "'.$_POST['fdate'].'" and "'.$_POST['tdate'].'" and j.tr_from in ("vendor_invoice_receive","service_bill") group by j.tr_no,j.sub_ledger';
...
$narration = 'Vendor VDS Payment. Invoice No.'.$data->invoice_no;
...
//add_to_journal($proj_id, $jv_no, $jv_date, $credit_ledger, $narration, 0, $total_vds, $tr_from, $tr_no,$credit_sub_ledger,$tr_id,$cc_code,$group,$entry_by,$entry_at);
	...
	$delete_query ="delete from secondary_journal where tr_from='vendor_vds_payment' and jv_no =".$jv_no." ";
...
<div class="form-container_large">
  ...
  $res='select sum(j.cr_amt-j.dr_amt) as vds_amt,j.tr_no,j.sub_ledger,s.sub_ledger_name,v.invoice_no,invoice_date from journal j left join general_sub_ledger s on s.sub_ledger_id=j.sub_ledger left join vendor_invoice_master v on v.system_invoice_no=j.tr_no where j.ledger_id="'.$vds_ledger.'" and j.jv_date between "'.$_POST['fdate'].'" and "'.$_POST['tdate'].'" and j.tr_from in ("vendor_invoice_receive","service_bill") '.$con.' group by j.tr_no,j.sub_ledger';
            ...
            $query = db_query($res);
				    ...
				    $paid_amount = find_a_field('journal','sum(dr_amt)','ledger_id="'.$vds_ledger.'" and sub_ledger="'.$row->sub_ledger.'" and tr_no="'.$row->tr_no.'" and tr_from="vendor_vds_payment"');
...
<input type="checkbox" name="system_invoice_no<?=$row->tr_no?>" class="form-check-input" id="system_invoice_no<?=$row->tr_no?>" onclick="payment_cal(<?=$row->tr_no?>)" /></td>
							<td><?=$row->tr_no?></td>
