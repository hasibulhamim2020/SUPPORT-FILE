...
if ($_GET['new'] > 0) {
	...
	unset($_SESSION['jv_no']['Receipt_bank']);

	unset($_SESSION['tr_no']['Receipt_bank']);

}


if ($_REQUEST['v']!='' && $_REQUEST['t']!='') {

	$_SESSION['jv_no']['Receipt_bank'] = url_decode(str_replace(' ', '+', $_REQUEST['v']));
	$_SESSION['tr_no']['Receipt_bank'] = url_decode(str_replace(' ', '+', $_REQUEST['t']));

}

if ($_GET['jv_no']['Receipt'] > 0) {

	$_SESSION['jv_no']['Receipt_bank'] = $_GET['jv_no']['Receipt'];

}

if ($_GET['tr_no']['Receipt'] > 0) {

	$_SESSION['tr_no']['Receipt_bank'] = $_GET['tr_no']['Receipt'];
...
$_SESSION['csrf_token'] = bin2hex(random_bytes(32));
	...
	if ($_SESSION['jv_no']['Receipt_bank'] == 0) {

		$jv_no = $_SESSION['jv_no']['Receipt_bank'] = next_journal_sec_voucher_id($tr_from, 'Receipt',$group_for,$_SESSION['user']['id']);

		$tr_no = $_SESSION['tr_no']['Receipt_bank'] = next_tr_no($tr_from);

	} else {

		$jv_no = $_SESSION['jv_no']['Receipt_bank'];

		$tr_no = $_SESSION['tr_no']['Receipt_bank'];
	...
	$field = 'file_upload';
	$file_name = $field . $_SESSION['jv_no']['Receipt_bank'];
	...
	if ($receive_ledger > 0 && $ledger_id>0 && $jv_date!='' && $jv_date!='0000-00-00') {
		if ($jv_no > 0) {
			add_to_sec_journal($proj_id, $jv_no, $jv_date, $ledger_id, $dnarr, '0', $amount, $tr_from, $tr_no, $sub_ledger, '', $cc_code,$group_for, $user_id, '', $r_from, $bank, $c_no, $cheq_date, $receive_ledger, $checked, $type, $employee, $remarks, $uploaded_file, $reference_id);
			 ...
			 $up = 'update secondary_journal set manual_voucher_no="' . $manual_voucher_no . '", exchange_rate="' . $exchange_rate . '", usd_amt="' . $usd_amt . '" where jv_no="' . $_SESSION['jv_no']['Receipt_bank'] . '"';
			db_query($up);
...
$_SESSION['csrf_token'] = bin2hex(random_bytes(32));
	if ($_SESSION['tr_no']['Receipt_bank'] > 0) {

		if ($_SESSION['jv_no']['Receipt_bank'] > 0) {

			$j_data = find_all_field('secondary_journal', '', 'jv_no="' . $_SESSION['jv_no']['Receipt_bank'] . '"');
	...
	$reconsilation_note = '';
			...
			$up = 'update secondary_journal set checked="NO",    manual_voucher_no="' . $j_data->manual_voucher_no . '", exchange_rate="' . $j_data->exchange_rate . '", usd_amt="' . $j_data->usd_amt . '",reconsilation_status="'.$reconsilation_type.'",reconsilation_note="'.$reconsilation_note.'" where jv_no="' . $_SESSION['jv_no']['Receipt_bank']. '" and tr_from="'.$tr_from.'"';
			db_query($up);


			$_SESSION['jv_no']['Receipt_bank'] = '';
...
$jv = find_all_field('secondary_journal', '', 'jv_no="' . $_SESSION['jv_no']['Receipt_bank'] . '"');
				...
				<div class="col-sm-6 col-md-6 col-lg-6 col-xl-6">
					<div class="container n-form2">
						...
						<div id="bank_check" class="form-group row m-0 pb-1">
							...
							<label
								class="col-sm-3 col-md-3 col-lg-3 col-xl-3 m-0 p-0 d-flex justify-content-end align-items-center pr-1 font-size12 bg-form-titel-text ">Bank
								A/C Debit:</label>
							<div class="col-sm-9 col-md-9 col-lg-9 col-xl-9 p-0 pr-2">
									<span id="type_bank" >
								<? if ($jv->tr_no > 0) { ?>
...
from  secondary_journal a where a.tr_from='Receipt' and a.type='BANK'  and SUBSTRING(a.tr_no,5,1)='0' and  a.dr_amt>0 and a.group_for in (" .user_company_access_list(). ") group by a.tr_no order by a.id desc limit 5";
...
from  secondary_journal a, accounts_ledger l where a.ledger_id=l.ledger_id and a.jv_no='" . $_SESSION['jv_no']['Receipt_bank'] . "' and a.cr_amt>0 and tr_from='Receipt'";
