...
$target_url = '../bank_voucher/general_voucher_print_view_for_draft.php';
...
if($_GET['new']>0){
	...
	unset($_SESSION['jv_no']['Payment_bank']);

	unset($_SESSION['tr_no']['Payment_bank']);

}
if($_GET['jv_no']>0){
  $_SESSION['jv_no']['Payment_bank']=$_GET['jv_no']['Payment'];
}

if($_GET['tr_no']['Payment']>0){
  $_SESSION['tr_no']['Payment_bank']=$_GET['tr_no']['Payment'];
}
...
if($_SESSION['jv_no']['Payment_bank']==0){

 $jv_no = $_SESSION['jv_no']['Payment_bank'] = next_journal_sec_voucher_id($tr_from,'Payment',$group_for,$_SESSION['user']['id']);

 $tr_no = $_SESSION['tr_no']['Payment_bank'] = next_tr_no($tr_from);
 ...
 $jv_no = $_SESSION['jv_no']['Payment_bank'];

$tr_no = $_SESSION['tr_no']['Payment_bank'];

	}

	$folder = 'PaymentVoucher';
	$field = 'file_upload';
	$file_name = $field . '-' . $_SESSION['jv_no']['Payment_bank'];
...
if($jv_no>0){	
	...
	$up = 'update secondary_journal set   manual_voucher_no="'.$manual_voucher_no.'" where jv_no="'.$_SESSION['jv_no']['Payment_bank'].'"';
	db_query($up);
...
if(isset($_POST['limmit']) && $_SESSION['csrf_token']===$_POST['csrf_token']){
    ...
    $_SESSION['csrf_token'] = bin2hex(random_bytes(32));
	  ...
	  if($_SESSION['tr_no']['Payment_bank']>0){
		if($_SESSION['jv_no']['Payment_bank']>0){
		  

			$j_data = find_all_field('secondary_journal','','jv_no="'.$_SESSION['jv_no']['Payment_bank'].'"');
			...
			add_to_sec_journal($proj_id, $_SESSION['jv_no']['Payment_bank'], $j_data->jv_date, $sub_led->ledger_id,  $detail, '0',$_POST['t_amount'], $tr_from,$j_data->tr_no,$j_data->relavent_cash_head,'',0,$group_for,$_SESSION['user']['id'],'',$j_data->received_from,$j_data->bank,$j_data->cheq_no,$j_data->cheq_date,$j_data->relavent_cash_head,$checked,
	...
	$reconsilation_note = '';
			...
			$up = 'update secondary_journal set checked="NO", manual_voucher_no="'.$j_data->manual_voucher_no.'", reconsilation_status="'.$reconsilation_type.'",reconsilation_note="'.$reconsilation_note.'" where jv_no="' . $_SESSION['jv_no']['Payment_bank']. '" and tr_from="'.$tr_from.'"';
			db_query($up);
            ...
            db_query('update cheque_book_details set status="Booked" where id="'.$j_data->cheq_no.'"');
			$_SESSION['jv_no']['Payment_bank']     = '';
...
$jv = find_all_field('secondary_journal','','jv_no="'.$_SESSION['jv_no']['Payment_bank'].'"');
...
<input 
    ...
    size="15" 
    tabindex="1" 
    value="<?php echo $tr_no; ?>" 
    <?php if ($jv->tr_no > 0) { echo 'disabled'; } ?> 
    class="form-control"
/>
...
<input 
    ...
    name="voucher_mode" 
    class="radio" 
    id="voucher_mode_auto" 
    value="1" 
    <?php if ($old_voucher_mode == 1) { echo 'checked="checked"'; } ?> 
...
/>
						...
						<div class="col-sm-6 col-md-6 col-lg-6 col-xl-6 pr-0">
    <div class="form-group row m-0 pb-1">
        <label class="col-sm-4 col-md-4 col-lg-4 col-xl-4 m-0 p-0 d-flex justify-content-end align-items-center pr-1 bg-form-titel-text font-size12 ">
            Paid To:
        </label>
        <div class="col-sm-8 col-md-8 col-lg-8 col-xl-8 p-0 pr-2 ">
            <input 
                ...
                type="text" 
                id="r_from" 
                value="<?php echo $jv->received_from; ?>" 
                class="form-control"  
                tabindex="3" 
                <?php if ($jv->tr_no > 0) { echo 'readonly'; } ?> 
            />
        </div>
    </div>
</div>	
							<div class="col-sm-6 col-md-6 col-lg-6 col-xl-6 pl-0">
								<div class="form-group row m-0 pb-1">
									...
									<div class="col-sm-7 col-md-7 col-lg-7 col-xl-7 p-0 pr-2 ">
										...
										<? if($jv->tr_no>0){ ?><input type="text" name="type" id="type" value="<?=$jv->type?>" readonly tabindex="4" class="form-control" /><? }else{?><select name="type" id="type" required onChange="check_type()" tabindex="4" class="form-control">
...
<div id="bank_check" class="form-group row m-0 pb-1">
    <label class="col-sm-3 col-md-3 col-lg-3 col-xl-3 m-0 p-0 d-flex justify-content-end align-items-center pr-1 font-size12 bg-form-titel-text ">
        Bank A/C Credit:
    </label>
    <div class="col-sm-9 col-md-9 col-lg-9 col-xl-9 p-0 pr-2">
		<span id="type_bank" >
        <?php if ($jv->tr_no > 0) { ?>
            <input 
                ...
                id="bank_disable_id"  
                tabindex="6" 
                value="<?php echo find_a_field('general_sub_ledger','sub_ledger_name','sub_ledger_id="'.$jv->relavent_cash_head.'"'); ?>" 
                readonly="readonly" 
            />
            <input 
                type="hidden" 
                name="b_id" 
                value="<?php echo $jv->relavent_cash_head; ?>" 
            />
        <?php } else { ?>
            <select 
                name="b_id" 
                id="bank_disable_id"  
                tabindex="2" onChange="getCheqBook(this.value)" 
                <?php if ($jv->tr_no > 0) { echo 'readonly'; } ?> 
            >
                ...
                <?php
                    foreign_relation(
                        ...
                        'sub_ledger_name',
                        $b_id,
                        "ledger_id=" . $gr_all->bank_ledger . " order by ledger_id"
                    );
                ?>
            </select>
        <?php } ?>
						...
						<span id="cheqBookBimol">
                        
                        <? 
						$cheqNo = find_a_field('cheque_book_details','cheq_no','cheq_no="'.$jv->cheq_no.'"');
						if($cheqNo>0){
							$cheqBookName =find_a_field('cheque_book_master','cheque_no_manual','cheq_no="'.$cheqNo.'"'); 
							?>
                          <div id="check_no_check" class="form-group row m-0 pb-1">
							<label class="col-sm-3 col-md-3 col-lg-3 col-xl-3 m-0 p-0 d-flex justify-content-end align-items-center pr-1 font-size12 bg-form-titel-text">Cheque Book No:</label>
							<div class="col-sm-9 col-md-9 col-lg-9 col-xl-9 p-0 pr-2">
                            <span id="cheqPage">

								<input name="cheqBookNo" type="text" id="cheqBookNo" value="<?php echo $cheqBookName?>" size="20" maxlength="25" tabindex="7" <? if($jv->tr_no>0) echo 'readonly'?> class="form-control"/>
                                </span>
                        ...
                        <div id="check_no_check" class="form-group row m-0 pb-1">
							<label class="col-sm-3 col-md-3 col-lg-3 col-xl-3 m-0 p-0 d-flex justify-content-end align-items-center pr-1 font-size12 bg-form-titel-text">Cheque/Page No:</label>
							<div class="col-sm-9 col-md-9 col-lg-9 col-xl-9 p-0 pr-2">
                            <span id="cheqPage">
...
<div id="of_bank_check" class="form-group row m-0 pb-1">
    ...
    <div class="col-sm-9 col-md-9 col-lg-9 col-xl-9 p-0 pr-2">
        ...
        />
        <datalist id="banks">
            <?php foreign_relation('drawee_bank', 'bank_name', 'bank_name', $jv->bank, '1 order by bank_name asc'); ?>
        </datalist>
    </div>
</div>
				...
				<div class="col-sm-6 col-md-6 col-lg-6 col-xl-6">
					<div class="container n-form2">
						<table  class="table1  table-striped table-bordered table-hover table-sm" border="0" cellspacing="0" cellpadding="0">
							...
							$sql2="select a.tr_no, a.cr_amt, a.narration,a.jv_date , a.jv_no,a.checked from  secondary_journal a where a.tr_from='Payment' and a.type='BANK' and  SUBSTRING(a.tr_no,5,1)='0' and  a.cr_amt>0 and a.group_for in (" .user_company_access_list(). ") group by a.tr_no order by a.id desc limit 5";
							...
							if(mysqli_num_rows($data2)>0){
								...
								{$dataa[2]=substr($dataa[2],0,20).'...';
									...
									<tr>
										...
										<a href="../cheque_setup/cheque_print_jv_print.php?jv_no=<?=$dataa[4];?>" target="_blank"><button type="button" class="btn2 btn1-bg-help"><i class="fa-solid fa-print"></i></button></a>
...
from  secondary_journal a, accounts_ledger l where a.ledger_id=l.ledger_id and a.jv_no='".$_SESSION['jv_no']['Payment_bank']."' and tr_from='Payment'";
...
<script>
function getCheqBook(bank){
	var group_for = document.getElementById('group_for').value*1;
	$.ajax({
        url: "getCheqBook_ajax.php",
        type: "post",
        data: { bank: bank, group_for:group_for} ,
		
        success: function (data) {

		  $("#cheqBookBimol").html(data);
        }
    })
	...
	function getCheqPage(cheq){
	var group_for = document.getElementById('group_for').value*1;
	$.ajax({
        url: "getCheqPage_ajax.php",
        type: "post",
        data: { cheq: cheq, group_for:group_for} ,
