...
<!--				<p class="text">Cell: <?=$group_data->mobile?>. Email: <?=$group_data->email?> <br> <?=$group_data->vat_reg?></p>-->
		...
		<td width="17%" align="right" class="qr-code">
              ...
              $print_url = "https://saaserp.ezzy-erp.com/app/views/acc_mod/bank_voucher/general_voucher_print_view_for_draft_client.php?c=" . rawurlencode($company_id) . "&v=" . rawurlencode($req_no_qr_data) . "&sub_ledger=" .rawurlencode($sub_ledger);
...
$sub_ledger=url_decode(str_replace(' ', '+', $_REQUEST['sub_ledger']));
 ...
 $sql2="SELECT a.ledger_id,a.ledger_name,sum(cr_amt) as cr_amt,sum(dr_amt) as dr_amt, a.ledger_group_id, b.narration, b.reference_id,b.sub_ledger, b.cc_code FROM accounts_ledger a, secondary_journal b where b.jv_no='$jv_no' and  b.sub_ledger='$sub_ledger' and a.ledger_id=b.ledger_id and jv_no=$jv_no group by b.id order by cr_amt desc";
$data2=db_query($sql2);
