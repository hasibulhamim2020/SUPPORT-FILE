...
<!--				<p class="text">Cell: <?=$group_data->mobile?>. Email: <?=$group_data->email?> <br> <?=$group_data->vat_reg?></p>-->
	...
	<div class="row">
		...
		<div class="col-md-6 col-sm-6 col-lg-6 left">
			...
			<p class="left-text mt-1 mb-1">Remarks : <span><?= $jv->remarks?> </span></p>
...
while($info=mysqli_fetch_object($data2)){		  
      ...
      <tr>
		...
		<? if ($jv_all->tr_from=='Payment') { ?>
					...
					<?php /*?><td align="left"><a style="text-decoration:none !important;" target="_blank" href="general_voucher_print_view_for_draft_client.php?jv_no=<?=rawurlencode(url_encode($jv_no))?>&sub_ledger=<?=rawurlencode(url_encode($info->sub_ledger))?>"><?=$info->ledger_id?></a></td><?php */?>
					
					
						<td align="left"><a style="text-decoration:none !important;" target="_blank" href="general_voucher_print_view_for_draft_client.php?c=<?=rawurlencode(url_encode($c_id))?>&v=<?=rawurlencode(url_encode($jv_no))?>&sub_ledger=<?=rawurlencode(url_encode($info->sub_ledger))?>"><?=$info->ledger_id?></a></td>
...
<div class="footer1">
	<table class="footer-table">
        ...
        <tr>
          ...
          <td class="text-center w-20">
		  <p style="font-weight:600; margin: 0;"> <?=find_a_field('user_activity_management','fname','user_id='.$jv->om_checked);?></p>
		  <p style="font-size:11px; margin: 0; margin-top: 3rem">
			   <?php 
