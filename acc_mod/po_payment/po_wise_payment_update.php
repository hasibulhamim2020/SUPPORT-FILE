...

{
	...
	$sub_ledger_id=$_POST['cr_ledger_id'];
	$cr_ledger_id = find_a_field('general_sub_ledger','ledger_id','sub_ledger_id='.$sub_ledger_id);
...
//   		$chalan_no=''.$yer.''.$month.''.$cy_id;
    ...
    $so_invoice = 'INSERT INTO payment_vendor (payment_no, payment_date, po_no, pr_no, vendor_id,ledger_id,sub_ledger_code,cr_ledger_id,sub_ledger_id,payment_amt, status, entry_at, entry_by)
  
  VALUES("'.$payment_no.'", "'.$payment_date.'", "'.$data->tr_id.'", "'.$tr_no.'", "'.$data->vendor_id.'", "'.$account_code.'","'.$sub_ledger_code.'","'.$cr_ledger_id.'","'.$sub_ledger_id.'","'.$payment_amt.'", "COMPLETE", "'.$entry_at.'", "'.$entry_by.'")';
            ...
            <div class="row">
                ...
                <div class="col-sm-9 col-md-9 col-lg-9 col-xl-9">
                    <div class="form-group row m-0">
                        <label class="col-sm-3 col-md-3 col-lg-3 col-xl-3 m-0 p-0 d-flex justify-content-end align-items-center pr-1 bg-form-titel-text">Supplier Name</label>
                        <div class="col-sm-9 col-md-9 col-lg-9 col-xl-9 p-0">
                           <select name="account_code" id="account_code">
							  <option></option>
							  <?
						
						foreign_relation('vendor','sub_ledger_id','vendor_name',$_POST['account_code'],'1');
						...
						<div class="container n-form2">
							<div class="form-group row m-0 pb-1">
								...
								<div class="col-sm-8 col-md-8 col-lg-8 col-xl-8 p-0 pr-2 ">
									...
									<? $dealer_data = find_all_field('vendor','','sub_ledger_id='.$_POST['account_code']); 
	$dealer_closing=find_a_field_sql("select sum(dr_amt-cr_amt) from journal where  ledger_id=".$dealer_data->ledger_id." and sub_ledger =".$_POST['account_code']." ");
			...
			<div class="container-fluid pt-5 p-0 ">
				...
				<table class="table1  table-striped table-bordered table-hover table-sm">
					...
					<?
						...
						$sql = "select sum(dr_amt) as payment_amt, ledger_id,sub_ledger tr_id  from journal where tr_from='Payment' group by tr_id ";
						$query = db_query($sql);
						while($data=mysqli_fetch_object($query)){
						$payment_amt[$data->ledger_id][$data->sub_ledger][$data->tr_id]=$data->payment_amt;
						
						}
						  
						  
						  
						 $sql = "select sum(dr_amt) as return_amt,ledger_id,sub_ledger,tr_id  from journal where tr_from='Purchase Return'  group by tr_id";
						$query = db_query($sql);
						while($data=mysqli_fetch_object($query)){
						$return_amt[$data->ledger_id][$data->sub_ledger][$data->tr_id]=$data->return_amt;
							 ...
							 $sql = "SELECT d.ledger_id,d.sub_ledger_id, d.vendor_name, j.tr_no, j.jv_date, sum(j.cr_amt) as invoice_amt, j.tr_id FROM journal j, vendor d WHERE j.ledger_id=d.ledger_id and j.sub_ledger=d.sub_ledger_id and j.tr_from in ('Purchase')  ".$account_code_con.$do_no_con."  group by j.tr_no";
						...
						<? if ($due_amt=$data->invoice_amt-$payment_amt[$data->ledger_id][$data->sub_ledger_id][$data->tr_no]+$return_amt[$data->ledger_id][$data->sub_ledger_id][$data->tr_no]>0) {?>
						  ...
						  <tr>
							...
							<td><strong>
							  <?=number_format($payment_amt[$data->ledger_id][$data->sub_ledger_id][$data->tr_no]+$return_amt[$data->ledger_id][$data->sub_ledger_id][$data->tr_no],2);?>
							</strong></td>
							<td><strong><? echo number_format($due_amt=$data->invoice_amt-$payment_amt[$data->ledger_id][$data->sub_ledger_id][$data->tr_no]+$return_amt[$data->ledger_id][$data->sub_ledger_id][$data->tr_no],2);?></strong>
							...
							<input name="sub_ledger_code_<?=$data->tr_no?>" id="sub_ledger_code_<?=$data->tr_no?>" type="hidden"  value="<?=$data->sub_ledger_id?>"  />
