...
<input name="receipt_no" type="text" id="receipt_no" class="form-control" size="15" value="<?=$tr_no?>" <? if($jv->tr_no>0) echo 'disabled'?> />
...
<td><? if($jv->tr_no>0){ ?><input type="text" name="type" id="type" value="<?=$jv->type?>" readonly="readonly" class="form-control" /><? }else{?><select name="type" id="type" required onChange="check_type()" class="form-control" >
...
<td colspan="3"><? if($jv->tr_no>0){ ?><input type="text" name="b_id_name" id="bank_disable_id" style="float:left" tabindex="2" value="<?=find_a_field('accounts_ledger','ledger_name','ledger_id="'.$jv->relavent_cash_head.'"');?>" readonly="readonly" class="form-control" /><input type="hidden" name="b_id" value="<?=$jv->relavent_cash_head?>" /><? }else{?><select name="b_id" id="bank_disable_id" style="float:left" tabindex="2" <? if($jv->tr_no>0) echo 'readonly'?>  class="form-control" >
...
<td colspan="3">
<input name="remarks" type="text" id="remarks" value="<?php echo $jv->remarks?>" class="form-control"   <? if($jv->tr_no>0) echo 'readonly'?> />
...
from  secondary_journal a where a.tr_from='Receipt'  and SUBSTRING(a.tr_no,5,1)='0' and  a.dr_amt>0 group by a.tr_no order by a.id desc limit 5";
...
<input name="add_new" class="btn btn-primary" type="submit" id="add_new" value="Add New" />

<input name="add" type="hidden" /></tr>

<tr>

<td align="center"><input type="text" class="form-control" id="ledger_id" name="ledger_id" onblur="getData2('acc_reference_ajax.php', 'acc_reference', this.value, 
...
<td align="center"><select name="cc_code" id="cc_code" class="form-control"  tabindex="3">
  <option></option>
...
<td align="center"><input name="amount" type="text" id="amount" size="5" class="form-control" tabindex="11"/></td>
</tr>
</table></td>
