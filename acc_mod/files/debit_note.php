...
if($_GET['mhafuz']>0){
	...
	unset($_SESSION['jv_no']['Payment']);

	unset($_SESSION['tr_no']['Payment']);

}
if($_GET['jv_no']>0){
  $_SESSION['jv_no']['Payment']=$_GET['jv_no']['Payment'];
}

if($_GET['tr_no']['Payment']>0){
  $_SESSION['tr_no']['Payment']=$_GET['tr_no']['Payment'];
}
...
if($_SESSION['jv_no']['Payment']==0){

 $jv_no = $_SESSION['jv_no']['Payment'] = next_journal_sec_voucher_id($tr_from,'Payment',$_SESSION['user']['group'],$_SESSION['user']['id']);

 $tr_no = $_SESSION['tr_no']['Payment'] = next_tr_no($tr_from);
 ...
 $jv_no = $_SESSION['jv_no']['Payment'];

$tr_no = $_SESSION['tr_no']['Payment'];

	}

	$folder = 'PaymentVoucher';
	$field = 'file_upload';
	$file_name = $field . '-' . $_SESSION['jv_no']['Payment'];
...
if($jv_no>0){	
	...
	$up = 'update secondary_journal set   manual_voucher_no="'.$manual_voucher_no.'" where jv_no="'.$_SESSION['jv_no']['Payment'].'"';
	db_query($up);
...
if(isset($_POST['limmit']) && $_SESSION['csrf_token']===$_POST['csrf_token']){
    ...
    $_SESSION['csrf_token'] = bin2hex(random_bytes(32));
	  ...
	  if($_SESSION['tr_no']['Payment']>0){
		if($_SESSION['jv_no']['Payment']>0){
		  

			$j_data = find_all_field('secondary_journal','','jv_no="'.$_SESSION['jv_no']['Payment'].'"');
			...
			add_to_sec_journal($proj_id, $_SESSION['jv_no']['Payment'], $j_data->jv_date, $sub_led->ledger_id,  $detail, '0',$_POST['t_amount'], $tr_from,$j_data->tr_no,$j_data->relavent_cash_head,'',0,'',$_SESSION['user']['id'],'',$j_data->received_from,$j_data->bank,$j_data->cheq_no,$j_data->cheq_date,$j_data->relavent_cash_head,$checked,
	...
	$reconsilation_note = '';
			...
			$up = 'update secondary_journal set checked="NO", manual_voucher_no="'.$j_data->manual_voucher_no.'", reconsilation_status="'.$reconsilation_type.'",reconsilation_note="'.$reconsilation_note.'" where jv_no="' . $_SESSION['jv_no']['Payment']. '" and tr_from="'.$tr_from.'"';
			db_query($up);
...
//
//
//		$del_jv = "delete from secondary_journal where tr_from='Payment' and tr_no = '".$_SESSION['tr_no']."'";
//		db_query($del_jv);
//
//
//$_SESSION['jv_no']     = '';
...
$jv = find_all_field('secondary_journal','','jv_no="'.$_SESSION['jv_no']['Payment'].'"');
...
<div class="form-container_large">
	...
	<?php 
						...
						<div id="bank_check" class="form-group row m-0 pb-1">
							...
							<label class="col-sm-3 col-md-3 col-lg-3 col-xl-3 m-0 p-0 d-flex justify-content-end align-items-center pr-1 font-size12 bg-form-titel-text ">Bank A/C Credit:</label>
							<div class="col-sm-9 col-md-9 col-lg-9 col-xl-9 p-0 pr-2">

								<? if($jv->tr_no>0){ ?><input type="text" name="b_id_name" id="bank_disable_id"  tabindex="6" value="<?=find_a_field('general_sub_ledger','sub_ledger_name','sub_ledger_id="'.$jv->relavent_cash_head.'"');?>" readonly="readonly" /><input type="hidden" name="b_id" value="<?=$jv->relavent_cash_head?>" /><? }else{?><select name="b_id" id="bank_disable_id"  tabindex="2" <? if($jv->tr_no>0) echo 'readonly'?> >
						...
						<!--<div class="form-group row m-0 pb-1">
							<label class="col-sm-3 col-md-3 col-lg-3 col-xl-3 m-0 p-0 d-flex justify-content-end align-items-center pr-1 font-size12 bg-form-titel-text">File Attachment:</label>
							<div class="col-sm-9 col-md-9 col-lg-9 col-xl-9 p-0 pr-2">

							<input name="file_upload" type="file" id="file_upload" value="<?php echo $jv->remarks ?>"
									tabindex="10" class="form-control" <? if ($jv->tr_no > 0) ?> />
								<?php $file_name = find_a_field('secondary_journal', 'file_upload', 'jv_no="' . $_SESSION['jv_no']['Payment'] . '" and file_upload!=""');
								if ($file_name != '') { ?>
									<a href="<?= SERVER_CORE ?>core/upload_view.php?name=<?= $file_name ?>&folder=PaymentVoucher&proj_id=<?= $_SESSION['proj_id'] ?>"
				...
				<div class="col-sm-6 col-md-6 col-lg-6 col-xl-6">
					<div class="container n-form2">
						<table  class="table1  table-striped table-bordered table-hover table-sm" border="0" cellspacing="0" cellpadding="0">
							...
							$sql2="select a.tr_no, a.cr_amt, a.narration,a.jv_date , a.jv_no,a.checked from  secondary_journal a where a.tr_from='Payment' and  SUBSTRING(a.tr_no,5,1)='0' and  a.cr_amt>0 and a.group_for=".$_SESSION['user']['group']." group by a.tr_no order by a.id desc limit 5";
							...
							if(mysqli_num_rows($data2)>0){
								...
								{$dataa[2]=substr($dataa[2],0,20).'...';
									...
									<tr>
										...
										<? if($cid == 'mamun'){?>
										<td><a href="general_voucher_print_view_for_draft_new.php?jv_no=<?php echo rawurlencode(url_encode($dataa[4]));?>" target="_blank"><img src="../images/print.png" width="16" height="16" border="0"></a></td>
										<? } else {?>
											<td><a href="general_voucher_print_view_for_draft.php?jv_no=<?php echo rawurlencode(url_encode($dataa[4]));?>" target="_blank"><img src="../images/print.png" width="16" height="16" border="0"></a> <?
										//foreign_relation('general_sub_ledger','sub_ledger_id','sub_ledger_name',$b_id,"ledger_id=".$gr_all->bank_ledger." order by ledger_id");
										 $sec_journal=find_a_field('secondary_journal','ledger_id','cr_amt!=0 and jv_no='.$dataa[4]);
										 if($gr_all->bank_ledger==$sec_journal){ ?>
										<?php /*?><a href="../cheque_setup/cheque_print_jv.php?jv_no=<?php echo rawurlencode(url_encode($dataa[4]));?>" target="_blank"><button type="button" class="btn2 btn1-bg-help"><i class="fa-solid fa-print"></i></button></a><?php */?>
...
from  secondary_journal a, accounts_ledger l where a.ledger_id=l.ledger_id and a.jv_no='".$_SESSION['jv_no']['Payment']."' and tr_from='Payment'";
