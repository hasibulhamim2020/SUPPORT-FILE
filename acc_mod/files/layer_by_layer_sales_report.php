...
$p="select DISTINCT a.ledger_name, 1, 1, a.ledger_id, c.group_id from accounts_ledger a, ledger_group c where a.ledger_group_id=c.group_id and a.ledger_group_id='".$_REQUEST['group_id']."'  ".$con." order by a.ledger_id";
...
$amt = find_all_field_sql('select SUM(dr_amt) as dr_amt, SUM(cr_amt) as cr_amt from journal where jv_date between '.$fdate.' and '.$tdate.' and ledger_id = '.$p[3]);

$open = find_all_field_sql("select SUM(dr_amt - cr_amt) as open from journal where jv_date < '$fdate' and ledger_id=".$p[3]);



echo 'select SUM(dr_amt) as dr_amt, SUM(cr_amt) as cr_amt from journal where jv_date between '.$fdate.' and '.$tdate.' and ledger_id = '.$p[3]; 
...
<td colspan="5" align="left" bgcolor="#003366"><span class="style3">Group Name : 
...
<td colspan="5" align="left" bgcolor="#FFFFFF" height="1"></td>
</tr>
...
<td colspan="5" align="left" bgcolor="#333333"><span class="style3">A/C Ledger Name :
...
<td colspan="5" align="left" bgcolor="#999999"><span class="style3">Sub Ledger  Name :
