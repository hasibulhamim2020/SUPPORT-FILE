...
if(isset($_REQUEST['show']))
{
	...
	if($cc_code > 0)
	{
		$g="select DISTINCT c.group_name,SUM(dr_amt),SUM(cr_amt),c.group_id FROM accounts_ledger a, journal b,ledger_group c where a.ledger_id=b.ledger_id and a.ledger_group_id=c.group_id and b.jv_date BETWEEN '$fdate' AND '$tdate' and c.group_for=".$_SESSION['user']['group'].$con." AND b.cc_code=$cc_code group by c.group_name";
		
	}
	else
	{
		$g="select DISTINCT c.group_name,SUM(dr_amt),SUM(cr_amt),c.group_id FROM accounts_ledger a, journal b,ledger_group c where a.ledger_id=b.ledger_id and a.ledger_group_id=c.group_id and b.jv_date BETWEEN '$fdate' AND '$tdate' and c.group_for=".$_SESSION['user']['group'].$con." group by c.group_name";
...
$total_opening = $opening + $total_opening;
  ...
  <tr <? $ia++; if($ia%2==0)$cls=' class="alt"'; else $cls=''; echo $cls;?>>
    <td align="right"><?=++$s;?></td>
