...
$title='Payment Statement(Detail)';
...
<?php
	if(isset($_REQUEST['show']))
	{
		...
		if($cc_code > 0)
		{
			$total=mysqli_fetch_row(db_query("select sum(dr_amt),sum(cr_amt) from journal where tr_from='Payment' and jv_date>'$fdate' and jv_date<'$tdate' and ledger_id like '$ledger_id' and 1 AND cc_code=$cc_code"));
			
			$p="select a.jv_date,b.ledger_name,a.dr_amt,a.cr_amt,a.tr_no,a.narration from journal a,accounts_ledger b where tr_from='Payment' and a.ledger_id=b.ledger_id and a.jv_date>'$fdate' and a.jv_date<'$tdate' and a.ledger_id like '$ledger_id' and 1 AND a.cc_code=$cc_code order by a.tr_no";
			
		}
		else
		{
			$total=mysqli_fetch_row(db_query("select sum(dr_amt),sum(cr_amt) from journal where tr_from='Payment' and jv_date>'$fdate' and jv_date<'$tdate' and ledger_id like '$ledger_id' and 1"));
			
			$p="select a.jv_date,b.ledger_name,a.dr_amt,a.cr_amt,a.tr_no,a.narration from journal a,accounts_ledger b where tr_from='Payment' and a.ledger_id=b.ledger_id and a.jv_date>'$fdate' and a.jv_date<'$tdate' and a.ledger_id like '$ledger_id' and 1 order by a.tr_no";
