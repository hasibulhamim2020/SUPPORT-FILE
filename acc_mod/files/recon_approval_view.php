...
$tr_from = url_decode(str_replace(' ', '+', $_REQUEST['tr_from']));


$jv = find_all_field('journal','*','tr_from="'.$tr_from.'" and jv_no='.$jv_no);

if($jv->tr_from=='Receipt'){$voucher_name='RECEIPT VOUCHER';$vtypes='Receipt';}

elseif($jv->tr_from=='Payment'){$voucher_name='PAYMENT VOUCHER';$vtypes='Payment';}

else{$vtype=='secondary_journal';$voucher_name=$jv->tr_from;$vtypes='secondary_journal';}
...
$recon_note = $_POST['recon_note'].'. Date='.date('Y-m-d').'';
$ssql='update secondary_journal set reconsilation_status="Yes", reconsilation_note="'.$recon_note.'", checked_at="'.date('Y-m-d H:i:s').'",checked_by="'.$_SESSION['user']['id'].'"  where tr_from = "'.$tr_from.'" and jv_no="'.$jv_no.'"';
db_query($ssql);

$ssql='update journal set reconsilation_status="Yes", reconsilation_note="'.$recon_note.'", checked_at="'.date('Y-m-d H:i:s').'",checked_by="'.$_SESSION['user']['id'].'"  where tr_from = "'.$tr_from.'" and jv_no="'.$jv_no.'"';
db_query($ssql);
//header('Location:recon_approval_view.php?jv_no='.$jv_no.'&&tr_from='.$tr_from.'');

header('Location: recon_approval_view.php?c=' . rawurlencode(url_encode($c_id)) . '&v=' . rawurlencode(url_encode($jv_no)) . '&tr_from=' . rawurlencode(url_encode($tr_from)));
}
...
echo $_SESSION['proj_name'];
              ...
              $print_url = "https://saaserp.ezzy-erp.com/app/views/acc_mod/files/recon_approval_view.php?c=" . rawurlencode($company_id) . "&v=" . rawurlencode($req_no_qr_data). "&tr_from=" . rawurlencode($tr_from);
...
<div id="pr">
<? if($jv->reconsilation_status!='Yes'){?>
<div align="left">
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" bgcolor="#CCCCCC">
  <tr>
    ...
    <td width="160" align="right">Note :</td>
    <td width="342"><input type="text" name="recon_note" id="recon_note" value="<?=$jv->reconsilation_note?>" style="width:80%;" /> </td>
    <td width="111"><input name="check" type="submit" id="check" value="CHECK" /></td>
  </tr>
</table>
...
<a target="_blank" href="chalan_view2.php?v_no=<?=$pr->po_no?>"></a></div><?  }?>
  ...
  <tr>
    <td><table width="100%" border="0" cellpadding="0" cellspacing="0" bordercolor="#000000" class="tabledesign">
	  ...
	  <?
 $sql2="SELECT a.ledger_id,a.ledger_name,sum(dr_amt) as dr_amt, a.ledger_group_id, b.narration, b.reference_id, b.cc_code,b.sub_ledger FROM accounts_ledger a, secondary_journal b where b.jv_no='$jv_no' and a.ledger_id=b.ledger_id and jv_no=$jv_no and dr_amt>0 group by b.id order by dr_amt desc";
$data2=db_query($sql2);
...
<?php }?>
<?
 $sql2="SELECT a.ledger_id,a.ledger_name,sum(cr_amt) as cr_amt, a.ledger_group_id, b.narration, b.reference_id, b.cc_code,b.sub_ledger FROM accounts_ledger a, secondary_journal b where b.jv_no='$jv_no' and a.ledger_id=b.ledger_id and jv_no=$jv_no and cr_amt>0 group by b.id order by cr_amt desc";
$data2=db_query($sql2);
while($info=mysqli_fetch_object($data2)){
$sub_ledger = find_a_field('general_sub_ledger','sub_ledger_name','sub_ledger_id="'.$info->sub_ledger.'"');		  
	  ?>
  ...
  <tr>
	...
	<tr>
        ...
        <td align="center" valign="bottom">
			...
			<?php } ?>
			<br>
			<? echo find_a_field('user_activity_management','fname','user_id='.$jv->om_checked);?>
		</td>
