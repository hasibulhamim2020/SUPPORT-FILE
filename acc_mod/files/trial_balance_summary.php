...
$title='Trial Balance(At a Glance)';
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
    ...
    <td><div class="left_report">
		    ...
		    As On :                                       </td>
							  ...
							  <tr>
								...
								<th width="45%" height="20" align="center">Head Of Ledger Group </th>
  ...
  if(isset($_REQUEST['show']))
  {
	...
	if($cc_code > 0)
	{
    	$p = "select DISTINCT c.group_name,SUM(dr_amt),SUM(cr_amt) from accounts_ledger a, journal b,ledger_group c where a.ledger_id=b.ledger_id and a.ledger_group_id=c.group_id and b.jv_date <= '$tdate' and c.group_for=".$_SESSION['user']['group']." AND cc_code=$cc_code group by c.group_name";

	}
	else
	{
    	$p = "select DISTINCT c.group_name,SUM(dr_amt),SUM(cr_amt) from accounts_ledger a, journal b,ledger_group c where a.ledger_id=b.ledger_id and a.ledger_group_id=c.group_id and b.jv_date <= '$tdate' and c.group_for=".$_SESSION['user']['group']." group by c.group_name";
...
$cr=$p[2];
  ...
  <tr>
    <th colspan="2" align="right">Total Balance : <?=number_format(($total_dr-$total_cr),2);?></th>
