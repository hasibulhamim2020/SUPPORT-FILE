...

{
	...
	if($fdate!='0000-00-00'&&$tdate!='0000-00-00')	{$con .= 'AND jv_date BETWEEN "'.$fdate.'" AND "'.$tdate.'" ';  }
	...
	if($user_id>0)		{$con .= "AND user_id = '".$user_id."'";  }
	 ...
	 $sql="SELECT DISTINCT 
				...
				WHERE
				  ...
				  1 and j.checked='UNFINISHED' ".$con."
...

{
    var type = document.getElementById('v_type').value;
	 if(type=='payment'){
	var URL = 'debit_note.php?'+theUrl;
	}else if(type=='receipt'){
	  var URL = 'credit_note.php?'+theUrl;
	}
	else if(type=='coutra'){
	  var URL = 'coutra_note_new.php?'+theUrl;
	}else if(type=='journal'){
	  var URL = 'journal_note_new.php?'+theUrl;
	}
	  ...
	  ?>
...

		{
			...
			$v_type = $_REQUEST['v_type'];$i++; if($i%2==0)$cls=' class="alt"'; else $cls='';?>
							   ...
							   <tr<?=$cls?> onclick="DoNav('<?php echo 'tr_no='.$vno[0].'&jv_no='.$vno[4] ?>');">
