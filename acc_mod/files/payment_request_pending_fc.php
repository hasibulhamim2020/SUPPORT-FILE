...
$unique = 'vo_no';
$status = 'UNCHECKED';
$target_url = 'payment_request_print_view_fc.php';
...
$con.= 'and s.jv_date between "'.$_POST['fdate'].'" and "'.$_POST['tdate'].'"';
  ...
  $res='select  	s.jv_no,  s.tr_no as Request_id, DATE_FORMAT(s.jv_date, "%d-%m-%Y") as date,   a.ledger_name as bank_name, sum(cr_amt) as Amount,   u.fname as entry_by,  s.entry_at from secondary_journal s, accounts_ledger a, user_activity_management u where 
s.ledger_id=a.ledger_id and s.entry_by=u.user_id and s.dr_amt<1 

and s.ca_checked>0 and s.fc_checked<1 and s.om_checked<1 and s.ceo_checked<1
