...
$con.= 'and s.jv_date between "'.$_POST['fdate'].'" and "'.$_POST['tdate'].'"';
  ...
  $res='select  	s.jv_no, s.tr_no as Request_id, DATE_FORMAT(s.jv_date, "%d-%m-%Y") as date,  a.ledger_name as bank_name, sum(cr_amt) as Amount, s.checked,   u.fname as checked_by,  s.checked_at from secondary_journal s, accounts_ledger a, user_activity_management u where 
s.ledger_id=a.ledger_id and s.checked_by=u.user_id and s.dr_amt<1 and
