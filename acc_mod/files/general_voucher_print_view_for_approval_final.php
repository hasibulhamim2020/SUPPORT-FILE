...
//$po = find_all_field('purchase_master','po_no','po_no='.$po_no);
    ...
    //$narration = 'Sale#'.$po->sale_no.'/ PO#'.$po->po_no.' (Bill#'.$bill_no.'/ Dt:'.$bill_date.')';
	  ...
	  $ssql='update secondary_journal set  checked="YES", checked_at="'.$time_now.'", checked_by="'.$_SESSION['user']['id'].'" where
	 tr_from = "'.$jv_all->tr_from.'" and jv_no="'.$jv_no.'"';
	db_query($ssql);

$sa_data = find_all_field('secondary_journal','','tr_from="'.$jv_all->tr_from.'" and jv_no="'.$jv_no.'"');


sec_journal_journal($jv_no,$jv_no,$jv_all->tr_from);


$sa_up2='update journal set secondary_approval="Yes", checked="YES", checked_by="'.$_SESSION['user']['id'].'", checked_at="'.$time_now.'",
 om_checked_at="'.$sa_data->om_checked_at.'" , om_checked="'.$sa_data->om_checked.'"  where jv_no="'.$jv_no.'" and tr_from="'.$jv_all->tr_from.'"';
db_query($sa_up2);
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0">
	  <tr>
	    <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
          <tr>
            <td width="1%">
			<img src="<?=SERVER_ROOT?>public/uploads/logo/<?=$c_id?>.png" style=" height:80px; width:auto;" />		</td>
...
echo $_SESSION['proj_name'];
		...
		<td width="17%" align="right" class="qr-code">
              ...
              $print_url = "https://saaserp.ezzy-erp.com/app/views/acc_mod/files/general_voucher_print_view_for_approval_final.php?c=" . rawurlencode($company_id) . "&v=" . rawurlencode($req_no_qr_data);
  ...
  <tr>
	...
	<tr>
        ...
        <td align="center" valign="bottom">
			...
			<?php } ?>
			<br>
			<? echo find_a_field('user_activity_management','fname','user_id='.$jv->om_checked);?></td>
        <td align="center" valign="bottom">
			 <?php 
			$sign_data = find_a_field('user_activity_management','sign','user_id='.$jv->checked_by);
...
<?
$page_name="General Voucher Approval Final";
