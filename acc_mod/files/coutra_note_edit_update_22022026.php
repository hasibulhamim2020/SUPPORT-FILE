...
$jv_val=find_a_field('journal','count(id)',"tr_no='".$_SESSION['tr_no']."' and tr_from='Contra'");
...
if($jv_val>0){
 $del_jv = "delete from journal where tr_from='Contra' and tr_no = '".$_SESSION['tr_no']."'";
 db_query($del_jv);
}
 ...
 where jv_no="'.$_SESSION['jv_no'].'" and tr_from="Contra"';
...
if($_SESSION['jv_no']==0){
 ...
 $jv_no = $_SESSION['jv_no'] = next_journal_sec_voucher_id($tr_from,'Contra');
...
var info = ledger_id+"<@>"+reference_id+"<@>"+cc_code+"<@>"+narration+"<@>"+dr_amt+"<@>"+cr_amt+"<@>"+flag;
...
and a.jv_no='".$_SESSION['jv_no']."'group by a.tr_no order by a.tr_no desc limit 5";
...
from  secondary_journal a, accounts_ledger l where a.ledger_id=l.ledger_id and a.jv_no='".$_SESSION['jv_no']."' and a.tr_from='Contra' ";
...
<table width="100%" border="0" align="right" cellpadding="2" cellspacing="2">
	...
	<? $deffr = find_a_field('secondary_journal','sum(dr_amt-cr_amt)','tr_from="Contra" and jv_no='.$_SESSION['jv_no']);
