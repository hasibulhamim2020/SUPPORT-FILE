...
$title='Depot Received Journal Varification';
		...
		if($_POST['dealer_group']!='') {$dealer_group_con=' and d.product_group="'.$_POST['dealer_group'].'"';}
		 ...
		 $sql="SELECT DISTINCT 
				...
				WHERE
				  ...
				  j.ledger_id = 1102000100000000 and 

				  j.user_id=u.user_id and

				  w.store_rcv_ledger_id = l.ledger_id and 

				  j.tr_from = 'StoreReceived' AND 

				  j.tr_no = m.pi_no and w.warehouse_id=m.warehouse_to and 
...
<?=number_format(($data->cr_amt-$data->dr_amt),2);?>
      ...
      <td align="center"><a target="_blank" href="store_received_journal_print_view.php?jv_no=<?=$data->jv_no ?>">
...
<input type="button" name="Button" value="YES"  onclick="window.open('store_received_journal_print_view.php?jv_no=<?=$data->jv_no;?>');" style=" font-weight:bold;width:40px; height:20px;background-color:#66CC66;"/>
...
<input type="button" name="Button" value="NO"  onclick="window.open('store_received_journal_print_view.php?jv_no=<?=$data->jv_no;?>');" style="font-weight:bold;width:40px; height:20px;background-color:#FF0000;"/>
