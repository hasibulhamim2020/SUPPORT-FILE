...
$title='Black Tea Stock Journal Verification';
...
<td rowspan="4"><input type="submit" name="submitit" id="submitit" value="View Entry" style="width:170px; font-weight:bold; font-size:12px; height:30px; color:#090"/></td>
              ...
              <tr>
                ...
                <td><div align="right"><strong>Stock Depot : </strong></div></td>

                <td colspan="3"><div align="left"><span class="oe_form_group_cell" style="padding: 2px 0 2px 2px;">

                  <select name="depot_id" id="depot_id" style="width:250px;">

                    <? foreign_relation('warehouse','warehouse_id','warehouse_name',$depot_id,'warehouse_id="2" order by warehouse_name');?>
...
if($_SESSION['user']['group']>1) $group_s='AND j.group_for='.$_SESSION['user']['group'];
		...
		if($_POST['dealer_group']!='') {$dealer_group_con=' and d.product_group="'.$_POST['dealer_group'].'"';}
		  ...
		  $sql="SELECT DISTINCT 
				...
				WHERE
				  ...
				  j.ledger_id = 1093000100000000 and 

				  j.user_id=u.user_id and

				  w.bt_stock_ledger_ctg = l.ledger_id and 

				  j.tr_from = 'BlackTea Stock' AND 
      ...
      <td align="center"><a target="_blank" href="black_tea_stock_verify_print_view.php?jv_no=<?=$data->jv_no ?>">
...
<input type="button" name="Button" value="YES"  onclick="window.open('black_tea_stock_verify_print_view.php?jv_no=<?=$data->jv_no;?>');" style=" font-weight:bold;width:40px; height:20px;background-color:#66CC66;"/>
...
<input type="button" name="Button" value="NO"  onclick="window.open('black_tea_stock_verify_print_view.php?jv_no=<?=$data->jv_no;?>');" style="font-weight:bold;width:40px; height:20px;background-color:#FF0000;"/>
