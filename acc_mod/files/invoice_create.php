...
if(isset($_POST['limmit'])){
	  ...
	  if($_SESSION['tr_no']>0){
           ...
           $jv_config = find_a_field('voucher_config','direct_journal','voucher_type="Invoice"');
		   
		   if($jv_config=="Yes"){

$time_now = date('Y-m-d H:i:s');
$up2='update secondary_journal set checked="YES",checked_at="'.$time_now.'",checked_by="'.$_SESSION['user']['id'].'" where jv_no="'.$_SESSION['jv_no'].'" and tr_from="Invoice"';
...
$up2='update secondary_journal set checked="NO" where jv_no="'.$_SESSION['jv_no'].'" and tr_from="Invoice"';
...
$jv = find_all_field('secondary_journal','','jv_no="'.$_SESSION['jv_no'].'" and tr_from="Invoice"');
			...
			<div class="row">
				...
				<div class="col-sm-6 col-md-6 col-lg-6 col-xl-6">
					<div class="container n-form2">
						...
						<div class="form-group row m-0 pb-1">
							<label class="col-sm-3 col-md-3 col-lg-3 col-xl-3 m-0 p-0 d-flex justify-content-end align-items-center pr-1 font-size12 bg-form-titel-text">Transaction by:</label>
							<div class="col-sm-9 col-md-9 col-lg-9 col-xl-9 p-0 pr-2">
								<input name="r_from" type="text" id="r_from" value="<?php echo $jv->received_from?>" tabindex="1" required <? if($jv->tr_no>0) echo 'readonly'?> class="form-control"/>
							</div>
						</div>
						...
						<div class="form-group row m-0 pb-1">
							...
							<div class="col-sm-9 col-md-9 col-lg-9 col-xl-9 p-0 pr-2">
								<input name="c_no" type="text" id="c_no" value="<?php echo $jv->cheq_no?>" size="20" maxlength="25" tabindex="4" <? if($jv->tr_no>0) echo 'readonly'?> class="form-control" />
							</div>
						</div>
						...
						<div class="form-group row m-0 pb-1">
							...
							<div class="col-sm-9 col-md-9 col-lg-9 col-xl-9 p-0 pr-2">
								<input name="c_date" type="text" id="c_date" value="<?php echo $jv->cheq_date?>" size="12" maxlength="15" tabindex="5" <? if($jv->tr_no>0) echo 'readonly'?> class="form-control" />
							</div>
						</div>
						...
						<div class="form-group row m-0 pb-1">
							...
							<div class="col-sm-9 col-md-9 col-lg-9 col-xl-9 p-0 pr-2">
								<input name="bank" type="text" id="bank" value="<?php echo $jv->bank?>" class="form-control"  tabindex="6" <? if($jv->tr_no>0) echo 'readonly'?>/>
							</div>
						</div>


						<div class="form-group row m-0 pb-1">
							<label class="col-sm-3 col-md-3 col-lg-3 col-xl-3 m-0 p-0 d-flex justify-content-end align-items-center pr-1 font-size12 bg-form-titel-text ">Cost Center:	</label>
							<div class="col-sm-9 col-md-9 col-lg-9 col-xl-9 p-0 pr-2">

								<? if($jv->tr_no>0){?><input type="text" name="cc_name" value="<?=find_a_field('cost_center','center_name','id="'.$jv->cc_code.'"');?>" readonly="readonly" class="form-control" /><input type="hidden" name="cc_code" value="<?=$jv->cc_code?>" /><? }else{ ?> <select name="cc_code" id="cc_code"  class="form-control" >
...
from  secondary_journal a where a.tr_from='Invoice' and a.group_for='".$_SESSION['user']['group']."' and SUBSTRING(a.tr_no,5,1)='0' and  a.dr_amt>0 order by a.tr_no desc limit 6";
							...
							if(mysqli_num_rows($data2)>0){
								...
								{$dataa[2]=substr($dataa[2],0,20).'...';
									...
									<tr>
										...
										<td><a href="voucher_print_receipt.php?v_type=Invoice&vo_no=<?php echo $dataa[4];?>" target="_blank"><img src="../images/print.png" width="16" height="16" border="0"></a></td>
		...
		<div class="container-fluid pt-4 p-0 ">
			...
			<table class="table1  table-striped table-bordered table-hover table-sm">
				...
				<tr>
					...
					<td align="center"><input name="d_amount" type="text" id="d_amount" size="10"  tabindex="11" class="form-control"/></td>




					<td align="center"><input name="c_amount" type="text" id="c_amount" size="10" tabindex="11" class="form-control"/></td>
		...
		<div class="container-fluid pt-4 p-0 ">
			...
			<table class="table1  table-striped table-bordered table-hover table-sm" align="center" border="1">
    ...
    from  secondary_journal a, accounts_ledger l where a.ledger_id=l.ledger_id and a.jv_no='".$_SESSION['jv_no']."' and tr_from='Invoice'";
				...
				while($data=mysqli_fetch_object($qr)){
					...
					<tr>
						...
						<td ><?=($data->dr_amt>0)? $data->dr_amt : '0' ?></td>

						<td ><?=($data->cr_amt>0)? $data->cr_amt : '0' ?></td>
	...
	<form method="post">
		...
		<tr>
			<? $deffr = find_a_field('secondary_journal','sum(dr_amt-cr_amt)','tr_from="Invoice" and jv_no='.$_SESSION['jv_no']);
			if($deffr != 0){ echo '	<div class="alert alert-warning p-1" role="alert">Debit and Credit are not same</div>';}
			else{
				?>

				<td colspan="4">
				<input name="receipt_varify" class="btn1 btn1-submit-input" type="button" id="receipt_varify" value="Invoice Verified" onclick="this.form.submit()" />
...
<input type="hidden" name="voucher_mode" class="radio" id="voucher_mode_auto" value="1" <?php if($old_voucher_mode==1){ ?>checked="checked"<? } ?> onclick="voucher_no(this.value)"/>
                      ...
                      <tr>
                        ...
                        <td><div align="right">Transaction by:</div></td>
                        <td><input name="r_from" type="text" id="r_from" value="<?php echo $jv->received_from?>" tabindex="1" required <? if($jv->tr_no>0) echo 'readonly'?> class="form-control"/></td>
                      ...
                      <tr>
                        ...
                        <td colspan="3"><input name="c_no" type="text" id="c_no" value="<?php echo $jv->cheq_no?>" size="20" maxlength="25" tabindex="4" <? if($jv->tr_no>0) echo 'readonly'?> class="form-control" /></td>
                      ...
                      <tr><td><div align="right"><span>Cheque</span> Date:</div></td>
                        <td colspan="3"><input name="c_date" type="text" id="c_date" value="<?php echo $jv->cheq_date?>" size="12" maxlength="15" tabindex="5" <? if($jv->tr_no>0) echo 'readonly'?> class="form-control" /></td>
...
<td  style="padding:1px;" ><a href="voucher_print_receipt.php?v_type=Invoice&vo_no=<?php echo $dataa[4];?>" target="_blank"><img src="../images/print.png" width="16" height="16" border="0"></a></td>
...
<table width="100%" border="0" align="right" cellpadding="2" cellspacing="2">
	...
	?>
              ...
              <td width="19%" style="text-align:right;"><input name="receipt_varify" class="btn1 btn1-submit-input" type="button" id="receipt_varify" value="Invoice Verified" onclick="this.form.submit()" /> <input name="limmit" type="hidden" value="" />             </td>
