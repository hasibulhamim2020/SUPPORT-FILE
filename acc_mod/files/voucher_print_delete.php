...
elseif($vtype=='contra'){$voucher_name='CONTRA VOUCHER';$vtype='contra';$vtypes='contra';}

elseif($vtype=='coutra'){$voucher_name='CONTRA VOUCHER';$vtypes='contra';}
else{ $voucher_name = str_replace("_"," ",ucwords($vtype)); }
...
$vdate=$vtype."_date";
 ...
 $vo_no = find_a_field('secondary_journal','tr_no','jv_no='.$_REQUEST['vo_no'].' and tr_from = "'.ucwords($vtype).'"');

$address=find_a_field('project_info','proj_address',"1");


if(isset($_POST['delete_vouchar'])){
//////////journal delete////////
	$del_sql='delete from journal where jv_no="'.$_GET['vo_no'].'" and tr_from="'.ucwords($vtype).'"';
	db_query($del_sql);
	
	//////////secondary Journal delete////////
	$sec_sql='delete from secondary_journal where jv_no="'.$_GET['vo_no'].'" and tr_from="'.ucwords($vtype).'"';
	db_query($sec_sql);
	header("Location: vouchar_delete_list.php");
}
...

{
 ...
 $sql1="select jv_date,cc_code,entry_by from secondary_journal where tr_no=$vo_no and tr_from = '".$vtype."' limit 1";
...
$sql2="SELECT a.ledger_name,a.ledger_group_id,b.* FROM accounts_ledger a, secondary_journal b where b.tr_no='$vo_no' and b.tr_from = '".$vtype."' and a.ledger_id=b.ledger_id order by b.id";

else

$sql2="SELECT a.ledger_name,a.ledger_group_id,b.* FROM accounts_ledger a, secondary_journal b where b.tr_no='$vo_no' and b.tr_from = '".$vtype."' and a.ledger_id=b.ledger_id order by b.dr_amt desc,b.id";
...
echo find_a_field('user_group','group_name','1');
...
<a href="voucher_print_view.php?v_type=<?=$_REQUEST['v_type']?>&vo_no=<?=$_REQUEST['vo_no']?>">Client Copy</a></div>
      ...
      <tr>
        ...
        <td class="tabledesign_text">Voucher Date : <?=date('d-m-Y',strtotime($vo_date))?></td>
  ...
  <tr>
    ...
    <td class="tabledesign_text"><table width="100%" border="0" cellspacing="0" cellpadding="0">
      ...
      <tr>
        ...
        <td><div align="center">Director/ED</div></td>

        <td><div align="center">DMD/MD/Proprietor </div></td>
...
<input type="submit" name="delete_vouchar" id="delete_vouchar" class="btn btn-danger" value="Delete" style="background-color:red;color:white;" />
</div>
<div class="col-md-4"></div>
