...
<!--			<img src="<?=SERVER_ROOT?>public/uploads/logo/--><?//=$_SESSION['proj_id']?><!--.png" style="width:100px;" />		-->
            </td>
...
echo $_SESSION['proj_name'];
			...
			<td width="17%" align="right" class="qr-code">
              ...
              $print_url = "https://saaserp.ezzy-erp.com/app/views/acc_mod/files/purchase_sec_print_view.php?c=" . rawurlencode($company_id) . "&v=" . rawurlencode($req_no_qr_data);
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" bgcolor="#CCCCCC">
  <tr>
    <td width="42"><input name="button" type="button" onclick="hide();window.print();" value="Print" /></td>
	
    <?php /*?><td width="130" align="left"><a href="../../../views/warehouse_mod/po_receiving/chalan_view2.php?v_no=<?php echo rawurlencode(url_encode($jv->tr_no)); ?>"target="_blank">View GRN</a></td><?php */?>
	
	<td width="130" align="left"><a href="../../../views/warehouse_mod/po_receiving/chalan_view2.php?c=<?php rawurlencode(url_encode($c_id)); ?>&v=<?php rawurlencode(url_encode($jv->tr_no)); ?>"target="_blank">View GRN</a></td>
	
    <td width="169" align="right">Voucher Date :</td>
    <td width="250">

<input name="jv_no" type="hidden" value="<?=$jv_no?>" />
<input name="voucher_date" type="text" id="voucher_date" value="<?=$jv->jv_date;?>" />
<?php /*?><input type="button" name="Submit" value="EDIT VOUCHER"  onclick="DoNav('<?php echo '&v_no='.$jv_no.'&view=Show' ?>');" /><?php */?></td>
    <td width="102"><input name="check" type="submit" id="check" value="CHECK" />
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" bgcolor="#00CC00">
 ...
 <?php /*?>   <td width="99" align="left"><a href="../../../views/warehouse_mod/po_receiving/chalan_view2.php?v_no=<?php echo rawurlencode(url_encode($jv->tr_no)); ?>"target="_blank">View GRN</a></td><?php */?>
	
	
	<td width="99" align="left"><a href="../../../views/warehouse_mod/po_receiving/chalan_view2.php?c=<?php echo rawurlencode(url_encode($c_id)); ?>&v=<?php echo rawurlencode(url_encode($jv->tr_no)); ?>"target="_blank">View GRN</a></td>
	
	
	
    <td width="139" align="right">Voucher Date :</td>
    <td width="250">

<input name="jv_no" type="hidden" value="<?=$jv_no?>" />
<input name="voucher_date" type="text" id="voucher_date" value="<?=$jv->jv_date;?>" />
<!--<input type="button" name="Submit" value="EDIT VOUCHER"  onclick="DoNav('<?php echo '&v_no='.$jv_no.'&view=Show' ?>');" />--></td>
    <td width="127" style="text-align:right"><!--<input name="check" type="submit" id="check" value="RE-CHECK" />-->
        <input type="hidden" name="req_no" id="req_no" value="<?=$jv->jv_on?>" /></td>
  </tr>
</table>
...
<a target="_blank" href="chalan_view2.php?v_no=<?=$pr->po_no?>"></a></div><? }?>
  ...
  $ssql='select * from general_sub_ledger where 1 group by sub_ledger_id';
   ...
   $sql2="SELECT a.ledger_id,a.ledger_name,sum(dr_amt) as dr_amt, a.ledger_group_id, b.narration,b.sub_ledger FROM accounts_ledger a, secondary_journal b where b.jv_no='$jv_no' and a.ledger_id=b.ledger_id and jv_no=$jv_no and dr_amt>0 group by b.ledger_id , b.sub_ledger order by dr_amt desc";
$data2=db_query($sql2);
...
<?
  ...
  $sql2="SELECT a.ledger_id,a.ledger_name,sum(cr_amt) as cr_amt, a.ledger_group_id, b.narration,b.sub_ledger FROM accounts_ledger a, secondary_journal b where b.jv_no='$jv_no' and a.ledger_id=b.ledger_id and jv_no=$jv_no and cr_amt>0 group by b.ledger_id, b.sub_ledger order by cr_amt desc ";
$data2=db_query($sql2);
while($info=mysqli_fetch_object($data2)){	
