...
//var_dump($_SESSION['jv_no']['Payment_cash']);
...
if($_GET['jv_no']>0){
  $_SESSION['jv_no']['Payment_cash']=$_GET['jv_no'];
}

if($_GET['tr_no']>0){
  $_SESSION['tr_no']['Payment_cash']=$_GET['tr_no'];
}


 $jv_val=find_a_field('journal','count(id)',"tr_no='".$_SESSION['tr_no']['Payment_cash']."' and tr_from='Payment'");
 $sj_val=find_a_field('secondary_journal','count(id)',"tr_no='".$_SESSION['tr_no']['Payment_cash']."' and tr_from='Payment' and cr_amt>0");
 
   $vou_type=find_a_field('secondary_journal','type',"tr_no='".$_SESSION['tr_no']['Payment_cash']."' and tr_from='Payment' and dr_amt>0");
...
//    $jv_no = $_SESSION['jv_no']['Payment_cash'] = next_journal_sec_voucher_id($tr_from);
...
if($jv_val>0){
 $del_jv = "delete from journal where tr_from='Payment' and tr_no = '".$_SESSION['tr_no']['Payment_cash']."'";
 db_query($del_jv);
}

if($sj_val>0){
 $del_sj = "delete from secondary_journal where tr_from='Payment' and tr_no = '".$_SESSION['tr_no']['Payment_cash']."' and cr_amt>0";
 db_query($del_sj);
}
...
$relavent_head = $_POST['b_id'];
 ...
 where jv_no="'.$_SESSION['jv_no']['Payment_cash'].'" and tr_from="Payment"';
...
if($_SESSION['jv_no']['Payment_cash']==0){
 ...
 $jv_no = $_SESSION['jv_no']['Payment_cash'] = next_journal_sec_voucher_id($tr_from,'Payment');
...
if($sj_val>0){
 ...
 db_query($del_sj);
			...
			$sum_cr_amt=find_a_field('secondary_journal','sum(dr_amt)','jv_no="'.$_SESSION['jv_no']['Payment_cash'].'" and dr_amt>0');

			$detail = 'Paid To '.$j_data->received_from;
			
			$sub_led = find_all_field('general_sub_ledger', '', 'sub_ledger_id="' . $j_data->relavent_cash_head . '"');


			add_to_sec_journal($proj_id, $j_data->jv_no, $j_data->jv_date, $sub_led->ledger_id,  $detail, '0',$sum_cr_amt, $tr_from, $j_data->tr_no,$j_data->relavent_cash_head,'','0','',$j_data->user_id,'',$j_data->received_from,$j_data->bank,$j_data->cheq_no,$j_data->cheq_date,$j_data->relavent_cash_head,$checked,$j_data->type,$j_data->employee_id, $j_data->remarks , '0' );
			...
			$up = 'update secondary_journal set checked="NO" where jv_no="'.$_SESSION['jv_no']['Payment_cash'].'"';
...
//
//
//		$del_jv = "delete from secondary_journal where tr_from='Payment' and tr_no = '".$_SESSION['tr_no']['Payment_cash']."'";
//		db_query($del_jv);
//
//
//$_SESSION['jv_no']['Payment_cash']     = '';
...
if($_SESSION['jv_no']['Payment_cash']==0){
	...
	$jv_no = next_journal_sec_voucher_id($tr_from,'Payment');
...
var info = ledger_id+"<@>"+reference_id+"<@>"+cc_code+"<@>"+narration+"<@>"+dr_amt+"<@>"+flag+"<@>"+sub_ledger_id;
getData2('payment_update_ajax.php', 'divi_'+id,id,info);
}
...
<table width="98%" border="0" cellspacing="0" cellpadding="0" align="center" style="background: #FAE5E4 ;">
  <tr>
    <td><div>
...
?>
                        ...
                        <tr>
                          <td><div align="right"><span class="style3">*</span>Paid Through:</div></td>
...
<td colspan="3">
<select name="b_id" id="b_id" style="float:left" tabindex="2"  >
<option value="<?php echo $jv->relavent_cash_head;?>"><?php echo find_a_field('general_sub_ledger','sub_ledger_name','sub_ledger_id="'.$jv->relavent_cash_head.'"');?></option>
<?
...
?>
                        ...
                        <tr id="of_bank_check">
                          <td align="left"><div align="right">Drawee:</div></td>
...
<td align="right"><input name="ms_update" class="btn" type="submit" id="ms_update" value="Data Update"
 style="width: 120px; text-align:center; padding: 10px 10px;   background:#6699FF; color:#FFFFFF;"   onclick="this.form.submit()"/></td>
                      ...
                      <td align="right" valign="top"><div class="box">
                        <table  class="table table-striped table-bordered" border="0" cellspacing="0" cellpadding="0">
                          <tr>
                            ...
                            <th bgcolor="#45777B"><span class="style3">&nbsp;</span></th>
                          </tr>
                          <? 
   ...
   $sql2="select a.tr_no, a.cr_amt, a.narration,a.jv_date , a.jv_no,a.checked

from  secondary_journal a where a.tr_from='Payment' and  SUBSTRING(a.tr_no,5,1)='0' and  a.cr_amt>0 and a.jv_no='".$_SESSION['jv_no']['Payment_cash']."' group by a.tr_no order by a.tr_no desc limit 5";
...
from  secondary_journal a, accounts_ledger l where a.ledger_id=l.ledger_id and a.jv_no='".$_SESSION['jv_no']['Payment_cash']."' and a.dr_amt>0 and tr_from='Payment'";
...
<td><input  type="text"  id="dr_amt_<?=$data->id?>"  name="dr_amt_<?=$data->id?>"  size="5" value="<?=$data->dr_amt?>" style="width:100px;" tabindex="11"/></td>
...
<? } ?>
            ...
            <form method="post">
              <table width="100%" border="0" align="right" cellpadding="2" cellspacing="2">
                <tr>
                  <td width="19%" style="text-align:right;"><input name="receipt_varify" class="btn" type="button" id="receipt_varify" value="Payment Verified" onclick="this.form.submit()" />
