...
<div class="row bg-form-titel">
        ...
        <div class="row">
            ...
            <div class="col-sm-6 col-md-6 col-lg-6 col-xl-6 pt-1">
                <div class="form-group row m-0">
                    ...
                    <div class="col-sm-8 col-md-8 col-lg-8 col-xl-8 p-0">
                        <select name="reconsilation_status" id="reconsilation_status" class="form-control">

						  <option></option>
	
						  <option <?=($_POST['reconsilation_status']=='No')?'Selected':'';?>>No</option>
	
						  <option <?=($_POST['reconsilation_status']=='Yes')?'Selected':'';?>>Yes</option>
	...
	<div class="container-fluid pt-5 p-0 ">
	  ...
	  $i=0;
		...
		if($_POST['reconsilation_status']!='') $checked_con = ' and j.reconsilation_status="'.$_POST['reconsilation_status'].'" ';
		...
		if($_POST['depot_id']>0) $depot_con = ' and w.warehouse_id="'.$_POST['depot_id'].'" ';
		     ...
		     $sql=" select DISTINCT j.tr_no, j.jv_no, j.jv_date, sum(j.dr_amt) as dr_amt, sum(j.cr_amt) as cr_amt, j.tr_from, u.fname, j.entry_at, j.reconsilation_status, j.secondary_approval
				

				FROM

				 journal j, accounts_ledger l, user_activity_management u 
				  
			


				WHERE
j.tr_from in('Payment','Receipt') and
...
$received = $received + $data[1];
	  ...
	  <td><?=find_a_field('secondary_journal','cheq_no','jv_no="'.$data[1].'"');?></td>
	  
	  <td><?=find_a_field('secondary_journal','cheq_date','jv_no="'.$data[1].'"');?></td>
      ...
      <!--<td align="center"><a target="_blank" href="recon_approval_view.php?jv_no=<?=rawurlencode(url_encode($data[1]))?>&tr_from=<?=$data[5]?>"><img src="../images/print.png" width="20" height="20" /></a></td>-->
	  
	  <td align="center"><a target="_blank" href="recon_approval_view.php?c=<?=rawurlencode(url_encode($c_id))?>&v=<?=rawurlencode(url_encode($data[1]))?>&tr_from=<?=rawurlencode(url_encode($data[5]))?>"><img src="../images/print.png" width="20" height="20" /></a></td>

      <td align="center"><span id="divi_<?=$data[1]?>">

            <? 

						  if(($data[8]=='Yes' || $data[8]=='YES' )){
			
			?>
			
			<input type="button" name="Button" value="YES" onclick="window.open('recon_approval_view.php?c=<?=rawurlencode(url_encode($c_id))?>&v=<?=rawurlencode(url_encode($data[1]))?>&tr_from=<?=rawurlencode(url_encode($data[5]))?>');" class="btn1 btn1-bg-submit"/>
			
			<?
			
			}elseif(($data[8]=='No' || $data[8]=='NO' || $data[8]=='' )){
			
			?>
			
			<input type="button" name="Button" value="NO"  onclick="window.open('recon_approval_view.php?c=<?=rawurlencode(url_encode($c_id))?>&v=<?=rawurlencode(url_encode($data[1]))?>&tr_from=<?=rawurlencode(url_encode($data[5]))?>');" class="btn1 btn1-bg-cancel" />
