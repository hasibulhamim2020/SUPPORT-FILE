...
a.status
	 ...
	 from sale_do_master a,dealer_info d,area b where 
	  ...
	  a.status in ('CHECKED','COMPLETED') and b.AREA_CODE=d.area_code and a.dealer_code=d.dealer_code and d.dealer_type='Distributor' ".$con." ";
	  ...
	  <?=$data[11]?></td>
      <td align="center"><?=find_a_field('accounts_ledger','ledger_name','ledger_id='.$data[9]);?></td>
      <td align="center"><?=$data[13];?>&nbsp;</td>
      <td align="right"><? echo number_format($data[5],2); $rcv_total = $rcv_total + $data[5];?></td>
      <td align="right"><? echo number_format($data[6],2);?></td>
      <td align="right"><? if(($data[15]=='CHECKED')||($data[15]=='COMPLETED')) echo $sales_amt = find_a_field('sale_do_details','sum(total_amt)','total_amt>0 and do_no='.$data[0]);?></td>
      <td align="right"><? if(($data[15]=='CHECKED')||($data[15]=='COMPLETED')) echo $do_amt = find_a_field('sale_do_details','sum(total_amt)','do_no='.$data[0]);?></td>
      <td align="center"><? if((($data[15]=='CHECKED')||($data[15]=='COMPLETED'))&&(($data[6]>0))) echo $diff_amt = (int)($do_amt-$data[6]);?></td>
      <td align="center"><? if((($data[15]=='CHECKED')||($data[15]=='COMPLETED'))&&($data[6]>0)) echo 'OK'; elseif(($data[15]=='CHECKED')||($data[15]=='COMPLETED')) echo 'Passed'; elseif(($data[6]==0)&&($do_amt==0)) echo 'Pending'; else echo 'HOLD'; ?></td>
      </tr>
	  <?  $do = $do + $do_amt;$sales = $sales + $sales_amt;}}?>
	        <tr class="alt">
        ...
        <td align="center">
              <div align="right">
                <?=number_format($do,2)?>
                  </div></td><td align="center"><div align="right"></div></td>
