...
//create_combobox('sub_group_id');
 ...
 $group_data = find_all_field('ledger_group','','group_id='.$group_id); 
...
$page="accounts_general_ledger_tree.php";		// PHP File Name
...
<thead>
<tr>
  <th style="background-color:#45777B;"><span class="style3">GL Code</span></th>
...
$con .='and a.warehouse_name like "%'.$_POST['warehouse_name'].'%" ';
 ...
 $td='select a.'.$unique.',  a.'.$shown.', a.acc_class, a.acc_sub_class,  a.ledger_group_id from '.$table.' a where a.ledger_group_id="'.$group_id.'"  '.$con.' order by a.ledger_id  ';

$report=db_query($td);

while($rp=mysqli_fetch_row($report)){$i++; if($i%2==0)$cls=' class="alt"'; else $cls='';?>

<tr<?=$cls?> <?php /*?>onclick="DoNav('<?php echo $rp[0];?>');"<?php */?>>
  <td><?=$rp[0];?></td>
...
document.getElementById('acc_class').value);">
                      						...
                      						<? foreign_relation('acc_class','id','CONCAT(id, ": ", class_name)',$acc_class,'id="'.$group_data->acc_class.'"');?>
                    					</select></td>
                                      </tr>
									  ...
									  <tr>
										...
										<select name="acc_sub_class" required id="acc_sub_class"  tabindex="2" style="width:220px;">
                      						...
                      						<? foreign_relation('acc_sub_class','id','CONCAT(id, ": ", sub_class_name)',$acc_sub_class,'id="'.$group_data->acc_sub_class.'"');?>
                    					</select></span></td>
                                      </tr>
									  ...
									  <tr>
										...
										<select name="ledger_group_id" required id="ledger_group_id"  tabindex="2" style="width:220px;">
                      						...
                      						<? foreign_relation('ledger_group','group_id','CONCAT(group_id, ": ", group_name)',$ledger_group_id,'group_id="'.$group_data->group_id.'"');?>
                    					</select></span></td>
                                      </tr>
