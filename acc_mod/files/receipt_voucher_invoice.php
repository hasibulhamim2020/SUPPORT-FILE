...
$title='Invoice & Bill Receipt Voucher';
...
$_SESSION['new_jv'] = $_POST['jv_no'];
$_SESSION['receipt_no'] = find_a_field('secondary_journal','tr_no','jv_no="'.$_SESSION['new_jv'].'"');
}
...

{
...

	{
		...
		if($_SESSION['receipt_no']==''){
         $_SESSION['receipt_no']=next_invoice('tr_no','secondary_journal');
		 }



		$manual_receipt_no = next_invoice('tr_no','secondary_journal');
...
{   
         ...
         if($_SESSION['new_jv']==''){
         $_SESSION['new_jv']=next_journal_sec_voucher_id();
		  }
  ...
  //$ledger2=str_replace($search,'',$ledgers[1]);
     ...
     $ledger_id = $ledger1;
			...
			$journal="INSERT INTO `secondary_journal` (
								...
								group_for,`bank`,`cheq_no`,`cheq_date`,
...
VALUES ('$proj_id', '".$_SESSION['new_jv']."', '$date', '$ledger_id', '$dnarr', '$amount', '0', 'Receipt', '".$_SESSION['receipt_no']."','$r_from','$tr_id','$sub_ledger','$cc_code','".$user_id."','".$_SESSION['user']['group']."','$bank','$c_no','$c_date','$receive_ledger','UNFINISHED')";
...
if(isset($_POST['limmit'])){
	  ...
	  if($_SESSION['new_jv']>0){
	  $j_data = find_all_field('secondary_journal','','jv_no="'.$_SESSION['new_jv'].'"');
	    $detail = 'Received from '.$j_data->received_from;
	    
		 $cr_journal="INSERT INTO `secondary_journal` (
			...
			VALUES ('$proj_id', '".$_SESSION['new_jv']."', '".$j_data->jv_date."', '".$j_data->relavent_cash_head."', '$detail', '0','".$_POST['t_amount']."', 'Receipt', '".$_SESSION['receipt_no']."','".$j_data->received_from."','', '".$j_data->cc_code."', '".$_SESSION['user']['id']."','".$_SESSION['user']['group']."','".$j_data->bank."','".$j_data->cheq_no."','".$j_data->cheq_date."','".$j_data->relavent_cash_head."','NO')";
			
			db_query($cr_journal);
			
			$up = 'update secondary_journal set checked="NO" where jv_no="'.$_SESSION['new_jv'].'"';
			db_query($up);
...
$fetch = find_all_field('secondary_journal','','jv_no="'.$_SESSION['new_jv'].'"');
...

	{
		...
		if( voucher_mode == 0 )
...

			{
				...
				document.getElementById('receipt_no').value		= "<?php echo $_SESSION['receipt_no'];?>";
...
<? if($fetch->tr_no>0){?>
<input name="receipt_no" type="text" id="receipt_no" size="15" value="<?=$fetch->tr_no?>" />
<? } else{ ?>
...
<input type="hidden" name="voucher_mode" class="radio" id="voucher_mode_auto" value="1" <?php if($old_voucher_mode==1){ ?>checked="checked"<? } ?> onclick="voucher_no(this.value)"/>
                      ...
                      <tr>
                        ...
                        <td colspan="3"><input name="r_from" type="text" id="r_from" value="<?php echo $fetch->received_from?>" class="input1"  tabindex="1" required/></td>
                      </tr>
...
$data2 = substr($data2, 0, -1);
                      ...
                      <tr>
                        ...
                        <td colspan="3"><input name="c_no" type="text" id="c_no" value="<?php echo $fetch->cheq_no?>" size="20" maxlength="25" tabindex="4"/></td>
                      </tr>
                      ...
                      <tr><td><div align="right"><span>Cheque</span> Date:</div></td>
                        ...
                        <td colspan="3"><? if($fetch->tr_no>0){?><input name="c_date" type="text" id="c_date" value="<?php echo date('Y-m-d',strtotime($fetch->cheq_date))?>" size="12" maxlength="15" tabindex="5"/><? } else{?><input name="c_date" type="text" id="c_date" value="" size="12" maxlength="15" tabindex="5"/> <? } ?></td>
                      </tr>
                      ...
                      <tr>
                        ...
                        <td colspan="3" align="left"><input name="bank" type="text" id="bank" value="<?php echo $fetch->bank?>" class="input1"   tabindex="6"/></td>
                      </tr>
...
from  secondary_journal a, accounts_ledger l where a.ledger_id=l.ledger_id and a.jv_no='".$_SESSION['new_jv']."' and tr_from='Receipt'";
       $qr = db_query($sql2);
