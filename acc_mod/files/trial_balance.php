...
$grp_con = " and  c.group_id='".$_REQUEST['group_id']."'";
	...
	if($cc_code > 0)
	{
		$g="select c.group_name,SUM(dr_amt),SUM(cr_amt),c.group_id FROM accounts_ledger a, journal b,ledger_group c where a.ledger_id=b.ledger_id and a.ledger_group_id=c.group_id and b.jv_date BETWEEN '$fdate' AND '$tdate' and c.group_for=".$_SESSION['user']['group']." ".$grp_con."  AND b.cc_code=$cc_code group by  c.group_id order by c.u_id asc";
		
	}
	else
	{
		$g="select c.group_name,SUM(dr_amt),SUM(cr_amt),c.group_id FROM accounts_ledger a, journal b,ledger_group c where a.ledger_id=b.ledger_id and a.ledger_group_id=c.group_id and b.jv_date BETWEEN '$fdate' AND '$tdate' ".$grp_con." and c.group_for=".$_SESSION['user']['group']."  group by  c.group_id order by c.u_id asc";
...
<tr <? $i++; if($i%2==0)$cls=' class="alt"'; else $cls=''; echo $cls;?>>
    ...
    <td align="right"><?=($tagc=='(Dr)')? number_format($closing,2) : 0.00;?></td>
    <td align="right"><?=($tagc=='(Cr)')? number_format($closing,2) : 0.00;?></td>
    </tr>
  <?php
