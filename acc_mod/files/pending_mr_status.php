...
$con .= 'and a.req_date between "'.$_POST['fdate'].'" and "'.$_POST['tdate'].'"';
 ...
 echo $res='select  	s.jv_no, s.jv_no, DATE_FORMAT(s.jv_date, "%d-%m-%Y") as jv_date,  a.ledger_name as bank_name, sum(cr_amt) as Amount,   u.fname as entry_by,  s.entry_at from secondary_journal s, accounts_ledger a, user_activity_management u where 
s.ledger_id=a.ledger_id and s.entry_by=u.user_id and s.dr_amt<1 '.$con.' and s.checked in ("NO") group by s.jv_no order by s.jv_no desc';
