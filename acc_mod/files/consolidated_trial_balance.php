									...
									<table width="100%" border="0" cellspacing="2" cellpadding="0">
                                      <tr>
                                        <td width="22%" align="right">
		   								 Period :</td>
                                        <td width="27%" align="left"><input name="fdate" type="text" id="fdate" size="12" autocomplete="off" style="max-width:250px;" value="<?php echo $_REQUEST['fdate'];?>" /> </td>
                                        
                                            <td width="5%" align="left">-----------</td>
                                            <td width="46%" align="left"><input name="tdate" type="text" id="tdate" size="12" autocomplete="off" style="max-width:250px;" value="<?php echo $_REQUEST['tdate'];?>"/></td>
                                      </tr>
									  
                                       <tr>
                                         <td height="50" align="right">Ledger Group</td>
                                         <td colspan="2" align="left"><select name="group_id" id="group_id">
                                           <option value="%" >All</option>
                                           <? foreign_relation('ledger_group','group_id','group_name',$_REQUEST['group_id'],"group_for=".$_SESSION['user']['group']);?>
                                         </select></td>
                                       </tr>
                                       <tr>
                                         <td height="50" align="right"> Cost Center: </td>
                                         <td colspan="2" align="left">
										 <select name="cc_code" id="cc_code">
										    <option value=""></option>
											<? foreign_relation('cost_center cc, cost_category c','cc.id','cc.center_name',$_REQUEST['cc_code'],"cc.category_id=c.id and c.group_for='".$_SESSION['user']['group']."' ORDER BY id ASC");?>
										 </select>
										 </td>
                                       </tr>
                                       <?php /*?><tr>
...
//and  c.group_id in (1016,1024,1032,1051,1070,1086,1092,1109,2032,2063,2067,2065,2069, 3027,1108) change by kamrul at 23-01-2021
...
{ $tagc='(Cr)'; $tclosing_total=$tclosing_total*(-1);}
  ...
  $tclosing_total = 0;
   ...
   $g="select c.group_name,c.group_id FROM ledger_group c where 1 ".$grp_con." and  c.group_id in (1093,1095,1098,1103,1104,1105,1110) and c.group_for=".$_SESSION['user']['group']."  group by  c.group_id";
  $gsql=db_query($g);
  while($g=mysqli_fetch_row($gsql))
  {
...
<tr <? $i++; if($i%2==0)$cls=' class="alt"'; else $cls=''; echo $cls;?>>
    <td align="center"><?=++$pi;?></td>
    <td align="left"><a href="transaction_listledger.php?show=show&fdate=<?=$_REQUEST['fdate']?>&tdate=<?=$_REQUEST['tdate']?>&ledger_id=<?=$p[2]?>" target="_blank"><?php echo $p[1];?></a></td>
    <td align="right"><?php if($tagc=='(Dr)') echo number_format($opening,2);?></td>
    <td align="right"><?php if($tagc=='(Cr)') echo number_format($opening,2);?></td>
    </tr>
  <?php
  if($tagc=='(Dr)') $total_dr=$total_dr+$opening; 
  if($tagc=='(Cr)') $total_cr=$total_cr+$opening;
  if($tagc=='(Dr)') $t_dr=$t_dr+$opening; 
  if($tagc=='(Cr)') $t_cr=$t_cr+$opening;
 // $t_cr=$t_cr+$cr;
...
{ $tagc='(Cr)'; $tclosing_total=$tclosing_total*(-1);}
  ...
  $tclosing_total = 0;
   ...
   $g="select c.group_name,c.group_id FROM ledger_group c where 1 ".$grp_con." and  c.group_id in (1090,1078,3001,3024,4029,4064,4065,4066,1061,1112,
   ...
   ) and c.group_for=".$_SESSION['user']['group']."  group by  c.group_id";
  $gsql=db_query($g);
  while($g=mysqli_fetch_row($gsql))
  {
...
{ $tagc='(Cr)'; $totalclosing_total=$totalclosing_total*(-1);}
  ...
  <tr>
    <th colspan="4" align="left">Balance Transferred from Balance Sheet Amount</th>
    </tr>

<? if (($total_cr-$total_dr)>0)  $balancesheet_amount_dr = ($total_cr-$total_dr) ; else $balancesheet_amount_cr = ($total_cr-$total_dr)* (-1); ?>
<tr <? $i++; if($i%2==0)$cls=' class="alt"'; else $cls=''; echo $cls;?>>
    <td align="center">1</td>
    <td align="left"><a href="#" >
Balance Transferred from Balance Sheet Amount</a></td>
    <td align="right"><?php echo number_format($balancesheet_amount_dr,2);?></td>
     <td align="right"><?php echo number_format($balancesheet_amount_cr,2);?></td>
    </tr>

  <tr style="background-color:#99CCFF;">
    <td colspan="2" align="center"><span class="style1">Total</span></td>
    <td align="right"><span class="style1">
      <?=number_format(($total_dr + $balancesheet_amount_dr),2);?>
    </span></td>
    <td align="right"><span class="style1">
      <?=number_format(($total_cr + $balancesheet_amount_cr),2);?>
    </span></td>
    </tr>
  ...
  <tr>
    <th colspan="2" align="right"><span class="style1">Total Balance :<?php /*?> <?php echo number_format((($total_cr)-($total_dr)),2);?><?php */?></span></th>
    <th align="right"><span class="style1"><?php echo number_format($total_dr+$balancesheet_amount_dr,2);?></span></th>
    <th align="right"><span class="style1"><?php echo number_format($total_cr+$balancesheet_amount_cr,2)?></span></th>
    </tr>
