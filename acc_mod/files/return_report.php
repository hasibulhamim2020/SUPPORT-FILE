...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td><div class="left_report">
		    ...
		    Period :                                       </td>
							...
							<th width="11%" align="center">Return</th>
  ...
  if(isset($_REQUEST['show'])){
    ...
    $q2= "SELECT DISTINCT ii.item,ii.customer FROM return_invoice ii, customer c,journal j where c.customer_type like '%$cus_type%' and c.customer_id like '%$cus_id%' and c.customer_id=ii.customer and j.jv_date>'$fdate' and j.jv_date<'$tdate'";
    $pi=0;
    $sql=db_query($q2);
...
FROM return_invoice ii, customer c, item_info it, journal j WHERE ii.item ='.$d[0].' AND ii.customer='.$d[1].' AND ii.p_inv_id = j.jv_no AND ii.customer = c.customer_id AND ii.item = it.item_id');
  ...
  <?php } ?>
  <tr>
    <th colspan="7" align="right">Total : <strong><?php echo $val;?></strong>&nbsp;&nbsp;</th>
    </tr>
  <?php }}
        else {
		
    $q2= 'SELECT item FROM return_invoice GROUP BY item';
  //echo $p;
    $pi=0;
    $sql=db_query($q2);
    ...
    $p=db_query('SELECT DISTINCT j.jv_date, c.customer_name, it.item_name, c.customer_type, ii.qty
	FROM return_invoice ii, customer c, item_info it, journal j
	WHERE ii.item ='.$d[0].'
	AND ii.p_inv_id = j.jv_no
