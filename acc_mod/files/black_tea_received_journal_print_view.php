...

{
	...
	sec_journal_journal($jv_no,$jv,'Sales');
...
$ch = find_all_field('sale_transfer_details','','st_no='.$jv->tr_no);
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" style="background-color:#CCCCCC;">
...
<a target="_blank" href="../../sales_mod/pages/report/invoice_view_acc_new.php?v_no=<?=$ch->chalan_no?>"></a></div>

<div align="left">

<form action="" method="post">

<table width="100%" border="0" cellpadding="0" cellspacing="0" style="background-color:#339900;">
...
<a target="_blank" href="../../warehouse_mod/pages/pr/chalan_view2.php?v_no=<?=$ch->pr_no?>"></a></div>
      ...
      <tr>
        ...
        <td class="tabledesign_text">Checked By :
          ...
          <? echo find_a_field('user_activity_management','fname','user_id='.$jv->user_id); ?></td>
  ...
  <tr>
    ...
    <td><? if($cc_code>0){?>CC CODE/PROJECT NAME: <? echo find_a_field('cost_center','center_name',"id='$cc_code'")?><? }?></td>
...
$sql2="SELECT a.ledger_id,a.ledger_name,cr_amt,dr_amt,b.narration FROM accounts_ledger a, journal b where b.jv_no='$jv_no' and a.ledger_id=b.ledger_id";

$data2=db_query($sql2);

while($info=mysqli_fetch_object($data2)){

if($info->dr_amt>0 || $info->cr_amt>0 ){		  
      ...
      <tr>
        ...
        <td align="left"><?=$info->ledger_id?>-<?=$info->ledger_name?></td>
