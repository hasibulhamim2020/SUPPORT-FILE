...

{
 ...
 $ssql='update secondary_journal set jv_date="'.$voucher_date.'",  checked_at="'.$time_now.'", checked_by="'.$_SESSION['user']['id'].'", checked="YES" where jv_no="'.$jv_no.'"';
	...
	sec_journal_journal($jv_no,$jv_no,'Sales');
	sec_journal_journal($_POST['cogs_jv_no'],$_POST['cogs_jv_no'],'COGS');
...
$cogs_jv_no_cus = find_a_field('secondary_journal','jv_no','tr_no="'.$jv->tr_no.'" and tr_from="COGS"');
...
echo $_SESSION['proj_name'];
			...
			<td width="17%" align="right" class="qr-code">
              ...
              $print_url = "https://saaserp.ezzy-erp.com/app/views/acc_mod/files/sales_sec_print_view.php?c=" . rawurlencode($company_id) . "&v=" . rawurlencode($req_no_qr_data);
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" bgcolor="#CCCCCC">
  ...
  <?php /*?>  <td width="180" align="right"><a target="_blank" href="../../../views/warehouse_mod/wo/sales_invoice_print_view_acc.php?v_no=<?=rawurlencode(url_encode($ch->chalan_no))?>">CH-
	...
	<td width="180" align="right"><a target="_blank" href="../../../views/warehouse_mod/wo/sales_invoice_print_view_acc.php?c=<?=rawurlencode(url_encode($c_id))?>&v=<?=rawurlencode(url_encode($ch->chalan_no))?>">CH-
...
<input name="cogs_jv_no" type="hidden" value="<?=$cogs_jv_no_cus?>" />
<input name="voucher_date" type="text" id="voucher_date" value="<?php echo $jv->jv_date;?>" />    </td>

    <td><? if($jv->checked==''){?><input name="check" type="submit" id="check" value="CHECK" style="font-size:12px; color:#990000" /><? }?>
...
<a target="_blank" href="../../../views/warehouse_mod/wo/sales_invoice_print_view_acc.php?v_no=<?=rawurlencode(url_encode($ch->chalan_no))?>"></a></div><? }else{?>
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" bgcolor="#339900">
  ...
  <tr>
   ...
   <?php /*?> <td width="200" align="right"><a target="_blank" href="../../../views/warehouse_mod/wo/sales_invoice_print_view_acc.php?v_no=<?=rawurlencode(url_encode($ch->chalan_no))?>">CH-

        <?=$ch->chalan_no?>
    </a></td><?php */?>
	
	<td width="200" align="right"><a target="_blank" href="../../../views/warehouse_mod/wo/sales_invoice_print_view_acc.php?c=<?=rawurlencode(url_encode($c_id))?>&v=<?=rawurlencode(url_encode($ch->chalan_no))?>">CH-
...
<a target="_blank" href="../../warehouse_mod/pages/pr/chalan_view2.php?v_no=<?=$ch->pr_no?>"></a></div><? }?>
      ...
      <tr>
        ...
        <td class="tabledesign_text">
          ...
          <?=date('d-m-Y',strtotime($ch->chalan_date));  ?></td>
  ...
  $sql2="SELECT a.ledger_id,a.ledger_name,cr_amt,dr_amt,b.narration,b.sub_ledger FROM accounts_ledger a, secondary_journal b where b.jv_no='$jv_no' and a.ledger_id=b.ledger_id and b.tr_from='Sales'";
...
$cogs_jv_no = find_a_field('secondary_journal','jv_no','tr_no="'.$jv->tr_no.'" and tr_from="COGS"');
 ...
 $sql2="SELECT a.ledger_id,a.ledger_name,cr_amt,dr_amt,b.narration,b.sub_ledger FROM accounts_ledger a, secondary_journal b where b.jv_no='$cogs_jv_no' and a.ledger_id=b.ledger_id and b.tr_from='COGS'";
$data2=db_query($sql2);
