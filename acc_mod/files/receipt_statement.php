...
$title='Receipt Statement(Detail)';
...
<?php
 ...
 $led=db_query("select ledger_id,ledger_name from accounts_ledger where group_for=".$_SESSION['user']['group']." order by ledger_name");
      $data = '[';
      $data .= '{ name: "All", id: "%" },';
...
<?php
	if(isset($_REQUEST['show']))
    {
		...
		if($cc_code > 0)
		{
			$total = mysqli_fetch_row(db_query("select sum(dr_amt),sum(cr_amt) from journal where tr_from='Receipt' and jv_date>'$fdate' and jv_date<'$tdate' and ledger_id like '$ledger_id' and group_for=".$_SESSION['user']['group']." AND cc_code=$cc_code"));
			$p = "select a.jv_date,b.ledger_name,a.dr_amt,a.cr_amt,a.tr_no,a.narration from journal a,accounts_ledger b where tr_from='Receipt' and a.ledger_id=b.ledger_id and a.jv_date>'$fdate' and a.jv_date<'$tdate' and a.ledger_id like '$ledger_id' and 1 AND a.cc_code=$cc_code order by a.tr_no and b.group_for=".$_SESSION['user']['group']."";
			
		}
		else
		{
			$total = mysqli_fetch_row(db_query("select sum(dr_amt),sum(cr_amt) from journal where tr_from='Receipt' and jv_date>'$fdate' and jv_date<'$tdate' and ledger_id like '$ledger_id' and group_for=".$_SESSION['user']['group'].""));
			$p = "select a.jv_date,b.ledger_name,a.dr_amt,a.cr_amt,a.tr_no,a.narration from journal a,accounts_ledger b where tr_from='Receipt' and a.ledger_id=b.ledger_id and a.jv_date>'$fdate' and a.jv_date<'$tdate' and a.ledger_id like '$ledger_id' and b.group_for=".$_SESSION['user']['group']." order by a.tr_no";
