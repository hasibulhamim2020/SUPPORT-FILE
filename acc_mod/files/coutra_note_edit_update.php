...
//var_dump($_SESSION['jv_no']['Contra']);
...
if($_GET['jv_no']>0){
  $_SESSION['jv_no']['Contra']=$_GET['jv_no'];
}

if($_GET['tr_no']>0){
  $_SESSION['tr_no']['Contra']=$_GET['tr_no'];
}


//$jv_val=find_a_field('journal','count(id)',"tr_no='".$_SESSION['tr_no']['Contra']."' and tr_from='Contra'");

 $jv_val=find_a_field('journal','count(id)',"tr_no='".$_SESSION['tr_no']['Contra']."' and tr_from='Contra'");
$sj_val=find_a_field('secondary_journal','count(id)',"tr_no='".$_SESSION['tr_no']['Contra']."' and tr_from='Contra' ");
 
  $vou_type=find_a_field('secondary_journal','type',"tr_no='".$_SESSION['tr_no']['Contra']."' and tr_from='Contra'");
...
//    $jv_no = $_SESSION['jv_no']['Contra'] = next_journal_sec_voucher_id($tr_from);
...
if($jv_val>0){
 $del_jv = "delete from journal where tr_from='Contra' and tr_no = '".$_SESSION['tr_no']['Contra']."'";
 db_query($del_jv);
}

if($sj_val>0){
   $del_sj = "delete from secondary_journal where tr_from='Contra' and tr_no = '".$_SESSION['tr_no']['Contra']."' ";
 db_query($del_sj);
}
 ...
 where jv_no="'.$_SESSION['jv_no']['Contra'].'" and tr_from="Contra"';
...
if($_SESSION['jv_no']['Contra']==0){
 ...
 $jv_no = $_SESSION['jv_no']['Contra'] = next_journal_sec_voucher_id($tr_from,'Contra');
...
if($sj_val>0){
 ...
 db_query($del_sj); 
			...
			//add_to_sec_journal($proj_id, $_SESSION['jv_no']['Contra'], $j_data->jv_date, $j_data->relavent_cash_head,  $detail, '0',$_POST['t_amount'], $tr_from, $_SESSION['tr_no']['Contra'],'','',$j_data->cc_code,'',$_SESSION['user']['id'],'',$j_data->received_from,$j_data->bank,$j_data->cheq_no,$j_data->cheq_date,$j_data->relavent_cash_head,$checked);
...
if($_SESSION['jv_no']['Contra']==0){
	...
	$jv_no = next_journal_sec_voucher_id($tr_from,'Contra');
...
and a.ledger_group_id in (10201,10202,231001)','ledger_id');
...
getData2('contra_update_ajax.php', 'divi_'+id,id,info);
}
...
from  secondary_journal a where a.tr_from='Contra'  and SUBSTRING(a.tr_no,5,1)='0' and  a.dr_amt>0 and a.checked in ('NO','YES') 
and a.jv_no='".$_SESSION['jv_no']['Contra']."'group by a.tr_no order by a.tr_no desc limit 5";
...
<? auto_complete_from_db('accounts_ledger a, ledger_group g','concat(a.ledger_id,"::",a.ledger_name," >>> ",g.group_name)','a.ledger_id','a.ledger_group_id=g.group_id
and a.ledger_group_id  in (112001,113001,231001)','ledger_id_'.$data->id);?>
...
<td><?php /*?><a onclick="if(!confirm('Are You Sure Execute this?')){return false;}" href="?del=<?=$data->id?>">&nbsp;<img src="del.png" width="25" height="auto" />&nbsp;</a><?php */?>
...
<table width="100%" border="0" align="right" cellpadding="2" cellspacing="2">
	...
	?>  
			  <input name="receipt_varify" class="btn" type="button" id="receipt_varify" <? if($deffr==0){ echo 'hi';}else{?>style="visibility:hidden"<? }?> value="Contra Verified" onclick="this.form.submit()" /> <input name="limmit" type="hidden" value="" />  
			             </td>
