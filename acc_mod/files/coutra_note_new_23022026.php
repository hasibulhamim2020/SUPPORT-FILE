...
if($_GET['mhafuz']>0){
	...
	unset($_SESSION['jv_no']['Contra']);

	unset($_SESSION['tr_no']['Contra']);

}

if($_GET['jv_no']['Contra']>0){
  $_SESSION['jv_no']['Contra']=$_GET['jv_no']['Contra'];
}

if($_GET['tr_no']['Contra']>0){
  $_SESSION['tr_no']['Contra']=$_GET['tr_no']['Contra'];
}
...
if($_SESSION['jv_no']['Contra']==0){

 $jv_no = $_SESSION['jv_no']['Contra'] = next_journal_sec_voucher_id($tr_from,'Contra',$_SESSION['user']['group'],$_SESSION['user']['id']);

 $tr_no = $_SESSION['tr_no']['Contra'] = next_tr_no($tr_from);
...
$jv_no = $_SESSION['jv_no']['Contra'];

$tr_no = $_SESSION['tr_no']['Contra'];

	}
	$folder = 'ContraVoucher';
	$field = 'file_upload';
	$file_name = $field . '-' . $_SESSION['jv_no']['Contra'];
...
add_to_sec_journal($proj_id, $jv_no, $jv_date, $ledger_id, $dnarr, $amount, '0', $tr_from, $tr_no,$sub_ledger,'',$cc_code,'',$user_id,'',$r_from, $bank,$c_no,$cheq_date,$$receive_ledger,$checked,$type,$employee,$remarks,$uploaded_file ,$reference_id);
 ...
 add_to_sec_journal($proj_id, $jv_no, $jv_date, $ledger_id, $dnarr, '0', $amount, $tr_from, $tr_no,$sub_ledger,'',$cc_code,'',$user_id,'',$r_from, $bank,$c_no,$cheq_date,$$receive_ledger,$checked,$type,$employee,$remarks,$uploaded_file ,$reference_id);
	...
	$up = 'update secondary_journal set  manual_voucher_no="'.$manual_voucher_no.'" where jv_no="'.$_SESSION['jv_no']['Contra'].'"';
	db_query($up);
}
...
if(isset($_POST['limmit']) && $_SESSION['csrf_token']===$_POST['csrf_token']){
    ...
    $_SESSION['csrf_token'] = bin2hex(random_bytes(32));
	  ...
	  if($_SESSION['tr_no']['Contra']>0){
if($_SESSION['jv_no']['Contra']>0){
		  

			$j_data = find_all_field('secondary_journal','','jv_no="'.$_SESSION['jv_no']['Contra'].'"');
			...
			$up = 'update secondary_journal set checked="NO" where jv_no="'.$_SESSION['jv_no']['Contra'].'"';
			...
			$_SESSION['jv_no']['Contra']= '';

			$_SESSION['receipt_no']['Contra'] = '';
...
if($_GET['del']>0)
{
		...
		//$crud->delete_all($condition);
		 $del_jv = "delete from secondary_journal where tr_from='Contra' and id = '".$_GET['del']."'";
		db_query($del_jv);
...
$jv = find_all_field('secondary_journal','','jv_no="'.$_SESSION['jv_no']['Contra'].'"');

if($_SESSION['jv_no']['Contra']==0){

	

	$jv_no = next_journal_sec_voucher_id($tr_from,'Contra',$_SESSION['user']['group'],$_SESSION['user']['id']);
...
js_ledger_subledger_autocomplete_new('contra',$proj_id,$voucher_type,$_SESSION['user']['group']); 
...
//auto_complete_from_db('accounts_ledger',"concat(ledger_id,'::',ledger_name)","concat(ledger_id)",'1','ledger_id');
?>
					...
					<div class="container n-form2">
							...
							<div class="col-sm-6 col-md-6 col-lg-6 col-xl-6 pr-0">
								<div class="form-group row m-0 pb-1">
									...
									<div class="col-sm-8 col-md-8 col-lg-8 col-xl-8 p-0 pr-2 ">
										...
										<input name="receipt_no" type="text" id="receipt_no" size="15" value="<?=$tr_no?>" tabindex="1" class="form-control" />
							...
							<div class="col-sm-6 col-md-6 col-lg-6 col-xl-6 pl-0">
								<div class="form-group row m-0 pb-1">
									...
									<div class="col-sm-7 col-md-7 col-lg-7 col-xl-7 p-0 pr-2 ">
										...
										<input name="r_from" type="hidden" id="r_from" value="<?php echo $jv->received_from?>" class="input1"  tabindex="1" required <? if($jv->tr_no>0) echo 'readonly'?>/>
						...
						<div class="form-group row m-0 pb-1">
							...
							<div class="col-sm-10 col-md-10 col-lg-10 col-xl-10 p-0 pr-2">
								<input name="remarks" type="text" id="remarks" tabindex="3" value="<?php echo $jv->remarks?>" <? if($jv->tr_no>0) echo 'readonly'?> class="form-control"/>
							</div>
						</div>
						<div class="form-group row m-0 pb-1">
						...
						<!--<div class="form-group row m-0 pb-1">
							...
							<input name="file_upload" type="file" id="file_upload"
								...
								<?php $file_name = find_a_field('secondary_journal', 'file_upload', 'jv_no="' . $_SESSION['jv_no']['Contra']. '" and file_upload!=""');
								if ($file_name != '') { ?>
									<a href="<?= SERVER_CORE ?>core/upload_view.php?name=<?= $file_name ?>&folder=ContraVoucher&proj_id=<?= $_SESSION['proj_id'] ?>"
...
from  secondary_journal a where a.tr_from='Contra'  and SUBSTRING(a.tr_no,5,1)='0' and  a.dr_amt>0 and a.checked in ('NO','YES') and a.group_for=".$_SESSION['user']['group']." group by a.tr_no order by a.tr_no desc limit 5";
		...
		<div class="container-fluid pt-4 p-0 ">
			...
			<table class="table1  table-striped table-bordered table-hover table-sm">
				...
				<tr>
					...
					<td><select name="t_type" id="t_type" onchange="mytype()" tabindex="4" class="form-control"><option>Debit</option><option>Credit</option></select></td>

					<td><input type="text" class="form-control" id="ledger_id" name="ledger_id" onblur="getData2('acc_reference_ajax.php', 'acc_reference', this.value,document.getElementById('ledger_id').value);" tabindex="1"  value="<?=$jv_data->ledger_id?>"  />
					...
					<td>
						...
						<table width="100%" border="1" align="center"  cellpadding="2" cellspacing="2">
							<tr>
								<td class="p-0"><input type="text" tabindex="6" class="form-control" id="ledger_name" name="ledger_name" value="<?=$jv_data->ledger_name?>" />
									<input type="hidden" class="input1" id="reference_id" name="reference_id"  value="<?=$jv_data->reference_id?>"/>
...
<? foreign_relation('general_sub_ledger','sub_ledger_id','sub_ledger_name',$sub_ledger,"1");?>
					...
					<td>
						<span id="cur">
							<select name="cc_code" id="cc_code" tabindex="7"  class="form-control"  tabindex="3">
								<option></option>
					...
					<td>
						<input name="detail" type="text" id="detail" class="form-control"  tabindex="8" />					</td>

					<td align="center"><input name="d_amount" type="text" id="d_amount" size="10" class="form-control" tabindex="9"/></td>

					<td align="center"><input name="c_amount" type="text" id="c_amount" size="10" class="form-control" tabindex="10"/></td>
...
from  secondary_journal a, accounts_ledger l where a.ledger_id=l.ledger_id and a.jv_no='".$_SESSION['jv_no']['Contra']."' and a.tr_from='Contra' ";
	...
	<form method="post">
		<tr>
			...
			<td colspan="4">
				<? $deffr = find_a_field('secondary_journal','sum(dr_amt-cr_amt)','tr_from="Contra" and jv_no='.$_SESSION['jv_no']['Contra']);

				?>
				<input name="receipt_varify" class="btn1 btn1-bg-submit" type="button" id="receipt_varify" <? if($deffr==0){ echo 'hi';}else{?>style="visibility:hidden"<? }?> value="Contra Verified" onclick="this.form.submit()" /> <input name="limmit" type="hidden" value="" />
				<input  name="csrf_token" type="hidden" id="csrf_token" value="<?=$_SESSION['csrf_token']?>"/>
			...
			<td colspan="3"><input name="t_amount" type="text" id="t_amount" size="15" readonly  value="<?php echo $deffr;?>"/></td>
...
<table width="98%" border="0" cellspacing="0" cellpadding="0" align="center" style="background: #EEE6F3  ">
...
<td colspan="3">
<input name="remarks" type="text" id="remarks" value="<?php echo $jv->remarks?>" <? if($jv->tr_no>0) echo 'readonly'?> class="form-control"/>
...
from  secondary_journal a where a.tr_from='Contra'  and SUBSTRING(a.tr_no,5,1)='0' and  a.dr_amt>0 and a.checked in ('NO','YES') group by a.tr_no order by a.tr_no desc limit 5";
...
?>
            ...
            <tr>
              ...
              <td><input type="text" class="form-control" id="ledger_id" name="ledger_id" onblur="getData2('acc_reference_ajax.php', 'acc_reference', this.value, 
document.getElementById('ledger_id').value);" tabindex="1"  value="<?=$jv_data->ledger_id?>"  /></td>
              <td align="center">			 

<span id="acc_reference">
<table width="100%" border="1" align="center"  style="border-collapse:collapse; border:1px solid #C1DAD7;" cellpadding="2" cellspacing="2">
	<tr> 
		<td><input type="text" class="form-control" id="ledger_name" name="ledger_name" value="<?=$jv_data->ledger_name?>" />
		<input type="hidden" class="input1" id="reference_id" name="reference_id" style="width:100px;"  value="<?=$jv_data->reference_id?>"/>
		

//<select name="reference_id" id="reference_id"  tabindex="2"  style="width:120px;">
//    <option></option>
//    <? foreign_relation('acc_reference','id','reference_name',$jv_data->reference_id,"ledger_id='".$jv_data->ledger_id."'");?>
//  </select>
              ...
              <td align="center"><span id="cur">
                ...
                <select name="cc_code" id="cc_code"  class="form-control"  tabindex="3">
  <option></option>
  <? foreign_relation('cost_center','id','center_name',$jv_data->cc_code,"1");?>
</select>
              ...
              <td align="center">
              	...
              	<input name="detail" type="text" id="detail" class="form-control"  tabindex="10" />              </td>
              ...
              <td align="center"><input name="d_amount" type="text" id="d_amount" size="10" class="form-control" tabindex="11"/></td>




              <td align="center"><input name="c_amount" type="text" id="c_amount" size="10" class="form-control" tabindex="11"/></td>
              </tr>
