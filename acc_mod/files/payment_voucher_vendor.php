...
$title='Mrr Wise Payment Voucher';
...
if(!isset($_GET['v_d'])&&!isset($_GET['v_type']))
	...
	if($voucher_mode==0&&$_POST['receipt_no']>0)
		...
		$receipt_no		   = next_invoice('tr_no','secondary_journal','Special Payment');

		$manual_receipt_no = next_invoice('tr_no','secondary_journal','Special Payment');
		...
		$receiptno=next_invoice('tr_no','secondary_journal','Special Journal');
...

{
	...
	$receiptno=next_invoice('tr_no','secondary_journal','Special Payment');
...
?>
<form action="payment_voucher_vendor_selection.php" id="form2" name="form2" method="post" >
...
from  secondary_journal a where a.tr_from='Special Payment' and a.group_for='".$_SESSION['user']['group']."'  and SUBSTRING(a.tr_no,5,1)='0' and  a.cr_amt>0 group by a.jv_no order by a.tr_no desc limit 6";
...
{$dataa[2]=substr($dataa[2],0,20).'...';
      ...
      <th width="12%">Gr Date </th>
      <th width="7%">Grn No #</th>

      <th width="9%">Party Name </th>

      <th width="10%">Total Amt </th>
      <th width="13%">Total Paid Amt </th>
      <th width="13%">Payable Amt</th>
		...
		//if($_POST['vendor_id']!='') {$vendor_con=' and r.vendor_id="'.$_POST['vendor_id'].'"';}
		  ...
		  $sql="SELECT  
				...
				WHERE
				  ...
				  j.ledger_id = l.ledger_id  and j.ledger_id='".$_POST['ledger_id']."' 

				  group by j.tr_no having sum(j.cr_amt)<> sum(j.dr_amt) order by j.jv_date ";
	  ...
	  $tot_paid_amt =  find_a_field('journal','sum(dr_amt)',' 1 and tr_id="'.$data->tr_no.'" and tr_from like "Special Payment" ');
	  $tot = $data->amt; 
	  $tot_payable_amt = ($tot-$tot_paid_amt);
      ...
      <td align="center"><div align="left"><input type="hidden" id="do_no_<?=$data->tr_no;?>" name="do_no_<?=$data->tr_no;?>" value="<?=$data->tr_no;?>" /><?=$data->tr_no;?></div></td>
      <td align="center"><div align="left"><? echo find_a_field('vendor','vendor_name','ledger_id='.$data->ledger_id) ;?></div></td>

      <td align="right"><? $tot;  echo number_format($tot,2);$received = $received + $tot;?> </td>
      <td align="right"><? echo $tot_paid_amt ; $total_paid+=$tot_paid_amt;?></td>
      <td align="right"><? echo $tot_payable_amt;?>
	  
	  
	  <input type="hidden" value="<?=$tot_payable_amt?>" name="tott_<?=$data->tr_no ?>" id="tott_<?=$i;?>" /></td>

      <td align="center"><button onclick="sum_sum(<?=$i;?>);calculateRBN()" class="btn btn-sm info" type="button">Full</button></td>

      <td align="center"><input type="text" name="v_amt_<?=$data->tr_no?>" id="v_amt_<?=$i;?>" style=" width: 90px; " onchange="sr_alert(<?=$i;?>);calculateRBN();" onblur="calculateRBN()" onkeyup="calculateRBN()" />
	  
	  <input type="hidden" name="vendor_ledger_id" value="<?=$_POST['ledger_id']?>" />
	  </td>
      <td align="center"><a target="_blank" href="purchase_sec_print_view.php?do_no=<?=$data->tr_no ?>"><img src="../images/print_hover.png" width="20" height="20" /></a></td>
      </tr>
  ...
  <br />
      <div align="center">
              <input type="submit" name="payment" class="btn btn-success" value="SUBMIT"  />
