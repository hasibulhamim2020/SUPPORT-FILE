...
var strURL="received_amt_ajax_edit.php?item_id="+item_id+"&receive_acc_head="+receive_acc_head+"&rec_date="+rec_date+"&ra="+ra+"&no="+no+"&flag="+flag;}
else
{
        ...
        <tr>
			<td><table width="50%" border="0" align="center" cellpadding="5" cellspacing="0" bgcolor="#FF9999">
              <tr>
                ...
                <td rowspan="2">                  <input type="submit" name="submitit" id="submitit" value="View DO Date" style="width:170px; font-weight:bold; font-size:12px; height:30px; color:#090"/>                </td>
              </tr>
              <tr>
...
a.acc_note
	 ...
	 a.rcv_amt>0 and a.status!='Manual' and b.AREA_CODE=d.area_code and a.dealer_code=d.dealer_code and d.dealer_type='Distributor'  and a.received_status='received' ".$con;
	 ...
	 $con .= " and a.do_date between '".$_POST['f_date']."' and '".$_POST['t_date']."'";
	  ...
	  ?>
	  <input name="rec_date_<?=$data[0]?>" type="text" id="rec_date_<?=$data[0]?>" value="<?=$data[11]?>" style="width:70px;" /></td>
      <td align="center"><div align="left">
...
<input type="button" name="Button" value="EDIT"  onclick="update_value(<?=$data[0]?>)" style="width:40px; height:20px;background-color:#FF3366;"/><? }else echo 'Set ACC CODE';?>
          </span></td>
      </tr>
	  <? }}?>
	        <tr class="alt">
