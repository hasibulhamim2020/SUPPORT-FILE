...
if($_POST['check']=='CHECK')
{
	...
	echo $ssql='update secondary_journal set jv_date="'.$voucher_date.'", checked_at="'.$time_now.'", checked_by="'.$_SESSION['user']['id'].'", checked="YES" , 	final_jv_no="'.$jv.'" where jv_no="'.$jv_no.'"';
	db_query($ssql);
	
	sec_journal_journal($jv_no,$jv,'StoreIssued');
}
}
$address=find_a_field('project_info','proj_address',"1");

$jv = find_all_field('secondary_journal','jv_date','jv_no='.$jv_no);
$ch = find_all_field('production_issue_master','pi_no','pi_no='.$jv->tr_no);
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" bgcolor="#CCCCCC">
  <tr>
    ...
    <td width="180" align="right"><a target="_blank" href="../../warehouse_mod/pages/SCS/print_view_price.php?req_no=<?=$ch->pi_no?>">PI-
        <?=$ch->pi_no?>
    </a></td>
...
<a target="_blank" href="../../warehouse_mod/pages/do/chalan_bill_view_acc.php?v_no=<?=$ch->pi_no?>"></a></div><? }else{?>
<div align="left">
<form action="" method="post">
<table width="100%" border="0" cellpadding="0" cellspacing="0" bgcolor="#339900">
  <tr>
    <td width="1"><input name="button" type="button" onclick="hide();window.print();" value="Print" /></td>
    <td width="200" align="right"><a target="_blank" href="../../warehouse_mod/pages/SCS/print_view_price.php?req_no=<?=$ch->pi_no?>">PI-
        <?=$ch->pi_no?>
    </a></td>
...
<a target="_blank" href="../../warehouse_mod/pages/pr/chalan_view2.php?v_no=<?=$ch->pi_no?>"></a></div><? }?>
</div></td>
        </tr>
      <tr>
        <td width="33%" class="tabledesign_text">Voucher Date :
          <?=date('d-m-Y',$jv->jv_date);?></td>
        <td width="33%" class="tabledesign_text">Chalan Date :<?=date('d-m-Y',strtotime($ch->pi_date));  ?></td>
        <td width="33%" class="tabledesign_text">Receive Date :
            <?=date('d-m-Y',strtotime($ch->receive_date));  ?></td>
      </tr>
      <tr>
        ...
        <td class="tabledesign_text">Receive SL No  : <?=$ch->rec_sl_no?>
          </td>
      </tr>
    </table></td>
  </tr>
