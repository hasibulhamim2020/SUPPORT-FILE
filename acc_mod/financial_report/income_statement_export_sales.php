...
$title='Comparative Income Statement Of Export Sales';
...
$sql = "select a.ledger_group_id, sum(j.dr_amt-j.cr_amt) as expense_amt
 ...
 where l.group_id=a.ledger_group_id and a.ledger_id=j.ledger_id and j.jv_date between '".$fdate."' and '".$tdate."' and l.acc_sub_sub_class in (315,4110)";
$query = db_query($sql);
while($data=mysqli_fetch_object($query)){
...
$total_export_sales=find_a_field('lc_master','sum(valueccc)','ldbc_date between "'.$fdate.'" and "'.$tdate.'" and up_no>0');
//echo 'select sum(realize_amt_taka) from lc_master where lc_date between "'.$fdate.'" and "'.$tdate.'"';
$comparative_total_export_sales=find_a_field('lc_master','sum(valueccc)','ldbc_date between "'.$cfdate.'" and "'.$ctdate.'" and up_no>0');
?>
...
<td align="right"><a href="export_sales_details.php?show=show&fdate=<?=$_REQUEST['fdate']?>&tdate=<?=$_REQUEST['tdate']?>" target="_blank"> 
<?=number_format($total_export_sales,2); $tot_sales_amt +=$total_export_sales;?> USD </a></td>
									  <td align="right"> 
                                        <?=number_format($comparative_total_export_sales,2); $tot_sales_amt_cm +=$comparative_total_export_sales;?> USD
									    </td>
									  </tr>
