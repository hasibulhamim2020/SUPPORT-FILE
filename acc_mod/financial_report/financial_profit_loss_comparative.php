...
$total_cogs_amt_cm=$material_cost_cm+$expense_amt_cm;
 ...
 $sql_sub1="select s.id, s.sub_class_name from acc_sub_class s where s.acc_class=3 and s.id=32
group by s.id";
...
?>
		 ...
		 <a href="financial_transaction_group.php?show=show&fdate=<?=$_REQUEST['fdate']?>&tdate=<?=rawurlencode(url_encode($_REQUEST['tdate']));?>&acc_sub_sub_class=<?=$data2->id?>" target="_blank">
		 <?=number_format($expenses_amt[$data2->id],2); $tot_expenses_amt +=$expenses_amt[$data2->id];?></a></td>
									      <td align="right"><a href="financial_transaction_group.php?show=show&amp;fdate=<?=rawurlencode(url_encode($_REQUEST['cfdate']));?>&amp;tdate=<?=$_REQUEST['ctdate']?>&amp;acc_sub_sub_class=<?=$data2->id?>" target="_blank">
...
$sql1="select ss.id, ss.sub_sub_class_name from acc_sub_sub_class ss where  ss.acc_sub_class=31 order by ss.id";
