...
$sql = "select l.group_id, sum(j.dr_amt-j.cr_amt) as opening_amt from acc_sub_sub_class s, ledger_group l, accounts_ledger a, journal j
 where s.id=l.acc_sub_sub_class and l.group_id=a.ledger_group_id and a.ledger_id=j.ledger_id and  j.jv_date<'".$fdate."' group by l.group_id";
$query = db_query($sql);
...
$opening_amt[$data->group_id]=$data->opening_amt;
 ...
 $sql = "select l.group_id, sum(j.dr_amt) as dr_amt, sum(j.cr_amt) as cr_amt from acc_sub_sub_class s, ledger_group l, accounts_ledger a, journal j
 where s.id=l.acc_sub_sub_class and l.group_id=a.ledger_group_id and a.ledger_id=j.ledger_id and  j.jv_date between '".$fdate."' and '".$tdate."' group by l.group_id";
$query = db_query($sql);
while($data=mysqli_fetch_object($query)){
...
$sql = "select l.group_id, sum(j.dr_amt-j.cr_amt) as closing_amt from acc_sub_sub_class s, ledger_group l, accounts_ledger a, journal j
 where s.id=l.acc_sub_sub_class and l.group_id=a.ledger_group_id and a.ledger_id=j.ledger_id and  j.jv_date<='".$tdate."' group by l.group_id";
$query = db_query($sql);
while($data=mysqli_fetch_object($query)){
...
<td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="financial_transaction_ledger_closing.php?show=show&fdate=<?=$_REQUEST['fdate']?>&tdate=<?=$_REQUEST['tdate']?>&group_id=<?=$data1->group_id?>" target="_blank" style="color:#000000;">
