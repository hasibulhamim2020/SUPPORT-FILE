...
$advance =  explode("#",$_POST['advance_ledger']);

//$advance_to_vendor_dr = $advance[1];
$advance_to_vendor_dr=$config_ledger->supplier_advance_control;
            ...
            ?>
                ...
                <table class="table1  table-striped table-bordered table-hover table-sm">
					...
					<tr class="bgc-info">
					 ...
					 <th colspan="5">Cash/Bank<input type="text" id="receive_sub_ledger" name="receive_sub_ledger" list="cash_bank" required/>
					 ...
					 <?php /*?><th>Advance G/L<input type="text" id="advance_ledger" name="advance_ledger" list="advance_ledgers"/>

					 <datalist id="advance_ledgers">

							  <? foreign_relation('accounts_ledger','concat(ledger_name,"#",ledger_id)','""',$advance_to_vendor_dr,'1')?>
...
<?php */?>
					...
					$paid_sql = "select sum(dr_amt) as amt,ledger_id,sub_ledger,tr_no from journal where tr_from ='Vendor_payment' group by ledger_id,sub_ledger,tr_no";
					
					$paid_query =db_query($paid_sql);
					
					while($paid_data=mysqli_fetch_object($paid_query)){
					
						$paid_amount[$paid_data->ledger_id][$paid_data->sub_ledger][$paid_data->tr_no]=$paid_data->amt;
					
					}
					
					

					//$advance_to_vendor_dr = "1022400004,1022400006";
					
					
					$adv_sql =" select sum(dr_amt-cr_amt) as amt,sub_ledger from journal where  ledger_id in (".$advance_to_vendor_dr.") and group_for = ".$_POST['group_for']." group by sub_ledger  ";		
					
					$adv_query = db_query($adv_sql);
					
					
					while($adv_data=mysqli_fetch_object($adv_query)){
					
						$advance_amt[$adv_data->sub_ledger]=$adv_data->amt;						

					}
					
					

					  $res='select m.invoice_no,m.system_invoice_no,m.invoice_date,m.po_no,m.grn_no,sum(d.amount) as amount,m.tds_percent,v.vendor_name,v.ledger_id,v.sub_ledger_id,m.entry_at,u.fname,m.tds_amount,m.deductible 
					
					from vendor_invoice_master m, vendor_invoice_details d, vendor v, user_activity_management u 
					
					where m.system_invoice_no=d.system_invoice_no and m.vendor_id=v.vendor_id and m.entry_by=u.user_id and m.status="PENDING" and m.po_type=2 '.$con.' group by m.system_invoice_no';
					...
					//$tds_amount = $row->tds_amount;//($row->amount*$row->tds_percent)/100;
					...
					//$advance_amt = find_a_field('journal','sum(dr_amt-cr_amt)','ledger_id in  ('.$advance_to_vendor_dr.') and sub_ledger="'.$row->sub_ledger_id.'" and group_for='.$_POST['group_for'].' ');

					

					/*$advance_amt_adjusted = find_a_field('journal','sum(cr_amt)','ledger_id="'.$advance_to_vendor_dr.'" and sub_ledger="'.$row->sub_ledger_id.'" and tr_no="'.$row->po_no.'" and tr_from="Vendor_payment"');*/

					

					
					if($advance_amt[$row->sub_ledger_id]>=1){
					$rest_advance_amt = $advance_amt[$row->sub_ledger_id];//$advance_amt-$advance_amt_adjusted;
					}else{
						$rest_advance_amt =0;
					}
					...
					$unpaid_amount = number_format($amount_with_vat-($paid_amount[$doc_vendor][$row->sub_ledger_id][$row->system_invoice_no]+$vds_amount+$tds_amount),2,'.','');
...
<input type="checkbox" name="system_invoice_no<?=$row->system_invoice_no?>" class="form-control" id="system_invoice_no<?=$row->system_invoice_no?>" onclick="payment_cal(<?=$row->system_invoice_no?>)" /></td>
                            ...
                            <input type="hidden" name="advance_deductable_old<?=$row->system_invoice_no?>" id="advance_deductable_old<?=$row->system_invoice_no?>" value="<?=$advance_deductable?>" />                            </td>

                            

                          <td colspan="2"><input type="text" name="actual_payable<?=$row->system_invoice_no?>" id="actual_payable<?=$row->system_invoice_no?>" value="<?=$unpaid_amount?>" onChange="advance_cal(<?=$row->system_invoice_no?>)" /><input type="hidden" name="actual_payable_old<?=$row->system_invoice_no?>" id="actual_payable_old<?=$row->system_invoice_no?>" value="<?=$unpaid_amount?>" /></td>
...
<td><?=$vds_amount?></td>
					...
					<tr>
					  ...
					  <td colspan="12"><input type="submit" name="confirm_payment" id="confirm_payment" value="Confirm Payment" /></td>
