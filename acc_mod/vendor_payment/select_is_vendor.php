...
$title='Inter Purchase Vendor Payment';
...
$target_url = '../vendor_invoice/new_is_bill_create.php';
...
$res='select m.invoice_no,m.system_invoice_no,m.invoice_date,m.po_no,m.grn_no,sum(d.amount) as amount,m.tds_percent,v.sub_ledger_name as vendor_name,v.sub_ledger_id,m.entry_at,u.fname,m.group_for,v.ledger_id as accounts_payable from is_invoice_master m,is_invoice_details d, general_sub_ledger v, user_activity_management u where m.system_invoice_no=d.system_invoice_no and m.vendor_id=v.sub_ledger_id and m.entry_by=u.user_id and m.status="PENDING"  '.$con.' group by m.system_invoice_no';
...
$paid_amount = find_a_field('journal','sum(dr_amt)','ledger_id="'.$payable_ledger.'" and sub_ledger="'.$data->sub_ledger_id.'" and tr_no="'.$data->system_invoice_no.'" and tr_from="Vendor_payment"');
                ...
                <div class="col-sm-5 col-md-5 col-lg-5 col-xl-5">
                    ...
                    <div class="form-group row m-0">
                        ...
                        <div class="col-sm-8 col-md-8 col-lg-8 col-xl-8 p-0">
							...
							<datalist id="vendor">
							  <? foreign_relation('general_sub_ledger','concat(sub_ledger_name,"#",sub_ledger_id)','""',$vendor_id,'1 and ledger_id="3022600018" ')?>
							</datalist>
                        </div>
                    </div>
            ...
            ?>
                ...
                <table class="table1  table-striped table-bordered table-hover table-sm">
					...
					<tr class="bgc-info">
					 ...
					 <th colspan="4">Cash/Bank<input type="text" id="receive_sub_ledger" name="receive_sub_ledger" list="cash_bank"/>
					 ...
					 <th>Payment Date<input type="date" id="cheq_date" name="cheq_date"/></th>
					...
					$payable_ledger = 3022600018;
					$advance_to_vendor_dr = 1022400004;
					 $res='select m.invoice_no,m.system_invoice_no,m.invoice_date,m.po_no,m.grn_no,sum(d.amount) as amount,m.tds_percent,v.sub_ledger_name as vendor_name,v.sub_ledger_id,v.sub_ledger_id,m.entry_at,u.fname from is_invoice_master m, is_invoice_details d, general_sub_ledger v, user_activity_management u where m.system_invoice_no=d.system_invoice_no and m.vendor_id=v.sub_ledger_id and m.entry_by=u.user_id and m.status="PENDING"  '.$con.' group by m.system_invoice_no';
					$query = db_query($res);
                    while($row = mysqli_fetch_object($query)){
					
					$vat_amount = ($row->amount-($row->amount*100)/115);
					
					
					
					$tds_amount = number_format((($row->amount-$vat_amount)*$row->tds_percent)/100,2,'.','');
					
					
					$po_info = find_all_field('purchase_master','','po_no="'.$row->po_no.'"');
					$vat_amount = ($row->amount*$po_info->vat)/100;
					$amount_with_vat = $row->amount;
					$amount_with_out_vat = $row->amount-$tds_amount;
				    ...
				    $paid_amount = find_a_field('journal','sum(dr_amt)','ledger_id="'.$payable_ledger.'" and sub_ledger="'.$row->sub_ledger_id.'" and tr_no="'.$row->system_invoice_no.'" and tr_from="Vendor_payment"');
					
					
					/*$advance_amt = find_a_field('journal','sum(dr_amt)','ledger_id="'.$advance_to_vendor_dr.'" and sub_ledger="'.$row->sub_ledger_id.'" and tr_no="'.$row->po_no.'" and tr_from="Inter_vendor_advance_payment"');*/
					...
					/*$advance_amt_adjusted = find_a_field('journal','sum(cr_amt)','ledger_id="'.$advance_to_vendor_dr.'" and sub_ledger="'.$row->sub_ledger_id.'" and tr_no="'.$row->po_no.'" and tr_from="Inter_vendor_payment"');*/
					...
					$unpaid_amount = $amount_with_vat-($paid_amount+$vds_amount+$tds_amount);
					$doc_payable = $amount_with_out_vat;
					if($unpaid_amount>0){
