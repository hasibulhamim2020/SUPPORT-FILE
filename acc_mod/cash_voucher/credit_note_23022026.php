...
if ($_GET['jv_no']['Receipt'] > 0) {
	...
	$_SESSION['jv_no']['Receipt_cash'] = $_GET['jv_no']['Receipt'];

}

if ($_GET['tr_no']['Receipt'] > 0) {

	$_SESSION['tr_no']['Receipt_cash'] = $_GET['tr_no']['Receipt'];
...
if (isset($_POST['add']) && $_SESSION['csrf_token'] === $_POST['csrf_token']) {
	...
	if ($_SESSION['jv_no']['Receipt_cash'] == 0) {
		...
		$jv_no = $_SESSION['jv_no']['Receipt_cash'] = next_journal_sec_voucher_id($tr_from, 'Receipt', $group_for, $_SESSION['user']['id']);
	...
	$field = 'file_upload';
	$file_name = $field . $_SESSION['jv_no']['Receipt_cash'];
	...
	if ($receive_ledger > 0 && $ledger_id > 0 && $jv_date != '' && $jv_date != '0000-00-00') {
		if ($jv_no > 0) {
			add_to_sec_journal($proj_id, $jv_no, $jv_date, $ledger_id, $dnarr, '0', $amount, $tr_from, $tr_no, $sub_ledger, '', $cc_code, $group_for, $user_id, '', $r_from, $bank, $c_no, $cheq_date, $receive_ledger, $checked, $type, $employee, $remarks, $uploaded_file, $reference_id);

			$up = 'update secondary_journal set manual_voucher_no="' . $manual_voucher_no . '", exchange_rate="' . $exchange_rate . '", usd_amt="' . $usd_amt . '" where jv_no="' . $_SESSION['jv_no']['Receipt_cash'] . '"';
			db_query($up);
...
if (isset($_POST['limmit']) && $_SESSION['csrf_token'] === $_POST['csrf_token']) {
	...
	if ($_SESSION['tr_no']['Receipt_cash'] > 0) {
		...
		if ($_SESSION['jv_no']['Receipt_cash'] > 0) {
			...
			$up = 'update secondary_journal set checked="NO",    manual_voucher_no="' . $j_data->manual_voucher_no . '", exchange_rate="' . $j_data->exchange_rate . '", usd_amt="' . $j_data->usd_amt . '",reconsilation_status="' . $reconsilation_type . '",reconsilation_note="' . $reconsilation_note . '" where jv_no="' . $_SESSION['jv_no']['Receipt_cash'] . '" and tr_from="' . $tr_from . '"';
			db_query($up);
					...
					<div class="container n-form2">
						...
						<div class="row pb-1">
							...
							<div class="col-sm-6 col-md-6 col-lg-6 col-xl-6 pr-0">
								<div class="form-group row m-0 pb-1">
									<label
										class="col-sm-4 col-md-4 col-lg-4 col-xl-4 m-0 p-0 d-flex justify-content-end align-items-center pr-1 bg-form-titel-text font-size12 ">Received
										From:</label>
									<div class="col-sm-8 col-md-8 col-lg-8 col-xl-8 p-0 pr-2 ">
										<input name="r_from" type="text" id="r_from"
											value="<?php echo $jv->received_from ?>" class="form-control" tabindex="3"
											<? if ($jv->tr_no > 0) {
												echo 'readonly';
											} ?> />
							...
							<div class="col-sm-6 col-md-6 col-lg-6 col-xl-6 pl-0">
								<div class="form-group row m-0 pb-1">
									<label
										class="col-sm-5 col-md-5 col-lg-5 col-xl-5 m-0 p-0 d-flex justify-content-end align-items-center pr-1 bg-form-titel-text  font-size12">Receive
										Mode:</label>
									<div class="col-sm-7 col-md-7 col-lg-7 col-xl-7 p-0 pr-2 ">
										...
										<? if ($jv->tr_no > 0) { ?><input type="text" name="type" id="type"
												...
												<option value="CASH" <?= ($jv->type == 'CASH') ? 'Selected' : ''; ?>>CASH
												</option>
											</select><? } ?>
						...
						<div class="form-group row m-0 pb-1">
							<label
								class="col-sm-3 col-md-3 col-lg-3 col-xl-3 m-0 p-0 d-flex justify-content-end align-items-center pr-1 font-size12 bg-form-titel-text">Company
								: </label>
							<div class="col-sm-9 col-md-9 col-lg-9 col-xl-9 p-0 pr-2 ">
								<? if ($_POST['group_for'] > 0) { ?>
									<input type="hidden" name="group_for" id="group_for" value="<?= $_POST['group_for']; ?>"
										class="m-0" readonly />
									<input type="text" name="group_for_show" id="group_for_show"
										value="<?= find_a_field('user_group', 'group_name', 'id=' . $_POST['group_for']); ?>"
										readonly="" />
...
from  secondary_journal a where a.tr_from='Receipt'  and SUBSTRING(a.tr_no,5,1)='0' and a.type='CASH' and a.dr_amt>0 and a.group_for in (" . user_company_access_list() . ") group by a.tr_no order by a.id desc limit 5";
								...
								if (mysqli_num_rows($data2) > 0) {
									...
									while ($dataa = mysqli_fetch_row($data2)) {
										...
										<tr>
											...
											<?php /*?><td style="padding:1px;"><a href="general_voucher_print_view_for_draft.php?jv_no=<?php echo rawurlencode(url_encode($dataa[4])); ?>" target="_blank" onclick="custom('<?=url_encode($dataa[4]);?>','<?=url_encode($c_id);?>');"><img src="../images/print.png" width="16" height="16" border="0"></a></td><?php */ ?>

											<td style="padding:1px;">
												<a href="general_voucher_print_view_for_draft.php?c=<?= rawurlencode(url_encode($c_id)); ?>&v=<?= rawurlencode(url_encode($dataa[4])); ?>"
													target="_blank">
													<img src="../images/print.png" width="16" height="16" border="0">
												</a>
											</td>



											<?php /*?><td style="padding:1px;"><a
														   href=""
															onclick="custom('<?=url_encode($dataa[4]);?>','<?=url_encode($c_id);?>');"><img src="../images/print.png" width="16" height="16"
															   border="0"></a></td><?php */ ?>
						...
						<td align="center">
							<span id="fetch_sub_ledger">
								...
								<datalist id="sub_ledgers">
									<option></option>
									<? foreign_relation('general_sub_ledger', 'sub_ledger_id', 'sub_ledger_name', $sub_ledger, " group_for=" . $_POST['group_for'] . ""); ?>
								</datalist>
							</span>
						</td>
						<td>
