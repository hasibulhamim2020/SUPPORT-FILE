...
<!--				<p class="text">Cell: <?=$group_data->mobile?>. Email: <?=$group_data->email?> <br> <?=$group_data->vat_reg?></p>-->
		...
		<td width="17%" align="right" class="qr-code">
              ...
              $print_url = "https://saaserp.ezzy-erp.com/app/views/acc_mod/cash_voucher/general_voucher_print_view_for_draft.php?c=" . rawurlencode($company_id) . "&v=" . rawurlencode($req_no_qr_data) . "&sub_ledger" . rawurlencode($sub_ledger);
...
while($info=mysqli_fetch_object($data2)){		  
      ...
      <tr>
		...
		<? if ($jv_all->tr_from=='Payment') { ?>
		
		
					<?php /*?><td class="general_voucher" align="left"><a style="
    text-decoration: none !important; target="_blank" href="general_voucher_print_view_for_draft_client.php?jv_no=<?=rawurlencode(url_encode($jv_no))?>&sub_ledger=<?=rawurlencode(url_encode($info->sub_ledger))?>">
					  <?=$info->ledger_id?>
					</a></td><?php */?>
					
					
					<td class="general_voucher" align="left"><a style="
    text-decoration: none !important; target="_blank" href="general_voucher_print_view_for_draft_client.php?c=<?=rawurlencode(url_encode($c_id))?>&v=<?=rawurlencode(url_encode($jv_no))?>&sub_ledger=<?=rawurlencode(url_encode($info->sub_ledger))?>">
					  <?=$info->ledger_id?>
					</a></td>
        ...
        <td align="left"><div align="center">
          <?=++$s;?>
			...
			<? if ($jv_all->tr_from=='Receipt') { ?>
				<?php /*?>	<td align="left"><a style="text-decoration: none;" target="_blank" href="general_voucher_print_view_for_draft_client.php?jv_no=<?=rawurlencode(url_encode($jv_no))?>&sub_ledger=<?=rawurlencode(url_encode($info->sub_ledger))?>"><?=$info->ledger_id?></a></td><?php */?>
					
					
					<td align="left"><a style="text-decoration: none;" target="_blank" href="general_voucher_print_view_for_draft_client.php?c=<?=rawurlencode(url_encode($c_id))?>&v=<?=rawurlencode(url_encode($jv_no))?>&sub_ledger=<?=rawurlencode(url_encode($info->sub_ledger))?>"><?=$info->ledger_id?></a></td>
...
<?php }?>
	...
	<p class="p bold">In Words : <? echo convertNumberMhafuz($ttc); ?> </p>
	...
	<table class="footer-table">
        ...
        <tr>
		  ...
		  <p style="font-size:11px; margin: 0;">
						(<?=find_a_field('user_activity_management','designation','user_id='.$jv->entry_by);?>) <br/> <?=$jv->entry_at?></p>
		  </td>
          <td class="text-center w-20">
		  <p style="font-weight:600; margin: 0;"> 
						<?=find_a_field('user_activity_management','fname','user_id='.$jv->om_checked);?></p>
		  <p style="font-size:11px; margin: 0; margin-top:4rem;">
			...
			<?php } ?>
						...
						(<?=find_a_field('user_activity_management','designation','user_id='.$jv->om_checked);?>) <br/> <?=$jv->om_checked_at?></p>
		  </td>
