...
$ledger_dr[$j_data[0]]=$j_data[2];
      ...
      $date1=find_a_field('opening','op_date','1 order by id desc');
	  $_SESSION['op_date']=$date1;
?>
...
<input name="op_date" type="text" id="op_date" size="30" maxlength="100" class="required" minlength="4" style="width:130px;"  
value="<?php echo $_POST['op_date'];?>" /></td>
...
?>
                    ...
                    <td><label>
                     ...
                     <input type="submit" name="submitit" id="submitit" value="Set Opening Balance" style="width:170px; font-weight:bold; font-size:12px; height:30px; color:#090"/>
...
<?php if(isset($_POST['submitit'])){
      ...
      $insql="INSERT INTO opening (op_date) VALUES ('".$_POST['op_date']."') ";
      $isql=db_query($insql);
	 //echo  $last_id=mysqli_insert_id($isql);
...
//$jv->dr_amt = $opening;
      ...
      <td align="center"><input name="jv_date_<?=$data[0]?>" type="text" id="jv_date_<?=$data[0]?>" style="width:70px;" readonly value="2022-05-01" />
